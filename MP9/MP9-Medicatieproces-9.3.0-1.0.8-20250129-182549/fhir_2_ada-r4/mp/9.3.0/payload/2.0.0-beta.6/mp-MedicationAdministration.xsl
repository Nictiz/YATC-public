...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:template match="f:MedicationAdministration"
      ...
      <medicatietoediening>
         ...
         <xsl:apply-templates select="f:supportingInformation"
                              mode="#current"/>
         <!-- relatie_wisselend_doseerschema relatie_wisselend_doseerschema up until MP9.3 beta.3 -->
   ...
   <xsl:template match="f:request"
      ...
      <xsl:choose>
         ...
         <!-- MP-1606: starting from MP9.3 beta.4 request is only mapped to relatie_medicatieafspraak -->
         ...
         <xsl:otherwise>
            <xsl:call-template name="util:logMessage">
               <xsl:with-param name="level"
                               select="$logWARN"/>
               <xsl:with-param name="msg">Encountered a medicationAdministration.request that could not be mapped to an ada element. Please investigate. 00000 
<xsl:value-of select="$outputMaOrWds"/>
   ...
   <xsl:template match="f:supportingInformation"
      ...
      <!-- Template to convert f:supportingInformation to relatie_toedieningsafspraak and relatie_wisselend_doseerschema
            ...
            finally we may have to assumxe a type of reference, which we think is better than to delete whatever was in derivedFrom/identifier -->
      ...
      <xsl:choose>
         ...
         <xsl:when test="$resolvedResource[f:MedicationDispense[f:identifier][f:category/f:coding/f:code/@value = $wdsCode]]">
            <!-- WDS -->
            <relatie_toedieningsafspraak>
               <xsl:call-template name="Identifier-to-identificatie">
                  <xsl:with-param name="in"
