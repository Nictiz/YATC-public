...
<!-- == Provenance: YATC-internal/ada-2-fhir/env/mp/9.0.7/fhir-xsd/STU3-sch/careteam_svrl.xsl == -->
...
<axsl:value-of select="$fileDirParameter"/>
   ...
   <!--RULE -->
   <axsl:template match="f:CareTeam/f:context"
                  priority="1020"
                  mode="M4">
      <svrl:fired-rule context="f:CareTeam/f:context"
                       xmlns:svrl="http://purl.oclc.org/dsdl/svrl"/>
      <!--ASSERT -->
      <axsl:choose>
         <axsl:when test="not(starts-with(f:reference/@value, '#')) or exists(ancestor::*[self::f:entry or self::f:parameter]/f:resource/f:*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, '#')]|/*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, '#')])"/>
   ...
   <!--RULE -->
   <axsl:template match="f:CareTeam/f:context/f:identifier/f:period"
                  priority="1019"
                  mode="M4">
      <svrl:fired-rule context="f:CareTeam/f:context/f:identifier/f:period"
                       xmlns:svrl="http://purl.oclc.org/dsdl/svrl"/>
      <!--ASSERT -->
      <axsl:choose>
         <axsl:when test="not(exists(f:start)) or not(exists(f:end)) or (f:start/@value &lt;= f:end/@value)"/>
   ...
   <!--RULE -->
   <axsl:template match="f:CareTeam/f:context/f:identifier/f:assigner"
                  priority="1018"
                  mode="M4">
      <svrl:fired-rule context="f:CareTeam/f:context/f:identifier/f:assigner"
                       xmlns:svrl="http://purl.oclc.org/dsdl/svrl"/>
      <!--ASSERT -->
      <axsl:choose>
         <axsl:when test="not(starts-with(f:reference/@value, '#')) or exists(ancestor::*[self::f:entry or self::f:parameter]/f:resource/f:*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, '#')]|/*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, '#')])"/>
   ...
   <axsl:template match="f:CareTeam/f:participant"
      ...
      <axsl:choose>
         ...
         <axsl:otherwise>
            <svrl:failed-assert test="contains(f:member/f:reference/@value, '/Practitioner/') or not(exists(f:onBehalfOf))"
                                xmlns:svrl="http://purl.oclc.org/dsdl/svrl">
               <axsl:attribute name="location">
                  <axsl:apply-templates select="."
                                        mode="schematron-select-full-path"/>
