...
<!-- == Provenance: YATC-internal/ada-2-fhir/env/mp/9.0.7/fhir-xsd/STU3-sch/condition_svrl.xsl == -->
...
<axsl:value-of select="$fileDirParameter"/>
   ...
   <axsl:template match="f:Condition"
      ...
      <axsl:choose>
         ...
         <axsl:otherwise>
            <svrl:failed-assert test="not(f:abatementBoolean/@value=true() or (not(exists(f:abatementBoolean)) and exists(*[starts-with(local-name(.), 'abatement')])) or f:clinicalStatus/@value=('resolved', 'remission', 'inactive'))"
                                xmlns:svrl="http://purl.oclc.org/dsdl/svrl">
               <axsl:attribute name="location">
                  <axsl:apply-templates select="."
                                        mode="schematron-select-full-path"/>
      ...
      <axsl:choose>
         ...
         <axsl:otherwise>
            <svrl:failed-assert test="f:verificationStatus/@value='entered-in-error' or exists(f:clinicalStatus)"
                                xmlns:svrl="http://purl.oclc.org/dsdl/svrl">
               <axsl:attribute name="location">
                  <axsl:apply-templates select="."
                                        mode="schematron-select-full-path"/>
   ...
   <!--RULE -->
   <axsl:template match="f:Condition/f:context"
                  priority="1026"
                  mode="M4">
      <svrl:fired-rule context="f:Condition/f:context"
                       xmlns:svrl="http://purl.oclc.org/dsdl/svrl"/>
      <!--ASSERT -->
      <axsl:choose>
         <axsl:when test="not(starts-with(f:reference/@value, '#')) or exists(ancestor::*[self::f:entry or self::f:parameter]/f:resource/f:*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, '#')]|/*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, '#')])"/>
   ...
   <!--RULE -->
   <axsl:template match="f:Condition/f:context/f:identifier/f:period"
                  priority="1025"
                  mode="M4">
      <svrl:fired-rule context="f:Condition/f:context/f:identifier/f:period"
                       xmlns:svrl="http://purl.oclc.org/dsdl/svrl"/>
      <!--ASSERT -->
      <axsl:choose>
         <axsl:when test="not(exists(f:start)) or not(exists(f:end)) or (f:start/@value &lt;= f:end/@value)"/>
   ...
   <!--RULE -->
   <axsl:template match="f:Condition/f:context/f:identifier/f:assigner"
                  priority="1024"
                  mode="M4">
      <svrl:fired-rule context="f:Condition/f:context/f:identifier/f:assigner"
                       xmlns:svrl="http://purl.oclc.org/dsdl/svrl"/>
      <!--ASSERT -->
      <axsl:choose>
         <axsl:when test="not(starts-with(f:reference/@value, '#')) or exists(ancestor::*[self::f:entry or self::f:parameter]/f:resource/f:*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, '#')]|/*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, '#')])"/>
