...
<!-- == Provenance: YATC-internal/ada-2-fhir/env/mp/9.0.7/fhir-xsd/STU3-sch/namingsystem_svrl.xsl == -->
...
<axsl:value-of select="$fileDirParameter"/>
   ...
   <axsl:template match="f:NamingSystem"
      ...
      <axsl:choose>
         ...
         <axsl:otherwise>
            <svrl:failed-assert test="not(f:replacedBy) or f:status/@value='retired'"
                                xmlns:svrl="http://purl.oclc.org/dsdl/svrl">
               <axsl:attribute name="location">
                  <axsl:apply-templates select="."
                                        mode="schematron-select-full-path"/>
   ...
   <!--RULE -->
   <axsl:template match="f:NamingSystem/f:replacedBy"
                  priority="1002"
                  mode="M4">
      <svrl:fired-rule context="f:NamingSystem/f:replacedBy"
                       xmlns:svrl="http://purl.oclc.org/dsdl/svrl"/>
      <!--ASSERT -->
      <axsl:choose>
   ...
   <!--RULE -->
   <axsl:template match="f:NamingSystem/f:replacedBy/f:identifier/f:period"
                  priority="1001"
                  mode="M4">
      <svrl:fired-rule context="f:NamingSystem/f:replacedBy/f:identifier/f:period"
                       xmlns:svrl="http://purl.oclc.org/dsdl/svrl"/>
      <!--ASSERT -->
      <axsl:choose>
         <axsl:when test="not(exists(f:start)) or not(exists(f:end)) or (f:start/@value &lt;= f:end/@value)"/>
   ...
   <!--RULE -->
   <axsl:template match="f:NamingSystem/f:replacedBy/f:identifier/f:assigner"
                  priority="1000"
                  mode="M4">
      <svrl:fired-rule context="f:NamingSystem/f:replacedBy/f:identifier/f:assigner"
                       xmlns:svrl="http://purl.oclc.org/dsdl/svrl"/>
      <!--ASSERT -->
      <axsl:choose>
         <axsl:when test="not(starts-with(f:reference/@value, '#')) or exists(ancestor::*[self::f:entry or self::f:parameter]/f:resource/f:*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, '#')]|/*/f:contained/f:*[f:id/@value=substring-after(current()/f:reference/@value, '#')])"/>
