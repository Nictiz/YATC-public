...
<!-- == Provenance: YATC-internal/fhir-2-ada/env/zibs2017/payload/zib-DispenseRequest-2.2.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:variable name="zib-Dispense-Location"
                 select="'http://nictiz.nl/fhir/StructureDefinition/zib-Dispense-Location'"/>
   <xsl:variable name="zib-DispenseRequest-RelatedMedicationAgreement"
                 select="'http://nictiz.nl/fhir/StructureDefinition/zib-DispenseRequest-RelatedMedicationAgreement'"/>
   <!-- ================================================================== -->
   <xsl:template match="f:MedicationRequest"
      ...
      <verstrekkingsverzoek>
         ...
         <xsl:apply-templates select="f:authoredOn"
                              mode="#current"/>
         <!--auteur-->
         <xsl:apply-templates select="f:requester/f:agent"
                              mode="#current"/>
         <!--te_verstrekken_hoeveelheid-->
         <xsl:apply-templates select="f:dispenseRequest/f:quantity"
                              mode="#current"/>
         <!--aantal_herhalingen-->
         ...
         <xsl:if test="f:dispenseRequest/f:expectedSupplyDuration or f:dispenseRequest/f:validityPeriod">
            <verbruiksperiode>
               <xsl:apply-templates select="f:dispenseRequest/f:validityPeriod"
                                    mode="#current"/>
               <xsl:apply-templates select="f:dispenseRequest/f:expectedSupplyDuration"
                                    mode="#current"/>
            </verbruiksperiode>
         </xsl:if>
         <!--beoogd_verstrekker-->
         <xsl:apply-templates select="f:dispenseRequest/f:performer"
                              mode="#current"/>
         <!--afleverlocatie-->
         <xsl:apply-templates select="f:dispenseRequest/f:extension[@url = $zib-Dispense-Location]"
                              mode="#current"/>
         <!--aanvullende_wensen-->
         <xsl:apply-templates select="f:extension[@url='http://nictiz.nl/fhir/StructureDefinition/zib-Medication-AdditionalInformation']"
            ...
            <xsl:with-param name="adaElementName"
                            tunnel="yes"
                            select="'aanvullende_wensen'"/>
         </xsl:apply-templates>
         <!--toelichting-->
         <xsl:apply-templates select="f:note"
                              mode="#current"/>
         <!--relatie_naar_medicatieafspraak-->
         <xsl:apply-templates select="f:extension[@url = $zib-DispenseRequest-RelatedMedicationAgreement]"
                              mode="#current"/>
      </verstrekkingsverzoek>
   </xsl:template>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
         ...
         <xsl:choose>
            <xsl:when test="f:extension[@url = $practitionerrole-reference]">
               <xsl:variable name="referenceValue"
                             select="f:extension[@url = $practitionerrole-reference]/f:valueReference/f:reference/@value"/>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="f:expectedSupplyDuration"
                 mode="zib-DispenseRequest-2.2">
   ...
   <xsl:template match="f:performer"
      ...
      <beoogd_verstrekker>
         <xsl:choose>
            <xsl:when test="f:extension[@url = $practitionerrole-reference]">
               <xsl:apply-templates select="f:extension[@url = $practitionerrole-reference]/f:valueReference"
                                    mode="#current"/>
            </xsl:when>
            <xsl:otherwise>
               <xsl:variable name="referenceValue"
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="f:extension[@url = $zib-Dispense-Location]"
                 mode="zib-DispenseRequest-2.2">
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="f:extension[@url = $zib-DispenseRequest-RelatedMedicationAgreement]"
                 mode="zib-DispenseRequest-2.2">
