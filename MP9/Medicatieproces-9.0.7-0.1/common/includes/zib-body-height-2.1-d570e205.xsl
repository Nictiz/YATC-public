...
<!-- == Provenance: YATC-internal/fhir-2-ada/env/zibs2017/payload/zib-body-height-2.1.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:template match="f:supportingInformation"
      ...
      <xsl:apply-templates select="ancestor::f:Bundle/f:entry[f:fullUrl/@value=$reference]/f:resource/f:Observation[f:meta/f:profile/@value='http://nictiz.nl/fhir/StructureDefinition/zib-BodyHeight']"
                           mode="zib-body-height-2.1"/>
   </xsl:template>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   ...
   <xsl:template match="f:valueQuantity"
      ...
      <lengte_waarde value="{f:value/@value}"
                     unit="{f:unit/@value}"/>
   </xsl:template>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
