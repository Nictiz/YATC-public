...
<!-- == Provenance: YATC-internal/fhir-2-ada/env/zibs2017/payload/zib-instructions-for-use-2.0.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:variable name="zib-Medication-RepeatPeriodCyclicalSchedule"
                 select="'http://nictiz.nl/fhir/StructureDefinition/zib-Medication-RepeatPeriodCyclicalSchedule'"/>
   <!-- ================================================================== -->
      ...
      <xsl:choose>
         <xsl:when test="not(preceding-sibling::*[self::f:dosageInstruction or self::f:dosage])">
            <gebruiksinstructie>
               ...
               <xsl:apply-templates select="parent::f:MedicationRequest/f:modifierExtension[@url=$zib-Medication-RepeatPeriodCyclicalSchedule]"
                                    mode="#current"/>
                     ...
                     <doseerinstructie>
                        ...
                        <dosering>
                           ...
                           <xsl:if test="f:doseQuantity|f:doseRange">
                              <keerdosis>
                                 <xsl:apply-templates select="f:doseQuantity"
                                                      mode="#current"/>
                                 <xsl:apply-templates select="f:doseRange"
                                                      mode="#current"/>
                              </keerdosis>
                           </xsl:if>
                           <!-- toedieningsschema -->
                           ...
                           <xsl:if test="f:rateRatio|f:rateRange">
                              <toedieningssnelheid>
                                 ...
                                 <!-- eenheid -->
                                 <!-- tijdseenheid -->
                                 <xsl:apply-templates select="f:rateRatio"
                                                      mode="#current"/>
                                 <xsl:apply-templates select="f:rateRange"
                                                      mode="#current"/>
                              </toedieningssnelheid>
                           </xsl:if>
                           <!-- toedieningsduur -->
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="f:modifierExtension[@url=$zib-Medication-RepeatPeriodCyclicalSchedule]"
                 mode="zib-instructions-for-use-2.0">
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="f:doseQuantity"
                 mode="zib-instructions-for-use-2.0">
   ...
   <xsl:template match="f:rateRange"
      ...
      <xsl:variable name="unit"
                    select="(*/f:unit/@value)[1]"/>
      <xsl:variable name="unit-UCUM"
                    select="substring-before($unit,'/')"/>
      ...
      <eenheid>
         ...
         <xsl:attribute name="codeSystemName"
                        select="local:getDisplayName($oidGStandaardBST902THES2)"/>
      </eenheid>
      <xsl:variable name="ucum-tijdseenheid"
                    select="substring-after($unit, '/')"/>
      ...
      <tijdseenheid value="{$ucum-tijdseenheid-value}"
                    unit="{nf:convertTime_UCUM_FHIR2ADA_unit($ucum-tijdseenheid-unit)}"/>
   </xsl:template>
</xsl:stylesheet>