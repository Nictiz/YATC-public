...
<!-- == Provenance: YATC-internal/fhir-2-ada/env/zibs2017/payload/zib-medicationagreement-2.2.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <!--<xsl:import href="../../fhir/fhir_2_ada_fhir_include.xsl"/>
    ...
    <xsl:import href="ext-zib-medication-additional-information-2.0.xsl"/>-->
   <xsl:variable name="zib-MedicationAgreement"
                 select="'http://nictiz.nl/fhir/StructureDefinition/zib-MedicationAgreement'"/>
   ...
   <xsl:variable name="zib-MedicationAgreement-BasedOnAgreementOrUse"
                 select="'http://nictiz.nl/fhir/StructureDefinition/zib-MedicationAgreement-BasedOnAgreementOrUse'"/>
   ...
   <xsl:template match="f:MedicationRequest"
      ...
      <medicatieafspraak>
         ...
         <xsl:apply-templates select="f:extension[@url = $zib-MedicationAgreement-BasedOnAgreementOrUse]"
                              mode="#current"/>
         <!-- relaties_ketenzorg -->
      ...
      <voorschrijver>
         <xsl:choose>
            <xsl:when test="f:agent/f:extension[@url = $practitionerrole-reference]">
               <xsl:variable name="referenceValue"
                             select="f:agent/f:extension[@url = $practitionerrole-reference]/f:valueReference/f:reference/@value"/>
            ...
            <xsl:otherwise>
               <xsl:variable name="referenceValue"
                             select="f:agent/f:reference/@value"/>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="f:extension[@url = $zib-MedicationAgreement-BasedOnAgreementOrUse]"
                 mode="zib-MedicationAgreement-2.2">
         ...
         <xsl:choose>
            <xsl:when test="f:valueReference/f:reference">
               ...
               <xsl:choose>
                  ...
                  <xsl:when test="$resourceCategoryCode = '6'">
                     ...
                     <xsl:call-template name="Identifier-to-identificatie">
                        <xsl:with-param name="in"
                                        select="$resource/f:MedicationStatement/f:identifier"/>
            ...
            <xsl:when test="f:valueReference/f:identifier">
               <xsl:variable name="identifier_type"
                             select="f:valueReference/f:identifier/f:type/f:coding/f:value/@value"/>
               <xsl:variable name="referenceValue"
                             select="f:valueReference/f:identifier/concat(f:system/@value,f:value/@value)"/>
               ...
               <xsl:choose>
                  ...
                  <xsl:when test="$identifier_type = '16076005'">
                     <xsl:call-template name="Identifier-to-identificatie">
                        <xsl:with-param name="in"
                                        select="f:valueReference/f:identifier"/>
                     </xsl:call-template>
                  </xsl:when>
      ...
      <relaties_ketenzorg>
         ...
         <xsl:choose>
            <xsl:when test="f:reference">
               ...
               <xsl:choose>
                  <xsl:when test="$resource/f:Encounter">
                     <xsl:call-template name="Identifier-to-identificatie">
                        <xsl:with-param name="in"
                                        select="$resource/f:Encounter/f:identifier"/>
                  ...
                  <xsl:when test="$resource/f:EpisodeOfCare">
                     <xsl:call-template name="Identifier-to-identificatie">
                        <xsl:with-param name="in"
                                        select="$resource/f:EpisodeOfCare/f:identifier"/>
            ...
            <xsl:when test="f:identifier">
               ...
               <xsl:variable name="encounter_resource"
                             select="ancestor::f:Bundle/f:entry/f:resource/f:Encounter/f:identifier[concat(f:system/@value,f:value/@value) = $referenceValue]"/>
               <xsl:variable name="episodeofcare_resource"
                             select="ancestor::f:Bundle/f:entry/f:resource/f:EpisodeOfCare/f:identifier[concat(f:system/@value,f:value/@value) = $referenceValue]"/>
               <xsl:choose>
                  <!-- Try to resolve f:identifier within Bundle -->
                  <xsl:when test="$encounter_resource">
                     <xsl:call-template name="Identifier-to-identificatie">
                        <xsl:with-param name="in"
                                        select="$encounter_resource"/>
                        <xsl:with-param name="adaElementName">identificatie_contactmoment</xsl:with-param>
                     </xsl:call-template>
                  </xsl:when>
