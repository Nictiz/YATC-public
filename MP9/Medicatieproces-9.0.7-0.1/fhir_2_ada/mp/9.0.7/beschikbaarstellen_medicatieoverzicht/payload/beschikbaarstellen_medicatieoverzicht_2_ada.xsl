...
<!-- == Provenance: YATC-internal/fhir-2-ada/env/mp/9.0.7/beschikbaarstellen_medicatieoverzicht/payload/beschikbaarstellen_medicatieoverzicht_2_ada.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:variable name="Bundle-MedicationOverview"
                 select="'http://nictiz.nl/fhir/StructureDefinition/Bundle-MedicationOverview'"/>
   <xsl:variable name="MedicationOverview-Verification"
                 select="'http://nictiz.nl/fhir/StructureDefinition/MedicationOverview-Verification'"/>
   <!-- ================================================================== -->
   <xsl:template match="/">
      <!-- Starts transformation for Medication Overview 9.0.7 from ADA to HL7 FHIR. -->
      <adaxml>
         <xsl:attribute name="xsi:noNamespaceSchemaLocation"
                        select="'../ada_schemas/ada_beschikbaarstellen_medicatieoverzicht.xsd'"/>
         <meta status="new"
         ...
         <data>
            <beschikbaarstellen_medicatieoverzicht app="mp-mp907"
               ...
               <xsl:choose>
                  ...
                  <xsl:otherwise>
                     <xsl:variable name="reference"
                                   select="f:Bundle[f:meta/f:profile/@value = $Bundle-MedicationOverview]/f:entry/f:resource/f:List[f:code/f:coding/f:code/@value = '11181000146103']/f:subject/f:reference/@value"/>
                     <xsl:apply-templates select="f:Bundle/f:entry[f:fullUrl/@value = $reference]/f:resource/f:Patient"
               ...
               <xsl:apply-templates select="f:Bundle[f:meta/f:profile/@value = $Bundle-MedicationOverview]/f:entry/f:resource/f:List[f:code/f:coding/f:code/@value = '11181000146103']"
                                    mode="MedicationOverview-2.0"/>
   ...
   <xsl:template match="f:List"
      ...
      <documentgegevens>
         ...
         <xsl:apply-templates select="f:date"
                              mode="#current"/>
         <!-- auteur -->
         <xsl:apply-templates select="f:source"
                              mode="#current"/>
         <!-- verificatie_patient -->
         <!-- verificatie_zorgverlener -->
         <xsl:apply-templates select="f:extension[@url = $MedicationOverview-Verification]"
                              mode="#current"/>
      </documentgegevens>
   </xsl:template>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   ...
   <xsl:template match="f:source"
      ...
      <xsl:variable name="MedicationOverview-SourceOrganization"
                    select="'http://nictiz.nl/fhir/StructureDefinition/MedicationOverview-SourceOrganization'"/>
      <auteur>
         <xsl:choose>
            <xsl:when test="f:extension/@url = $MedicationOverview-SourceOrganization">
               <xsl:variable name="reference"
                             select="f:extension[@url = $MedicationOverview-SourceOrganization]/f:valueReference/f:reference/@value"/>
               <auteur_is_zorgaanbieder>
                  <xsl:apply-templates select="ancestor::f:Bundle/f:entry[f:fullUrl/@value=$reference]/f:resource/f:Organization"
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="f:extension[@url = $MedicationOverview-Verification]"
                 mode="MedicationOverview-2.0">
         ...
         <xsl:if test="f:extension[@url = 'VerificationPatientDate']">
            <verificatie_datum>
               <xsl:attribute name="value">
                  <xsl:call-template name="format2ADADate">
                     <xsl:with-param name="dateTime"
                                     select="f:extension[@url = 'VerificationPatientDate']/f:valueDateTime/f:value/@value"/>
                  </xsl:call-template>
               </xsl:attribute>
            </verificatie_datum>
         </xsl:if>
         ...
         <xsl:if test="f:extension[@url = 'VerificationHealthProfessionalDate']">
            <verificatie_datum>
               <xsl:attribute name="value">
                  <xsl:call-template name="format2ADADate">
                     <xsl:with-param name="dateTime"
                                     select="f:extension[@url = 'VerificationHealthProfessionalDate']/f:valueDateTime/f:value/@value"/>
                  </xsl:call-template>
               </xsl:attribute>
            </verificatie_datum>
         </xsl:if>
