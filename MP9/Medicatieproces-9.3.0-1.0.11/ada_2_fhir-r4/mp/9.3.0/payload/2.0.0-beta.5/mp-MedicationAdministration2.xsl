...
<!-- == Provenance: YATC-internal/ada-2-fhir-r4/env/mp/9.3.0/payload/2.0.0-beta.5/mp-MedicationAdministration2.xsl == -->
      ...
      <xsl:for-each select="$in">
         <MedicationAdministration>
            ...
            <xsl:variable name="dosage">
               ...
               <!-- toedieningssnelheid nominale waarde from MP9 3.0 beta.4 onwards -->
               <xsl:for-each select="toedieningssnelheid[not(tijdseenheid|eenheid)][nominale_waarde[@value | @unit]]">
                  <xsl:call-template name="_buildAdministrationRate"/>
               </xsl:for-each>
               <!-- toedieningssnelheid nominale waarde old style, left here for legacy support -->
               <xsl:for-each select="toedieningssnelheid[*/nominale_waarde]">
                  ...
                  <xsl:choose>
                     ...
                     <xsl:when test="not(tijdseenheid | eenheid) and */nominale_waarde[@unit]">
                        <xsl:call-template name="_buildAdministrationRate">
                           <xsl:with-param name="in"
                                           select="*[nominale_waarde[@unit]]"/>
                        </xsl:call-template>
                     </xsl:when>
                  </xsl:choose>
               </xsl:for-each>
