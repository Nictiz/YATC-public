      ...
      <xsl:for-each select="$in">
         <effectiveTime xmlns="http://hl7.org/fhir">
            <xsl:attribute name="xsi:type"
                           select="'Timing'"/>
            <xsl:choose>
               <!-- NICTIZ-27509 the 'native' fhir f:effectiveTime of f:timing in adaextension gets precedence over the ada elements -->
               <xsl:when test="adaextension/(f:effectiveTime | f:timing)">
                  <xsl:copy-of select="adaextension/(f:effectiveTime | f:timing)/*"
                               copy-namespaces="no"/>
               </xsl:when>
               <xsl:otherwise>
