...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:function name="nf:renderName"
      ...
      <xsl:variable name="renderedName"
                    select="nf:_renderNameFromParts($given, nf:_renderFamilyName($in), $in/roepnaam)"/>
      <!-- support for unstructured name which is a real life possibility especially for health professionales and related persons -->
      <xsl:variable name="unstructured"
                    select="normalize-space($in/ongestructureerde_naam/@value)"
                    as="xs:string?"/>
      <xsl:choose>
         <xsl:when test="string-length(normalize-space($renderedName)) = 0 and string-length($unstructured) gt 0">
            <xsl:value-of select="$unstructured"/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="$renderedName"/>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:function>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
