            ...
            <xsl:choose>
               <xsl:when test="doseerinstructie">
                  <xsl:for-each select="doseerinstructie">
                     ...
                     <xsl:choose>
                        <!-- NICTIZ-33289: <xsl:when test="dosering/*"> to check if dosering exists with content -->
                        <xsl:when test="dosering/*">
                           <xsl:for-each select="dosering">
                              ...
                              <xsl:variable name="zonodigcriterium"
                                            select="(zonodig | zo_nodig)//criterium[not(criterium)]"
                                            as="element(criterium)*"/>
                              <content>
                                 <xsl:for-each select="$zonodigcriterium[position() gt 1]">
                                    <extension url="http://nictiz.nl/fhir/StructureDefinition/ext-Dosage-AsNeededFor">
                                 ...
                                 <!-- MP-2045, FHIR has only one zo nodig element, the additional ones must be conveyed in extension -->
                                 <xsl:for-each select="$zonodigcriterium[position() = 1]">
                        ...
                        <!-- Fallback for when no dosering is defined or dosering is empty but a volgnummer or doseerduur is present -->
