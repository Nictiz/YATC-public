...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <!--
    ...
    -->
   <!-- ================================================================== -->
   <xsl:import href="all_zibs-d614e279.xsl"/>
         ...
         <adaxml xsi:noNamespaceSchemaLocation="../ada_schemas/ada_sturen_laboratoriumresultaten.xsd"
            ...
            <data>
               <sturen_laboratoriumresultaten app="lab"
                  ...
                  <xsl:choose>
                     <xsl:when test="count(distinct-values(f:Bundle/f:entry/f:resource/(f:Observation/f:subject | f:Device/f:patient)/f:reference/@value)) ge 2">
                        ...
                        <!-- we'll output all patients, garbage in ... -->
                        <xsl:apply-templates select="f:Bundle/f:entry/f:resource/f:Patient"
                                             mode="nl-core-Patient"/>
                     </xsl:when>
                     <xsl:otherwise>
                        <patientgegevens>
                           <!-- patient, since there may be more than one due to mp voorschrift in same Bundle, we select the first we encounter in primary lab resources -->
                           <xsl:apply-templates select="nf:resolveRefInBundle((f:Bundle/f:entry/f:resource/f:Observation/f:subject[f:reference/@value])[1])/*"
                                                mode="nl-core-Patient"/>
                        </patientgegevens>
                     </xsl:otherwise>
                  </xsl:choose>
