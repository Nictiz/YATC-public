...
<!-- == Provenance: YATC-internal/fhir-2-ada-r4/env/mp/9.3.0/payload/2.0.0-rc.2/mp-antwoord.xsl == -->
   ...
   <xsl:template match="f:Communication"
      ...
      <antwoord>
         ...
         <xsl:for-each select="f:payload/f:contentString/f:extension[@url=$urlExtCommunicationPayloadContentCodeableConcept]/f:valueCodeableConcept">
            <xsl:element name="{$adaAntwoordElementName}">
               <xsl:call-template name="CodeableConcept-to-code">
                  <xsl:with-param name="adaElementName"
                                  select="concat($adaAntwoordElementName, '_besluit')"/>
               </xsl:call-template>
            </xsl:element>
         </xsl:for-each>
      </antwoord>
