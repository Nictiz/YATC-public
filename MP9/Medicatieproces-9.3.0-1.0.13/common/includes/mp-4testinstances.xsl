...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <!-- ================================================================== -->
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <!-- separate template to make overriding possible for testing purposes, we add gebruiksperiode and gebruiksperiode/criterium because we are converting MP9 to 6.12 -->
   <xsl:template name="gebruiksomschrijving2text612">
      ...
      <xsl:for-each select="$in[@value]">
         <!-- TODO: add gebruiksperiode and gebruiksperiode/criterium in 6.12 text -->
         <xsl:variable name="theGebruiksperiode"
                       select="../../gebruiksperiode[*]"/>
         <xsl:variable name="stringGebruiksperiode"
                       select="nf:periode-string($theGebruiksperiode/start_datum_tijd, $theGebruiksperiode/tijds_duur, $theGebruiksperiode/eind_datum_tijd, $theGebruiksperiode/criterium)"/>
         <text mediaType="text/plain"
               xmlns="urn:hl7-org:v3">
            <xsl:value-of select="normalize-space(concat($stringGebruiksperiode, ', ', @value))"/>
         </text>
      </xsl:for-each>
   </xsl:template>
               ...
               <xsl:otherwise>
                  <xsl:for-each select="dosering">
                     <xsl:variable name="zo-nodig"
                                   as="xs:string*">
                        <xsl:value-of select="zo_nodig/(criterium | criterium/(code | criterium))[not(@originalText)]/@displayName"/>
                        <xsl:value-of select="zo_nodig/(criterium | criterium/(code | criterium))/@originalText"/>
                     </xsl:variable>
