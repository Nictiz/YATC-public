...
<xsl:stylesheet exclude-result-prefixes="#all"
      ...
      <Observation>
         <xsl:call-template name="insertLogicalId">
            <xsl:with-param name="profile"
                            select="nf:get-profilename-from-adaelement(.)"/>
         </xsl:call-template>
         <meta>
   ...
   <xsl:template name="_nl-core-LaboratoryTestResult-individualTest"
      ...
      <xsl:variable name="isUitbesteed"
                    select="$in/is_uitbesteed/@value = 'true'"/>
      <Observation>
         <xsl:call-template name="insertLogicalId">
            <xsl:with-param name="profile"
                            select="nf:get-profilename-from-adaelement(.)"/>
         ...
         <xsl:for-each select="$parent/uitvoerder">
            <xsl:if test="$isUitbesteed">
               <!-- when the test has been outsourced to a different laboratory, a slice for actual performing lab is used.
                    It is currently not expected to know the details about the actual performer, but the slice is maintained with a dataAbsentReason extension and the correct code for extension event-performerFunction-->
               <performer>
                  <extension url="http://hl7.org/fhir/StructureDefinition/event-performerFunction">
                     <valueCodeableConcept>
                        <coding>
         ...
         <xsl:for-each select="$in/point_of_care_test[@value = 'true'] | $in/is_point_of_care_test[@value = 'true']">
            <component>
               <code>
                  <coding>
                     <system value="http://snomed.info/sct"/>
                     <code value="405262001"/>
                     <display value="point-of-care testing (verrichting)"/>
                  </coding>
               </code>
               <valueBoolean value="{@value}"/>
            </component>
      ...
      <xsl:for-each select="$in">
         <Specimen>
            ...
            <xsl:variable name="collection"
               ...
               <xsl:for-each select="$in/verzamelperiode">
                  <collectedPeriod>
               ...
               <xsl:for-each select="$in/verzamelperiode/tijds_duur">
                  <duration>
                     <xsl:call-template name="hoeveelheid-to-Quantity"/>
                  </duration>
               </xsl:for-each>
