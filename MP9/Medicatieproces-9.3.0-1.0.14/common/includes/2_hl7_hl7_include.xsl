...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:template name="format2HL7Date">
      ...
      <xsl:param name="precision"
                 as="xs:string?"
                 select="nf:determine_date_precision_from_ada_datetime($dateTime)">
         <!-- Determines the picture of the datetime format only for a datetime castable input. Millisecond is the default, but may be set to second. When this param is set to empty the precision is taken from the actual precision of the input datetime -->
      </xsl:param>
      ...
      <xsl:variable name="derivedPrecision">
         <xsl:choose>
            <xsl:when test="$processedDateTime castable as xs:dateTime and string-length($precision) = 0">
               <xsl:value-of select="nf:determine_date_precision_from_ada_datetime($processedDateTime)"/>
            </xsl:when>
            <xsl:otherwise>
      ...
      <xsl:variable name="picture"
                    as="xs:string?">
         <xsl:choose>
            <xsl:when test="upper-case($derivedPrecision) = ('SECOND', 'SECONDS', 'SECONDE', 'SECONDES', 'SECONDEN', 'SEC', 'S')">[Y0001][M01][D01][H01][m01][s01][Z0001]</xsl:when>
            <xsl:when test="upper-case($derivedPrecision) = ('MILLISECOND', 'MILLISECONDS', 'MILLISECONDE', 'MILLISECONDES', 'MILLISECONDEN', 'MILLISEC', 'MS', 'MSEC')">[Y0001][M01][D01][H01][m01][s01].[f001][Z0001]</xsl:when>
            <xsl:otherwise>[Y0001][M01][D01][H01][m01][s01].[f001][Z0001]</xsl:otherwise>
         </xsl:choose>
      </xsl:variable>
      <xsl:choose>
         ...
         <xsl:when test="$processedDateTime castable as xs:date">
            <xsl:value-of select="format-date(xs:date($processedDateTime), '[Y0001][M01][D01][Z0001]')"/>
         </xsl:when>
         <!-- input dateTime stops at minutes with timezone -->
         <xsl:when test="matches($processedDateTime, '\d{4}-(0[1-9]|1[012])-(0[1-9]|[12]\d|3[01])T([01]\d|2[0-3])(:(0\d|[1-5]\d))(Z|((\+|-)\d{2}:\d{2}))$')">
            <xsl:value-of select="format-dateTime(xs:dateTime(replace($processedDateTime, '(\d{4}-(0[1-9]|1[012])-(0[1-9]|[12]\d|3[01])T([01]\d|2[0-3])(:(0\d|[1-5]\d)))((Z|((\+|-)\d{2}:\d{2}))$)', '$1:00$7')), '[Y0001][M01][D01][H01][m01][Z0001]')"/>
         </xsl:when>
         <!-- input dateTime stops at minutes without timezone -->
         ...
         <!-- input dateTime stops at hours and has timezone -->
         <xsl:when test="matches($processedDateTime, '\d{4}-(0[1-9]|1[012])-(0[1-9]|[12]\d|3[01])T([01]\d|2[0-3])(Z|((\+|-)\d{2}:\d{2}))$')">
            <xsl:value-of select="format-dateTime(xs:dateTime(replace($processedDateTime, '(\d{4}-(0[1-9]|1[012])-(0[1-9]|[12]\d|3[01])T([01]\d|2[0-3]))((Z|((\+|-)\d{2}:\d{2}))$)', '$1:00:00$5')), '[Y0001][M01][D01][H01][Z0001]')"/>
         </xsl:when>
         ...
         <!-- input date stops at months,but has timezone, we know that is a bit strange -->
         <xsl:when test="matches($processedDateTime, '\d{4}-(0[1-9]|1[012])(Z|((\+|-)\d{2}:\d{2}))$')">
            <xsl:value-of select="format-date(xs:date(replace($processedDateTime, '(\d{4}-(0[1-9]|1[012]))((Z|((\+|-)\d{2}:\d{2}))$)', '$1-01$3')), '[Y0001][M01][Z0001]')"/>
         </xsl:when>
         ...
         <!-- input date stops at year, but has timezone, we know that is a bit strange -->
         <xsl:when test="matches($processedDateTime, '\d{4}(Z|((\+|-)\d{2}:\d{2}))$')">
            <xsl:value-of select="format-date(xs:date(replace($processedDateTime, '(\d{4})((Z|((\+|-)\d{2}:\d{2})))', '$1-01-01$2')), '[Y0001][Z0001]')"/>
         </xsl:when>
...
<xsl:value-of select="$inputDateTime"/>' which could not be converted to HL7 date(time). Input = output.</xsl:with-param>
   ...
   <xsl:template name="makeSCValue">
      ...
      <xsl:param name="elemName"
                 select="'value'"/>
      <xsl:param name="inputValue"
                 select="(@originalText, @displayName, @value)[1]"
                 as="xs:string?"/>
      <xsl:variable name="inputNullFlavor"
                    select="@nullFlavor"
                    as="xs:string?"/>
      <xsl:element name="{$elemName}">
         ...
         <xsl:if test="@code or string-length($inputNullFlavor) gt 0">
            <xsl:call-template name="makeCodeAttribs">
               <!-- datatype SC does not support originalText, if present (unlikely) it will have to be in the text node, see param inputValue -->
               <xsl:with-param name="originalText"/>
               <xsl:with-param name="strOriginalText"/>
            </xsl:call-template>
         </xsl:if>
...
<xsl:value-of select="$theValue"/>'</xsl:with-param>
   ...
   <xsl:template name="makeTSValueAttr"
      ...
      <xsl:param name="precision"
                 as="xs:string?"
                 select="nf:determine_date_precision_from_ada_datetime($inputValue)">
