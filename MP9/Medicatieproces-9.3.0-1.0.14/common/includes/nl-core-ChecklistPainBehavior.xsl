...
<!-- == Provenance: YATC-internal/ada-2-fhir-r4/env/zibs/2020/payload/0.10.0-beta.1/nl-core-ChecklistPainBehavior.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
                ...
                xmlns:local="#local.2024111408212880185670100">
   <!-- ================================================================== -->
   <!--
        Converts ADA checklist_pijn_gedrag to FHIR Observation resource conforming to profile nl-core-ChecklistPainBehavior.
    -->
   <!-- ================================================================== -->
   <!--
        Copyright © Nictiz
   ...
   <xsl:variable name="profileNameChecklistPainBehavior"
                 select="'nl-core-ChecklistPainBehavior'"/>
   <!-- ================================================================== -->
   <xsl:template match="checklist_pijn_gedrag"
                 name="nl-core-ChecklistPainBehavior"
                 mode="nl-core-ChecklistPainBehavior"
                 as="element(f:Observation)">
      <!-- Creates an nl-core-ChecklistPainBehavior instance as an Observation FHIR instance from ADA checklist_pijn_gedrag element. -->
      <xsl:param name="in"
                 as="element()?"
                 select=".">
         ...
         <Observation>
            <xsl:call-template name="insertLogicalId">
               <xsl:with-param name="profile"
                               select="$profileNameChecklistPainBehavior"/>
            </xsl:call-template>
            <meta>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:function name="nf:_maybeWriteComponent"
                 as="element(f:component)?">
      <xsl:param name="code"
                 as="xs:string"/>
