...
<xsl:stylesheet exclude-result-prefixes="#all"
      ...
      <xsl:choose>
         <xsl:when test="$in/f:extension/@url = $urlExtHL7NullFlavor">
            ...
            <xsl:element name="{$adaElementName}">
               ...
               <!-- originalText -->
               <xsl:choose>
                  <!-- let's honour the input param when available -->
                  <xsl:when test="string-length($originalText) gt 0">
                     <xsl:attribute name="originalText"
                                    select="$originalText"/>
                  </xsl:when>
                  <!-- however, we need originalText with nullFlavor -->
                  <xsl:when test="$in/f:text/@value">
                     <xsl:attribute name="originalText"
                                    select="$in/f:text/@value"/>
                  </xsl:when>
               </xsl:choose>
            ...
            <xsl:for-each select="$in/f:*[local-name() = $inElementName]">
               <xsl:element name="{$adaElementName}">
                  ...
                  <xsl:choose>
                     ...
                     <!-- however, we need originalText with nullFlavor -->
                     <xsl:when test="($in/*[local-name() = $inElementName]/f:system/@value = $oidMap[@oid = $oidHL7NullFlavor]/@uri or $in/*[local-name() = $inElementName]/f:system/@value = concat('urn:oid:', $oidHL7NullFlavor)) and $in/f:text/@value">
                        <xsl:attribute name="originalText"
                                       select="$in/f:text/@value"/>
                     </xsl:when>
                  </xsl:choose>
   ...
   <xsl:template name="Identifier-to-identificatie"
      ...
      <xsl:variable name="darCode"
                    select="$in/f:value/f:extension[@url=$urlExtHL7DataAbsentReason]/f:valueCode/@value"
                    as="xs:string?"/>
      <xsl:element name="{$adaElementName}">
         <xsl:choose>
            <xsl:when test="$in/f:extension/@url = $urlExtHL7NullFlavor">
            ...
            <xsl:when test="string-length($darCode) gt 0">
               <xsl:variable name="matchedMap"
                             select="$fhirDataAbsentReason_to_NullFlavor/map[@inCode = $darCode]"/>
               <xsl:choose>
                  <xsl:when test="$matchedMap">
                     <xsl:attribute name="nullFlavor"
                                    select="$matchedMap/@code"/>
                  </xsl:when>
                  <xsl:otherwise>
                     <!-- default to NI = No Information, the most insignificant nullFLavor that exists -->
                     <xsl:attribute name="nullFlavor"
                                    select="'NI'"/>
                  </xsl:otherwise>
               </xsl:choose>
