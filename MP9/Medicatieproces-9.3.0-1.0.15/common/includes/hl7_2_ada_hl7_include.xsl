...
<xd:ref type="parameter"
   ...
   <xsl:function name="nf:formatHL72VagueAdaDate"
      ...
      <xsl:variable name="yyyy"
                    as="xs:string?">
         <xsl:if test="string-length($inputWithoutTimeZone) ge 4 and substring($inputWithoutTimeZone, 1, 4) castable as xs:integer">
...
<xsl:value-of select="$input-hl7-date"/>). Officially this is not allowed, but we'll do our best to convert it anyway.</xsl:with-param>
      ...
      <xsl:choose>
         <xsl:when test="string-length($yyyy) = 0"/>
      </xsl:choose>
      <xsl:choose>
         <xsl:when test="string-length($yyyy) = 0">
            <!-- ouch, if there is not even a year we simply copy input to output -->
...
<xsl:value-of select="@root"/>. This MUST be fixed in the source application before continuing.</xsl:with-param>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.3.10.1_20180601000000"
      ...
      <xsl:element name="{$elmPatient}">
         ...
         <!-- it is unlikely, but should deceased information exist in HL7, let's convert it to avoid data loss -->
         <xsl:call-template name="handleBL">
            <xsl:with-param name="in"
