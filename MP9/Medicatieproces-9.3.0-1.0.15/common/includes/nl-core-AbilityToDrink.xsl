...
<!-- == Provenance: YATC-internal/ada-2-fhir-r4/env/zibs/2020/payload/0.11.0-beta.1/nl-core-AbilityToDrink.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
                ...
                xmlns:local="#local.2024111408212917697460100">
   <!-- ================================================================== -->
   <!--
   ...
   <xsl:variable name="profileNameAbilityToDrink"
                 select="'nl-core-AbilityToDrink'"/>
   <xsl:variable name="profileNameAbilityToDrinkDrinkingLimitations"
                 select="'nl-core-AbilityToDrink.DrinkingLimitations'"/>
   <!-- ================================================================== -->
               ...
               <hasMember>
                  <xsl:call-template name="makeReference">
                     <xsl:with-param name="in"
                                     select="."/>
                     <xsl:with-param name="profile"
                                     select="$profileNameAbilityToDrinkDrinkingLimitations"/>
                  </xsl:call-template>
               </hasMember>
            </xsl:for-each>
         </Observation>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="drink_beperkingen[parent::vermogen_tot_drinken]"
                 name="nl-core-AbilityToDrink.DrinkingLimitations"
                 mode="nl-core-AbilityToDrink.DrinkingLimitations"
                 as="element(f:Observation)?">
      <!-- Creates an nl-core-AbilityToDrink.DrinkingLimitations instance as an Observation FHIR instance from ADA drink_beperkingen element. -->
      <xsl:param name="in"
                 select="."
                 as="element()?">
         <!-- ADA element as input. Defaults to self. -->
