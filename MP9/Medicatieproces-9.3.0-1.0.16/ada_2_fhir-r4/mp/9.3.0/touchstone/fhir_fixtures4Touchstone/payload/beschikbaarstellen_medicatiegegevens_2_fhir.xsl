...
<xsl:stylesheet exclude-result-prefixes="#all"
                ...
                xmlns:local="#local.2024102408112286602540200"
   ...
   <xsl:template name="Medicatiegegevens_90">
      ...
      <!-- Aanpassingen inzake NICTIZ-36431 -->
      <xsl:result-document href="{$outputDir}/{$resultBundle/f:Bundle/f:id/@value}.xml">
         <xsl:apply-templates select="$resultBundle/node()"
                              mode="addBasedOnDisplay"/>
      </xsl:result-document>
   </xsl:template>
   <!-- ========================================================= -->
   <!-- Overlay: voeg display direct onder basedOn toe            -->
   <!-- voor RelationAdministrationAgreement extensies            -->
   <!-- ========================================================= -->
   ...
   <xsl:template match="f:basedOn[         f:extension[@url='http://nictiz.nl/fhir/StructureDefinition/ext-RelationAdministrationAgreement']         and not(f:display)         ]"
                 mode="addBasedOnDisplay">
      <!-- extract system en identifier -->
      <xsl:variable name="system"
                    select="f:extension[@url='http://nictiz.nl/fhir/StructureDefinition/ext-RelationAdministrationAgreement']             /f:valueReference/f:identifier/f:system/@value"/>
      <xsl:variable name="id"
                    select="f:extension[@url='http://nictiz.nl/fhir/StructureDefinition/ext-RelationAdministrationAgreement']             /f:valueReference/f:identifier/f:value/@value"/>
      <xsl:variable name="taDisplay"
                    select="concat('relatie naar toedieningsafspraak met identificatie: ', $id, ' || ', $system)"/>
      <xsl:copy>
         <xsl:apply-templates select="@*|node()"
                              mode="addBasedOnDisplay"/>
         <display value="{$taDisplay}"/>
      </xsl:copy>
   </xsl:template>
</xsl:stylesheet>