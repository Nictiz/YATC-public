            ...
            <xsl:for-each select="./controlact_wrapper">
               <ControlActProcess classCode="CACT"
                  ...
                  <xsl:variable name="mve"
                                select="($payload_xml//beschikbaarstellen_medicatiegegevens/medicamenteuze_behandeling/medicatieverstrekking[verstrekker/zorgaanbieder])[1]"
                                as="element()?"/>
                  <xsl:variable name="mveAuteur"
                                select="$mve/../../bouwstenen/zorgaanbieder[@id = $mve/verstrekker/zorgaanbieder/@value]"
                     ...
                     <xsl:when test="$mveAuteur">
                        <xsl:for-each select="$mveAuteur">
                           <authorOrPerformer typeCode="AUT">
                              <participant>
                                 <AssignedDevice>
                                    <xsl:for-each select="$author/assigned_device/id">
                                       <id extension="{./@value}"
                                           root="{@root}"/>
                                    </xsl:for-each>
