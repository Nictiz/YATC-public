...
<xsl:stylesheet version="2.0"
   ...
   <xsl:template name="generateLogicalId"
      ...
      <xsl:variable name="logicalIdStartString"
         ...
         <xsl:choose>
            ...
            <xsl:when test="$profile = 'lu-LaboratoryTestResult-DiagnosticReport'">
               <xsl:value-of select="replace($profile, 'LaboratoryTestResult', 'LTR')"/>
            </xsl:when>
            <xsl:when test="self::monster | self::laboratorium_test">
               <xsl:value-of select="replace($profile, 'LaboratoryTestResult', 'LTR')"/>
            </xsl:when>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="aanvraaggegevens"
                 mode="_generateId">
      <xsl:param name="profile"
                 as="xs:string?"/>
      <xsl:variable name="uniqueString">
         <xsl:choose>
            <xsl:when test="aanvraagnummer/@value">
               <xsl:value-of select="replace(aanvraagnummer/@value, '_', '')"/>
            </xsl:when>
            <xsl:otherwise>
   ...
   <xsl:template match="laboratorium_test"
                 mode="_generateId">
      ...
      <xsl:variable name="uniqueString"
                    as="xs:string?">
         <xsl:choose>
            <xsl:when test="test_identificatie[@value]">
               <xsl:for-each select="(test_identificatie[@value])[1]">
   ...
   <xsl:template match="monster"
      ...
      <xsl:variable name="uniqueString"
                    as="xs:string?">
         <xsl:choose>
            <xsl:when test="monsternummer[@value]">
               <xsl:for-each select="(monsternummer[@value])[1]">
                  <!-- we remove '.' in root oid and '_' in extension to enlarge the chance of staying in 64 chars -->
                  <xsl:value-of select="replace(@value, '_', '')"/>
               </xsl:for-each>
            </xsl:when>
