...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:import href="ada2hl7_all-zibs-d622e312.xsl"/>
...
<xsl:value-of select="$herhaalperiode/@value"/>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9026_20150318000000">
      ...
      <MedicationDispenseList>
         ...
         <xsl:for-each select="$tamveSeparatedByProduct/data/beschikbaarstellen_medicatiegegevens/medicamenteuze_behandeling[count(toedieningsafspraak) != 1 or count(medicatieverstrekking) != 1]">
            <xsl:call-template name="util:logMessage">
               <xsl:with-param name="level"
                               select="$logERROR"/>
               <xsl:with-param name="msg"
                               select="concat('Converting ada to 6.12 verstrekking. Encountered an MBH which does not have 1 TA and 1 MVE for the same product, we will skip those (found ', count(toedieningsafspraak), ' TA(''s) and ', count(medicatieverstrekking), ' MVE(''s)). This is not supported. MBH with id @extension: ', identificatie/@value, ' (oid: ', identificatie/@root, '). Product/@id in ada = ', farmaceutisch_product/@id, '.')"/>
            </xsl:call-template>
         </xsl:for-each>
         <xsl:for-each select="$tamveSeparatedByProduct/data/beschikbaarstellen_medicatiegegevens/medicamenteuze_behandeling[count(toedieningsafspraak) = 1][count(medicatieverstrekking) = 1]">
            <component>
