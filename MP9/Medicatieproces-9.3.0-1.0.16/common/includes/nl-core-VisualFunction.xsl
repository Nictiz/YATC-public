...
<!-- == Provenance: YATC-internal/ada-2-fhir-r4/env/zibs/2020/payload/0.11.0-beta.1/nl-core-VisualFunction.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
                ...
                xmlns:local="#local.2024111408212975372580100">
   <!-- ================================================================== -->
   <!--
        Converts ADA functie_zien to FHIR Observation resource conforming to profile nl-core-VisualFunction, FHIR DeviceUseStatement resource conforming to profile nl-core-VisualFunction.VisualAid and FHIR Device resource conforming to profile nl-core-VisualFunction.VisualAid.Product.
    -->
   <!-- ================================================================== -->
   <!--
        Copyright © Nictiz
   ...
   <xsl:variable name="profileNameVisualFunction"
                 select="'nl-core-VisualFunction'"/>
   <xsl:variable name="profileNameVisualFunctionVisualAid"
                 select="'nl-core-VisualFunction.VisualAid'"/>
   <xsl:variable name="profileNameVisualFunctionVisualAidProduct"
   ...
   <xsl:template match="functie_zien"
      ...
      <!-- Creates an nl-core-VisualFunction instance as an Observation FHIR instance from ADA functie_zien element. -->
      <xsl:param name="in"
                 select="."
                 as="element()?">
   ...
   <xsl:template match="zien_hulpmiddel/medisch_hulpmiddel"
      ...
      <!-- Creates an nl-core-VisualFunction.VisualAid instance as a DeviceUseStatement FHIR instance from ADA zien_hulpmiddel/medisch_hulpmiddel element. -->
      <xsl:param name="in"
                 select="."
                 as="element()?">
         <!-- ADA element as input. Defaults to self. -->
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="zien_hulpmiddel/medisch_hulpmiddel[product or product_omschrijving]"
                 name="nl-core-VisualFunction.VisualAid.Product"
                 mode="nl-core-VisualFunction.VisualAid.Product"
                 as="element(f:Device)?">
      <!-- Creates an nl-core-VisualFunction.VisualAid.Product instance as a Device FHIR instance from ADA product element. -->
      <xsl:param name="in"
                 select="."
                 as="element()?">
         <!-- ADA element as input. Defaults to self. -->
