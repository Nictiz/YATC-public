...
<xsl:stylesheet version="2.0"
   ...
   <!-- AWE, to be able to use stable uuids also for hl7_2_ada, add text() nodes to allow for non coded MedicationKind in HL7v3 -->
   <xsl:function name="nf:getGroupingKeyDefault"
      ...
      <xsl:param name="in"
                 as="element()?"/>
      <xsl:if test="$in">
         <xsl:value-of select="upper-case(string-join(($in//@value, $in//@root, $in//@unit, $in//@code[not(../@codeSystem = $oidHL7NullFlavor)], $in//@codeSystem[not(. = $oidHL7NullFlavor)], $in//*[@codeSystem = $oidHL7NullFlavor][@code = 'OTH']/@originalText, $in//text())/normalize-space(), ''))"/>
      </xsl:if>
   </xsl:function>
</xsl:stylesheet>