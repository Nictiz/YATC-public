...
<xsl:stylesheet exclude-result-prefixes="#all"
                ...
                xmlns:local="#local.2024102208231512996790200"
...
-->
   ...
   <xsl:template name="mp-MedicationAdministration2"
      ...
      <xsl:param name="performer"
                 select="toediener/*[self::patient or self::zorgverlener or self::zorgaanbieder or self::mantelzorger]/*"
                 as="element()?">
      ...
      <xsl:for-each select="$in">
         <MedicationAdministration>
            ...
            <xsl:for-each select="(../identificatie[@value | @root | @nullFlavor] | medicamenteuze_behandeling_id[@value | @root | @nullFlavor])">
                        ...
                        <xsl:attribute name="value">
                           <xsl:call-template name="code-to-code">
                              <xsl:with-param name="codeMap"
                                              as="element()*">
                                 <map inCodeSystem="{$oidHL7ActStatus}"
                                      inCode="active"
                                      code="in-progress"/>
                                 <map inCodeSystem="{$oidHL7ActStatus}"
                                      inCode="suspended"
            ...
            <xsl:if test="relatie_zorgepisode/(identificatie | identificatienummer)[@value | @root | @nullFlavor] and not(relatie_contact/(identificatie | identificatienummer)[@value | @root | @nullFlavor])">
               <context>
                  ...
                  <xsl:apply-templates select="relatie_zorgepisode[(identificatie | identificatienummer)[@value | @root | @nullFlavor]][position() = 1]/(identificatie | identificatienummer)"
            ...
            <xsl:choose>
               ...
               <xsl:when test="relatie_toedieningsafspraak/identificatie[@value | @root | @nullFlavor]">
                  <supportingInformation>
                     ...
                     <identifier>
                        <xsl:call-template name="id-to-Identifier">
                           <xsl:with-param name="in"
                                           select="relatie_toedieningsafspraak/identificatie[@value | @root | @nullFlavor]"/>
                        </xsl:call-template>
                     </identifier>
               ...
               <xsl:otherwise>
                  <xsl:if test="not(empty($administrationAgreement))">
                     <xsl:call-template name="makeReference">
                        <xsl:with-param name="in"
                                        select="$administrationAgreement"/>
               ...
               <xsl:when test="toedienings_datum_tijd[@value]">
                  <effectiveDateTime>
                     <xsl:attribute name="value">
                        <xsl:call-template name="format2FHIRDate">
                           <xsl:with-param name="dateTime"
                                           select="(toedienings_datum_tijd[@value])[1]/@value"/>
                        </xsl:call-template>
                     </xsl:attribute>
                  </effectiveDateTime>
               </xsl:when>
                        ...
                        <xsl:when test="self::zorgverlener">
                           <xsl:call-template name="makeReference">
                              <xsl:with-param name="profile"
                                              select="$ada2resourceType/nm:map[@ada = current()/local-name()][@resource = 'PractitionerRole']/@profile"/>
                           </xsl:call-template>
                        </xsl:when>
            ...
            <!-- MP-1606 from MP9.3.0 beta.4 onwards relatie_wisselend_doseerschema is no longer mapped to request but to supportingInformation -->
            <xsl:choose>
            ...
            <xsl:variable name="dosage">
               ...
               <!-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->
               <!-- toedieningssnelheid , in MTD only nominale waarde -->
               <!-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->
               ...
               <xsl:for-each select="toedieningssnelheid[*/nominale_waarde]">
                  ...
                  <xsl:choose>
                     ...
                     <!-- without tijdseenheid (starting from version MP9.3 beta.3) -->
