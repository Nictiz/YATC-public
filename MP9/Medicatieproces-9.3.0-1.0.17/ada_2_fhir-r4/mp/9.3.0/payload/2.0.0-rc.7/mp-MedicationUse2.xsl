...
<!-- == Provenance: YATC-internal/ada-2-fhir-r4/env/mp/9.3.0/payload/2.0.0-rc.7/mp-MedicationUse2.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
                ...
                xmlns:local="#local.202410220823150440870200"
                xmlns:nm="http://www.nictiz.nl/mappings">
   <!-- ================================================================== -->
   <!--
        Converts ADA medicatie_gebruik to FHIR MedicationStatement conforming to profile mp-MedicationUse2
    -->
   <!-- ================================================================== -->
   <!--
        Copyright © Nictiz
   ...
   <xsl:variable name="mgbCode930"
                 select="$mgbCode[1]"/>
   <!-- ================================================================== -->
   <xsl:template name="mp-MedicationUse2"
                 mode="mp-MedicationUse2"
                 match="medicatie_gebruik | medicatiegebruik"
                 as="element(f:MedicationStatement)?">
      <!-- Create an mp-MedicationUse2 instance as a MedicationStatement FHIR instance from ADA medicatie_gebruik. -->
      <xsl:param name="in"
                 as="element()?"
                 select=".">
      ...
      <xsl:param name="subject"
                 select="patient/*"
                 as="element()?">
         <!-- The MedicationStatement.subject as ADA element or reference. -->
      </xsl:param>
      <xsl:param name="medicationReference"
                 select="gebruiksproduct/farmaceutisch_product"
                 as="element()?">
         <!-- The MedicationStatement.medicationReference as ADA element or reference. -->
      </xsl:param>
      <xsl:param name="prescriber"
                 select="voorschrijver/zorgverlener"
                 as="element()?">
         <!-- The MedicationStatement.prescriber as ADA element or reference. -->
      </xsl:param>
      <xsl:for-each select="$in">
         <MedicationStatement>
            ...
            <!-- MP-2104 added medicamenteuze_behandeling_id and kept identifcatie for backwards compatibility-->
            <xsl:for-each select="(../identificatie | medicamenteuze_behandeling_id[@value | @root | @nullFlavor])">
                  ...
                  <valueReference>
                     <xsl:choose>
                        ...
                        <xsl:when test="ancestor::data/*//zorgverlener[@id = current()/auteur_is_zorgverlener/zorgverlener/@value]">
                           <xsl:call-template name="makeReference">
                              <xsl:with-param name="in"
                                              select="ancestor::data/*//zorgverlener[@id = current()/auteur_is_zorgverlener/zorgverlener/@value]"/>
                              <xsl:with-param name="profile"
                                              select="$profileNameHealthProfessionalPractitionerRole"/>
                           </xsl:call-template>
                        </xsl:when>
                        <xsl:when test="ancestor::data/*//zorgaanbieder[@id = current()/auteur_is_zorgaanbieder/zorgaanbieder/@value]">
                           <xsl:call-template name="makeReference">
                              <xsl:with-param name="in"
                                              select="ancestor::data/*//zorgaanbieder[@id = current()/auteur_is_zorgaanbieder/zorgaanbieder/@value]"/>
                              <xsl:with-param name="profile"
               ...
               <context>
                  <xsl:if test="count(relatie_zorgepisode[(identificatie | identificatienummer)[@value]]) gt 1">
                     <xsl:for-each select="relatie_zorgepisode[(identificatie | identificatienummer)[@value]][position() gt 1]">
                        <xsl:call-template name="ext-Context-EpisodeOfCare">
                           <xsl:with-param name="in"
                                           select="identificatie | identificatienummer"/>
                        </xsl:call-template>
                     </xsl:for-each>
                  </xsl:if>
                  <xsl:apply-templates select="relatie_zorgepisode[(identificatie | identificatienummer)[@value]][position() = 1]/(identificatie | identificatienummer)"
               ...
               <xsl:choose>
                  <xsl:when test="ancestor::data/*//contactpersoon[@id = current()/self::persoon/contactpersoon/@value]">
                     <informationSource>
                        <xsl:call-template name="makeReference">
                           <xsl:with-param name="in"
                                           select="ancestor::data/*//contactpersoon[@id = current()/self::persoon/contactpersoon/@value]"/>
                        </xsl:call-template>
                     </informationSource>
                  </xsl:when>
                  ...
                  <xsl:when test="ancestor::data/*//zorgverlener[@id = current()/self::informant_is_zorgverlener/zorgverlener/@value]">
                     <informationSource>
                        <xsl:call-template name="makeReference">
                           <xsl:with-param name="in"
                                           select="ancestor::data/*//zorgverlener[@id = current()/self::informant_is_zorgverlener/zorgverlener/@value]"/>
                           <xsl:with-param name="profile"
                                           select="$profileNameHealthProfessionalPractitionerRole"/>
                        </xsl:call-template>
                     </informationSource>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="medicatie_gebruik | medicatiegebruik"
                 mode="_generateId">
