   ...
   <xsl:template name="Voorschrift_9x">
      ...
      <xsl:for-each select="$in">
         ...
         <xsl:if test="$schematronRef">
            <xsl:processing-instruction name="xml-model">phase="#ALL" href="../../../../../../../../SVN/AORTA/branches/Onderhoud_Mp_v90/Publicaties/20241118/mp-xml-20241118T151205/schematron_closed_warnings/mp-mp93_vos.sch" type="application/xml" schematypens="http://purl.oclc.org/dsdl/schematron" phase="#ALL"</xsl:processing-instruction>
            <xsl:comment>Generated from ada instance with title: "
<xsl:value-of select="@title"/>" and id: "
<xsl:value-of select="@id"/>".</xsl:comment>
         </xsl:if>
         <organizer classCode="CLUSTER"
            ...
            <!-- Do not sort on Medicamenteuze behandeling anymore, makes comparison with results of new dataset structure with moved mbhid harder -->
            <!-- Medicatieafspraak -->
            <xsl:for-each select="$mbh/medicatieafspraak[not(kopie_indicator/@value = 'true')]">
               <component typeCode="COMP">
                  <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9430_20221122132432"/>
               </component>
            </xsl:for-each>
            <!-- Medicatieafspraak -->
            <xsl:for-each select="$mbh/medicatieafspraak[kopie_indicator/@value = 'true']">
               <component typeCode="COMP">
