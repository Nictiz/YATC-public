   ...
   <xsl:template match="/">
      ...
      <xsl:result-document href="{$outputDir}/bsma-example-930-1.xml">
         ...
         <xsl:variable name="tx"
                       select="$inputAdaFiles/adaxml/data/*[not(self::sturen_voorstel_medicatieafspraak) and (medicamenteuze_behandeling[medicatieafspraak] or medicatieafspraak)]"/>
         <xsl:variable name="mbh"
                       select="$tx/medicamenteuze_behandeling[medicatieafspraak] | $tx[medicatieafspraak]"/>
      ...
      <xsl:result-document href="{$outputDir}/bswds-example-930-1.xml">
         <!-- Nieuw MBHid backward compatible -->
         <xsl:variable name="tx"
                       select="$inputAdaFiles/adaxml/data/*[medicamenteuze_behandeling[wisselend_doseerschema] or wisselend_doseerschema]"/>
         <xsl:variable name="mbh"
                       select="$tx/medicamenteuze_behandeling[wisselend_doseerschema] | $tx[wisselend_doseerschema]"/>
         <xsl:variable name="patient"
                       select="($tx/patient)[1]"/>
         <xsl:if test="$schematronRef">
      ...
      <xsl:result-document href="{$outputDir}/bsvv-example-930-1.xml">
         <!-- Nieuw MBHid backward compatible -->
         <xsl:variable name="tx"
                       select="$inputAdaFiles/adaxml/data/*[not(self::sturen_voorstel_verstrekkingsverzoek) and (medicamenteuze_behandeling[verstrekkingsverzoek] or verstrekkingsverzoek)]"/>
         <xsl:variable name="mbh"
                       select="$tx/medicamenteuze_behandeling[verstrekkingsverzoek] | $tx[verstrekkingsverzoek]"/>
         <xsl:variable name="patient"
                       select="($tx/patient)[1]"/>
         <xsl:if test="$schematronRef">
      ...
      <xsl:result-document href="{$outputDir}/bsta-example-930-1.xml">
         <!-- Nieuw MBHid backward compatible -->
         <xsl:variable name="tx"
                       select="$inputAdaFiles/adaxml/data/*[medicamenteuze_behandeling[toedieningsafspraak] or toedieningsafspraak]"/>
         <xsl:variable name="mbh"
                       select="$tx/medicamenteuze_behandeling[toedieningsafspraak] | $tx[toedieningsafspraak]"/>
         <xsl:variable name="patient"
                       select="($tx/patient)[1]"/>
         <xsl:if test="$schematronRef">
      ...
      <xsl:result-document href="{$outputDir}/bsmve-example-930-1.xml">
         <!-- Nieuw MBHid backward compatible -->
         <xsl:variable name="tx"
                       select="$inputAdaFiles/adaxml/data/*[medicamenteuze_behandeling[medicatieverstrekking] or medicatieverstrekking]"/>
         <xsl:variable name="mbh"
                       select="$tx/medicamenteuze_behandeling[medicatieverstrekking] | $tx[medicatieverstrekking]"/>
         <xsl:variable name="patient"
                       select="($tx/patient)[1]"/>
         <xsl:if test="$schematronRef">
      ...
      <xsl:result-document href="{$outputDir}/bsmgb-example-930-1.xml">
         <!-- Nieuw MBHid backward compatible -->
         <xsl:variable name="tx"
                       select="$inputAdaFiles/adaxml/data/*[medicamenteuze_behandeling[medicatiegebruik or medicatie_gebruik] or medicatiegebruik or medicatie_gebruik]"/>
         <xsl:variable name="mbh"
                       select="$tx/medicamenteuze_behandeling[medicatiegebruik or medicatie_gebruik] | $tx[medicatiegebruik or medicatie_gebruik]"/>
         <xsl:variable name="patient"
                       select="($tx/patient)[1]"/>
         <xsl:if test="$schematronRef">
      ...
      <xsl:result-document href="{$outputDir}/bsmtd-example-930-1.xml">
         <!-- Nieuw MBHid backward compatible -->
         <xsl:variable name="tx"
                       select="$inputAdaFiles/adaxml/data/*[medicamenteuze_behandeling[medicatietoediening] or medicatietoediening]"/>
         <xsl:variable name="mbh"
                       select="$tx/medicamenteuze_behandeling[medicatietoediening] | $tx[medicatietoediening]"/>
         <xsl:variable name="patient"
                       select="($tx/patient)[1]"/>
         <xsl:if test="$schematronRef">
