...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:template name="makeMBHwithTamveSeparatedByProduct">
      <xsl:param name="mbh"
                 as="element()*"
                 select=".">
         <!-- for a true MP9 3.x message we cannot always properly convert to 6.12 structure, because it is difficult to link MVE to TA -->
         <!--The MBH with TA's and MVE's. Must be in context of an ada structure with data as parent and bouwstenen as sibling. They will be put in an ada structure for MP9 3 and separated per ada product id-->
         <!-- MP-2104 there is no longer an medicamenteuze_behandeling element, so for that dataset structure separate handling: group by mbh-id ín the bouwstenen -->
      </xsl:param>
      <xsl:choose>
         <xsl:when test="$mbh[self::medicamenteuze_behandeling]">
            <!-- legacy dataset structure -->
            <xsl:for-each select="$mbh">
               <xsl:variable name="currentMbh"
                             select="."/>
               <!-- separate ta and mve bouwstenen by product -->
               <adaxml>
                  <data>
                     <beschikbaarstellen_medicatiegegevens>
                        <xsl:for-each select="../bouwstenen/farmaceutisch_product[@id = $currentMbh/(toedieningsafspraak | medicatieverstrekking)/*/farmaceutisch_product/@value]">
                           <!-- start with mbh -->
                           <medicamenteuze_behandeling>
                              <xsl:copy-of select="$currentMbh/identificatie"/>
                              <!-- copy the product we are grouping on -->
                              <xsl:copy>
                                 <xsl:copy-of select="@id"/>
                              </xsl:copy>
                              <xsl:copy-of select="$currentMbh/(toedieningsafspraak | medicatieverstrekking)[.//farmaceutisch_product[@value = current()/@id]]"/>
                           </medicamenteuze_behandeling>
                        </xsl:for-each>
                        <!-- add the bouwstenen -->
         ...
         <xsl:otherwise>
            <!-- MP-2104, new dataset structure -->
            <xsl:for-each-group select="$mbh/(toedieningsafspraak | medicatieverstrekking)"
                                group-by="medicamenteuze_behandeling_id/concat(@value, '-', @root)">
               <xsl:variable name="currentMbh"
               ...
               <adaxml>
                  <data>
                     <beschikbaarstellen_medicatiegegevens>
                        <xsl:for-each-group select="$currentMbh/*/farmaceutisch_product"
                                            group-by="@value">
                           <!-- start with mbh -->
                           <medicamenteuze_behandeling>
                              <xsl:copy-of select="$currentMbh[1]/medicamenteuze_behandeling_id"/>
                              <!-- copy the product we are grouping on -->
                              <xsl:copy>
         ...
         <xsl:otherwise>
            <xsl:call-template name="util:logMessage">
               ...
               <xsl:with-param name="msg"
                               select="concat('onverwachte tijdseenheid, kan niet omrekenen naar dagen: ', $inputDuur, ' ', $eenheid_UCUM)"/>
            </xsl:call-template>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:function>
