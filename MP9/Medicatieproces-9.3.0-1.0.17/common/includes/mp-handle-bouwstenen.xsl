...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:output method="xml"
               indent="yes"/>
   <!-- ================================================================== -->
   <xsl:variable name="adaMPBouwsteenNames"
                 as="xs:string*"
                 select="('medicatieafspraak', 'wisselend_doseerschema', 'verstrekkingsverzoek', 'toedieningsafspraak', 'medicatieverstrekking', 'medicatie_verstrekking', 'medicatiegebruik', 'medicatie_gebruik', 'medicatietoediening')"/>
   <!-- ================================================================== -->
   <xsl:template match="medicamenteuze_behandeling[not(following-sibling::medicamenteuze_behandeling)][not(@datatype = 'reference')] | data/*/*[local-name() = $adaMPBouwsteenNames][not(following-sibling::*[local-name() = $adaMPBouwsteenNames])][not(@datatype = 'reference')]"
      ...
      <!-- Bouwstenen are directly after the last medicamenteuze_behandeling and in the datasetstructure after MP-2104 after the last MP bouwsteen -->
      <xsl:copy>
      ...
      <bouwstenen>
         ...
         <xsl:variable name="concactpersonen"
                       select="..//contactpersoon[not(parent::bouwstenen)]"/>
         ...
         <xsl:variable name="farmaceutischeProducten"
                       select="..//farmaceutisch_product[not(parent::bouwstenen)]"/>
         ...
         <xsl:variable name="zorgverleners"
                       select="..//zorgverlener[not(zorgverlener)][not(parent::bouwstenen)]"/>
         ...
         <xsl:variable name="zorgaanbieders"
                       select="..//zorgaanbieder[not(zorgaanbieder)][not(parent::bouwstenen)]"/>
...
-->
   ...
   <xsl:template match="(farmaceutisch_product | contactpersoon | zorgaanbieder[not(zorgaanbieder)] | zorgaanbieder[not(zorgaanbieder)] | zorgverlener[not(zorgverlener)] | voorstel_gegevens/medicamenteuze_behandeling | voorstel_gegevens/medicatieafspraak | voorstel_gegevens/verstrekkingsverzoek)[not(parent::bouwstenen)]"
      ...
      <xsl:copy>
         <xsl:apply-templates select="@conceptId"
                              mode="#current"/>
         <xsl:attribute name="value">
            <xsl:value-of select="(ancestor::data/*/bouwstenen/*[key/text() = nf:getGroupingKeyDefaulthl72ada(current())]/@id)[1]"/>
         </xsl:attribute>
         <xsl:attribute name="datatype">reference</xsl:attribute>
      </xsl:copy>
   </xsl:template>
   ...
   <xsl:template match="node() | @*"
                 mode="handleBouwstenen deduplicateBouwstenenStep1 deduplicateBouwstenenStep2 addBouwstenen">
