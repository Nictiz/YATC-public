...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:template name="uni-Voorstel">
      ...
      <xsl:param name="outputNewMbhModel"
                 as="xs:boolean"
                 select="true()"/>
      <xsl:param name="mbhHl7Id"
                 as="element(hl7:id)?"/>
      <xsl:param name="maAdaId"
                 as="xs:string?"
                 select="()"/>
      <xsl:param name="vvAdaId"
                 as="xs:string?"
                 select="()"/>
      <voorstel>
         ...
         <xsl:if test="$maAdaId">
            <medicatieafspraak datatype="reference"
                               value="{$maAdaId}"/>
         </xsl:if>
         <xsl:if test="$vvAdaId">
            <verstrekkingsverzoek datatype="reference"
                                  value="{$vvAdaId}"/>
         </xsl:if>
         <xsl:if test="not($outputNewMbhModel) and $mbhAdaId">
   ...
   <xsl:template name="uni-Antwoord">
      ...
      <antwoord>
         ...
         <!-- MP-949, MP-1020, MP-2033. Voorstelgegevens antwoord. Allow for new dataset structure and add support for toelichting. -->
         <xsl:for-each select="$inComponent[hl7:code[@code = '9'][@codeSystem = '2.16.840.1.113883.2.4.3.11.60.20.77.5.3']]/hl7:entryRelationship/hl7:observation[hl7:code[@code = '17'][@codeSystem = '2.16.840.1.113883.2.4.3.11.60.20.77.5.2']]">
            <antwoord_verstrekkingsverzoek>
               <xsl:call-template name="handleCV">
                  <xsl:with-param name="in"
                                  select="hl7:value"/>
                  <xsl:with-param name="elemName">antwoord_verstrekkingsverzoek_besluit</xsl:with-param>
               </xsl:call-template>
               <xsl:call-template name="uni-toelichting">
                  <xsl:with-param name="in"
                                  select="."/>
                  <xsl:with-param name="adaElemName"
                                  select="'antwoord_verstrekkingsverzoek_toelichting'"/>
               </xsl:call-template>
            </antwoord_verstrekkingsverzoek>
         </xsl:for-each>
            ...
            <antwoord_medicatieafspraak>
               <xsl:call-template name="handleCV">
                  ...
                  <xsl:with-param name="elemName"
                                  select="'antwoord_medicatieafspraak_besluit'"/>
               </xsl:call-template>
               <xsl:call-template name="uni-toelichting">
                  <xsl:with-param name="in"
                                  select="."/>
                  <xsl:with-param name="adaElemName"
                                  select="'antwoord_medicatieafspraak_toelichting'"/>
               </xsl:call-template>
            </antwoord_medicatieafspraak>
         </xsl:for-each>
      </antwoord>
