...
<!-- == Provenance: YATC-internal/fhir-2-ada-r4/env/mp/9.3.0/beschikbaarstellen_medicatiegegevens/payload/beschikbaarstellen_medicatiegegevens_2_ada.xsl == -->
   ...
   <xsl:template match="/">
      ...
      <xsl:variable name="bouwstenen"
         ...
         <xsl:for-each select="f:Bundle/f:entry/f:resource/(f:MedicationUse | f:MedicationStatement)/f:extension[@url = $urlExtMedicationUseAuthor]/f:valueReference">
            <xsl:apply-templates select="nf:resolveRefInBundle(.)/f:Location"
                                 mode="nl-core-HealthcareProvider-Organization"/>
         </xsl:for-each>
      </xsl:variable>
      <adaxml xsi:noNamespaceSchemaLocation="../ada_schemas/ada_beschikbaarstellen_medicatiegegevens.xsd"
         ...
         <data>
            <beschikbaarstellen_medicatiegegevens app="mp-mp93"
               ...
               <!-- MP-2104 -->
               <!-- before MP9 3.0.0 rc.2 there was an medicamenteuze_behandeling element in ada -->
               ...
               <!-- MP-2104 vanaf MP9 3.0.0-rc.2 is de medicamenteuze behandeling geen ada element meer maar een veld in de medicatiebouwstenen -->
               <!--
                        <medicamenteuze_behandeling>
                         ...
                         -->
               <!-- from MP9 3.0.0 rc.2 onwards each resource contains a medicamenteuze_behandeling_id in ada -->
               <xsl:for-each select="f:Bundle/f:entry/f:resource/(f:MedicationRequest | f:MedicationDispense | f:MedicationStatement | f:MedicationAdministration)">
                  <!-- medicatieafspraak -->
                  <xsl:apply-templates select="current()[self::f:MedicationRequest/f:category/f:coding/f:code/@value = $maCode]"
                                       mode="mp-MedicationAgreement"/>
                  <!--wisselendDoseerschema in f:MedicationRequest-->
                  <xsl:apply-templates select="current()[self::f:MedicationRequest/f:category/f:coding/f:code/@value = $wdsCode]"
                                       mode="mp-VariableDosingRegimen"/>
                  <!-- verstrekkingsverzoek -->
                  <xsl:apply-templates select="current()[self::f:MedicationRequest/f:category/f:coding/f:code/@value = $vvCode]"
                                       mode="nl-core-DispenseRequest"/>
                  <!-- toedieningsafspraak -->
                  <xsl:apply-templates select="current()[self::f:MedicationDispense/f:category/f:coding/f:code/@value = $taCode]"
                                       mode="mp-AdministrationAgreement"/>
                  <!-- verstrekking -->
                  <xsl:apply-templates select="current()[self::f:MedicationDispense/f:category/f:coding/f:code/@value = $mveCode]"
                                       mode="nl-core-MedicationDispense"/>
                  <!-- medicatie_gebruik -->
                  <xsl:apply-templates select="current()[self::f:MedicationStatement/f:category/f:coding/f:code/@value = $mgbCode]"
                                       mode="mp-MedicationUse2"/>
                  <!-- medicatietoediening -->
                  <xsl:apply-templates select="current()[self::f:MedicationAdministration]"
