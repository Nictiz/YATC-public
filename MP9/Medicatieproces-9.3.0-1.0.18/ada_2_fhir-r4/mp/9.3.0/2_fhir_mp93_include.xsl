...
<xsl:stylesheet exclude-result-prefixes="#all"
                ...
                xmlns:local="#local.2024102208230950858220200"
   ...
   <xsl:variable name="bouwstenen-930"
                 as="element(f:entry)*">
      <!-- medicatieafspraken -->
      <!-- MP-2104 excluding the medicationagreement in a proposal -->
      <xsl:for-each select="//medicatieafspraak[not(parent::voorstel)]">
               ...
               <xsl:call-template name="mp-MedicationAgreement">
                  ...
                  <xsl:with-param name="requester"
                     ...
                     <xsl:choose>
                        ...
                        <!-- voorstel stuff support for different mbh-id structures see MP-2104 -->
                        <xsl:when test="(../../voorstel_gegevens/voorstel/auteur/* | ../voorstel_gegevens/voorstel/auteur/*)">
                           <xsl:for-each select="(../../voorstel_gegevens/voorstel/auteur | ../voorstel_gegevens/voorstel/auteur)">
                              <xsl:choose>
                  ...
                  <xsl:with-param name="nextPractitioner"
                                  as="element()?"
                                  select="(ancestor::*[@app]/bouwstenen/zorgverlener[@id = current()/volgende_behandelaar/zorgverlener/@value] | ancestor::*[@app]/bouwstenen/zorgaanbieder[@id = current()/volgende_behandelaar/zorgaanbieder/@value])[1]"/>
               </xsl:call-template>
            </resource>
         </entry>
      </xsl:for-each>
      ...
      <!-- MP-2104 excluding the dispenserequest in a proposal -->
      <xsl:for-each select="//verstrekkingsverzoek[not(parent::voorstel)]">
         <!-- entry for MedicationRequest -->
         <entry>
            <fullUrl value="{$fhirMetadata[nm:group-key/text() = nf:getGroupingKeyDefault(current())]/nm:full-url/text()}"/>
            <resource>
      ...
      <xsl:for-each select="//medicatietoediening">
         ...
         <xsl:variable name="fullUrlFromMeta"
                       select="$fhirMetadata[nm:group-key/text() = $key]/nm:full-url/text()"/>
         <!-- generate-id(.) is uniek binnen dit document -->
         <xsl:variable name="fallbackId"
                       select="concat('medadmin-', translate(generate-id(.), 'abcdefghijklmnopqrstuvwxyz', ''))"/>
         <entry>
            <fullUrl value="{ if (normalize-space($fullUrlFromMeta))                         then $fullUrlFromMeta                     else concat('urn:uuid:', $fallbackId) }"/>
            <resource>
         ...
         <entry>
            ...
            <resource>
               <Communication xsl:exclude-result-prefixes="#all">
                  ...
                  <meta>
                     <xsl:choose>
                        <xsl:when test="(antwoord_medicatieafspraak | antwoord_medicatieafspraak/antwoord_medicatieafspraak_besluit)[@code]">
                           <profile value="{concat($urlBaseNictizProfile, 'mp-ReplyProposalMedicationAgreement')}"/>
                        </xsl:when>
                        <xsl:when test="(antwoord_verstrekkingsverzoek | antwoord_verstrekkingsverzoek/antwoord_verstrekkingsverzoek_besluit)[@code]">
                  ...
                  <xsl:for-each select="antwoord_medicatieafspraak | antwoord_verstrekkingsverzoek">
                     <payload>
                        <contentString>
                           <xsl:variable name="toelichting"
                                         select="normalize-space((                                                     antwoord_medicatieafspraak_toelichting/@value,                                                     antwoord_verstrekkingsverzoek_toelichting/@value                                                 )[1])"/>
                           <xsl:if test="string-length($toelichting) gt 0">
                              <xsl:attribute name="value"
                                             select="$toelichting"/>
                           </xsl:if>
                           <!-- Besluit in de bestaande ContentCodeableConcept extension -->
                           <extension url="{$urlExtCommunicationPayloadContentCodeableConcept}">
                              <valueCodeableConcept>
                                 <xsl:choose>
                                    <xsl:when test="antwoord_medicatieafspraak_besluit[@code]">
                                       <xsl:for-each select="antwoord_medicatieafspraak_besluit">
                                          <xsl:call-template name="code-to-CodeableConcept"/>
                                       </xsl:for-each>
                                    </xsl:when>
                                    <xsl:when test="antwoord_verstrekkingsverzoek_besluit[@code]">
                                       <xsl:for-each select="antwoord_verstrekkingsverzoek_besluit">
                                          <xsl:call-template name="code-to-CodeableConcept"/>
                                       </xsl:for-each>
                                    </xsl:when>
                                    <xsl:otherwise>
   ...
   <xsl:template name="date-to-datetime">
      <!--  override date xslt template to allow for T-dates, TODO temp solution, should be done outside of normal xslt's  -->
      <xsl:param name="in"
                 as="element()?"
                 select=".">
