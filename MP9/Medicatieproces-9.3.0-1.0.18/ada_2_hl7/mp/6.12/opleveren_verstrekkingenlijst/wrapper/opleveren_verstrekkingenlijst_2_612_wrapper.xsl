...
<!-- == Provenance: YATC-internal/ada-2-hl7/env/mp/6.12/opleveren_verstrekkingenlijst/wrapper/opleveren_verstrekkingenlijst_2_612_wrapper.xsl == -->
...
<xsl:stylesheet version="2.0"
   ...
   <!--
    ...
    -->
   <!-- ================================================================== -->
   <xsl:import href="../payload/opleveren_verstrekkingenlijst_2_612.xsl"/>
...
<xsl:value-of select="./@phase"/>"</xsl:processing-instruction>
         ...
         <xsl:element name="{./root_xml_element/@value}">
            ...
            <xsl:for-each select="hl7:acknowledgement">
               <xsl:copy-of select="."/>
            </xsl:for-each>
            ...
            <xsl:for-each select="./controlact_wrapper">
               <ControlActProcess classCode="CACT"
                                  moodCode="EVN">
                  <!-- authorOrPerformer is the same organizaiton as author -->
                  <xsl:variable name="mve"
                                select="($payload_xml//beschikbaarstellen_medicatiegegevens//medicatieverstrekking[verstrekker/zorgaanbieder])[1]"
                                as="element()?"/>
                  <xsl:variable name="mveAuteur"
                                select="$mve/ancestor::*/bouwstenen/zorgaanbieder[@id = $mve/verstrekker/zorgaanbieder/@value]"
                                as="element(zorgaanbieder)?"/>
                  <xsl:variable name="author"
                                select="author_or_performer"/>
                  <xsl:choose>
                     <xsl:when test="$mveAuteur">
                        <xsl:for-each select="$mveAuteur">
                           <authorOrPerformer typeCode="AUT">
                              <participant>
                                 <AssignedDevice>
                                    <xsl:for-each select="$author/assigned_device/(id | hl7:id)">
                                       <xsl:call-template name="makeId"/>
                                    </xsl:for-each>
                                    <xsl:call-template name="makeOrganization">
                     ...
                     <xsl:otherwise>
                        <xsl:for-each select="$author/assigned_device">
                           <authorOrPerformer typeCode="AUT">
                              <participant>
                                 <AssignedDevice>
                                    ...
                                    <Organization classCode="ORG"
                                       ...
                                       <xsl:copy-of select="organization/hl7:*"/>
                                    </Organization>
                                 </AssignedDevice>
                              </participant>
                           </authorOrPerformer>
                  ...
                  <subject>
                     ...
                     <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9026_20150318000000">
                        <xsl:with-param name="mbh"
                                        select="$payload_xml//beschikbaarstellen_medicatiegegevens/(medicamenteuze_behandeling | .)"/>
                        <xsl:with-param name="patient"
                                        select="$payload_xml//beschikbaarstellen_medicatiegegevens/patient"/>
                     </xsl:call-template>
                  </subject>
                  <xsl:for-each select="hl7:queryAck">
                     <xsl:copy-of select="."/>
                  </xsl:for-each>
               </ControlActProcess>
            </xsl:for-each>
   ...
   <xsl:template name="makeDevice">
      <device classCode="DEV"
         ...
         <xsl:for-each select="name | hl7:name">
            <name>
               <xsl:value-of select="./@value"/>
            </name>
         </xsl:for-each>
   ...
   <xsl:template name="makeId">
      <id extension="{(@value | @extension)[1]}"
          root="{@root}"/>
   </xsl:template>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   ...
   <xsl:template name="makeOrganization">
      <xsl:param name="in"
                 as="element(zorgaanbieder)?"
                 select="."/>
