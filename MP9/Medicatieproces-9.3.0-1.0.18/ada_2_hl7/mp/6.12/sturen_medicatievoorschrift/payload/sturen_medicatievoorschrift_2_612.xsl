...
<xsl:stylesheet version="2.0"
   ...
   <xsl:template match="/">
      ...
      <xsl:variable name="mbhCandidates"
                    as="element()*">
         <xsl:call-template name="makeVoorschriftMbhGroups">
            <xsl:with-param name="voorschrift"
                            select="$voorschrift"/>
         </xsl:call-template>
      </xsl:variable>
      <xsl:variable name="mbhCandidateNodes"
                    select="$mbhCandidates[self::medicamenteuze_behandeling] | $mbhCandidates/medicamenteuze_behandeling"
                    as="element(medicamenteuze_behandeling)*"/>
      <xsl:choose>
         <xsl:when test="$mbhCandidateNodes[verstrekkingsverzoek]">
            ...
            <xsl:call-template name="Voorschrift_612">
               ...
               <xsl:with-param name="mbh"
                               select="$mbhCandidateNodes[verstrekkingsverzoek]"/>
            </xsl:call-template>
         </xsl:when>
         <xsl:otherwise>
   ...
   <xsl:template name="Make_Voorschrift_612">
      ...
      <xsl:variable name="mbhCandidateNodes"
                    select="$mbhCandidates[self::medicamenteuze_behandeling] | $mbhCandidates/medicamenteuze_behandeling"
                    as="element(medicamenteuze_behandeling)*"/>
      <xsl:if test="$mbhCandidateNodes[verstrekkingsverzoek]">
         <!-- alleen conversie naar 6.12 vooraankondiging als er ook een verstrekkingsverzoek is -->
         <xsl:call-template name="Voorschrift_612">
            <xsl:with-param name="patient"
                            select="$voorschrift/patient"/>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template name="makeVoorschriftMbhGroups">
      <!--
            Normalization:
            - old: medicatieafspraak/verstrekkingsverzoek below medicamenteuze_behandeling
            - new: medicatieafspraak/verstrekkingsverzoek as top-level bouwstenen with medicamenteuze_behandeling_id
        -->
      <xsl:param name="voorschrift"
                 as="element()*"/>
      <xsl:choose>
         <xsl:when test="$voorschrift/(medicatieafspraak | verstrekkingsverzoek)[medicamenteuze_behandeling_id]">
            <sturen_medicatievoorschrift xmlns="">
               <xsl:copy-of select="$voorschrift/bouwstenen"/>
               <xsl:for-each-group select="$voorschrift/(medicatieafspraak | verstrekkingsverzoek)[medicamenteuze_behandeling_id]"
                                   group-by="concat(medicamenteuze_behandeling_id/@root, '|', medicamenteuze_behandeling_id/@value)">
                  <xsl:if test="current-group()[self::verstrekkingsverzoek]">
                     <medicamenteuze_behandeling>
                        <xsl:copy-of select="current-group()[1]/medicamenteuze_behandeling_id"/>
                        <xsl:copy-of select="current-group()[self::medicatieafspraak]"/>
                        <xsl:copy-of select="current-group()[self::verstrekkingsverzoek]"/>
                     </medicamenteuze_behandeling>
                  </xsl:if>
               </xsl:for-each-group>
         ...
         <xsl:otherwise>
            <xsl:sequence select="$voorschrift/medicamenteuze_behandeling"/>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:template>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
