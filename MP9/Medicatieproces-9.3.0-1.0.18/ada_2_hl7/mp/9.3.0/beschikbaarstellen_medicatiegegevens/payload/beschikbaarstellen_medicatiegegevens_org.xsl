...
<!-- == Provenance: YATC-internal/ada-2-hl7/env/mp/9.3.0/beschikbaarstellen_medicatiegegevens/payload/beschikbaarstellen_medicatiegegevens_org.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
                ...
                xmlns:local="#local.2024011810474753748190100">
   <!-- ================================================================== -->
   <!--
   ...
   <xsl:template match="/"
      ...
      <xsl:if test="$schematronRef">
         <xsl:processing-instruction name="xml-model">phase="#ALL" href="file:/C:/SourceTree/SVN/AORTA/trunk/Zorgtoepassing/Medicatieproces/DECOR/mp-runtime-develop/mp-mp93_mgb.sch" type="application/xml" schematypens="http://purl.oclc.org/dsdl/schematron"</xsl:processing-instruction>
         <xsl:text>
         </xsl:text>
      </xsl:if>
      <xsl:choose>
               ...
               <xsl:for-each select="$in">
                  <xsl:call-template name="Medicatiegegevens_930">
                     ...
                     <xsl:with-param name="mbh"
                                     select="if (exists(medicamenteuze_behandeling))                                 then medicamenteuze_behandeling                                 else ."/>
                  </xsl:call-template>
               </xsl:for-each>
            </batch>
         </xsl:when>
         <xsl:otherwise>
            <xsl:call-template name="Medicatiegegevens_930">
               ...
               <!-- backward compatible container -->
               <xsl:with-param name="mbh"
                               select="if (exists($in/medicamenteuze_behandeling))                         then $in/medicamenteuze_behandeling                         else $in"/>
            </xsl:call-template>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:template>
   ...
   <xsl:template name="Medicatiegegevens_930">
      ...
      <organizer classCode="CLUSTER"
         ...
         <xsl:for-each select="$mbh/wisselend_doseerschema">
            <component typeCode="COMP">
         ...
         <!-- Verstrekking -->
         <xsl:for-each select="$mbh/(medicatieverstrekking | verstrekking)">
            <component typeCode="COMP">
         ...
         <!-- Medicatiegebruik -->
         <xsl:for-each select="$mbh/(medicatie_gebruik | medicatiegebruik)">
            <component typeCode="COMP">
         ...
         <!-- Medicatietoediening -->
         <xsl:for-each select="$mbh/medicatietoediening">
            <component typeCode="COMP">
