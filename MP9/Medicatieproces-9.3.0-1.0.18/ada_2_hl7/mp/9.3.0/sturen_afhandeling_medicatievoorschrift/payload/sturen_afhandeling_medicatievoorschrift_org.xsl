...
<xsl:value-of select="$in/@title"/>" and id: "
<xsl:value-of select="$in/@id"/>".</xsl:comment>
      </xsl:if>
      <organizer classCode="CLUSTER"
         ...
         <!-- Toedieningsafspraak -->
         <xsl:for-each select="$mbh/toedieningsafspraak[not(kopie_indicator/@value = 'true')]">
            <component typeCode="COMP">
               <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9416_20221121074758"/>
            </component>
         </xsl:for-each>
         <!-- Toedieningsafspraak kopie -->
         <xsl:for-each select="$mbh/toedieningsafspraak[kopie_indicator/@value = 'true']">
            <component typeCode="COMP">
