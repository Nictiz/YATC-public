...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <!--
    ...
    -->
   <!-- ================================================================== -->
   <!-- import because we want to be able to override the params -->
   ...
   <xsl:template name="any-to-value">
      ...
      <xsl:for-each select="$in">
         ...
         <xsl:choose>
            ...
            <xsl:when test="$theDatatype = ('count','quantity', 'duration', 'currency', 'decimal', 'integer') or @unit">
...
<xsl:value-of select="$in/@value"/>" </xsl:with-param>
   ...
   <xsl:template name="string-to-string"
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <!-- value attribute may not be empty in FHIR, but if it really is empty, let's stick a non-breaking-space in it ;-) -->
            <xsl:attribute name="value"
                           select="' '"/>
         </xsl:otherwise>
      </xsl:choose>
   ...
   <xsl:template name="code-to-CodeableConcept"
      ...
      <xsl:choose>
         ...
         <xsl:when test="$out[not(@codeSystem = $oidHL7NullFlavor) or $treatNullFlavorAsCoding]">
            ...
            <xsl:for-each select="($out/translation, $in/following-sibling::*[1][self::vertaling])">
               <xsl:element name="{$elementName}"
                            namespace="http://hl7.org/fhir">
                  <xsl:call-template name="code-to-Coding">
                     <xsl:with-param name="in"
...
<xsl:value-of select="$oid"/>"</xsl:with-param>
   ...
   <xsl:function name="nf:make-fhir-logicalid"
      ...
      <xsl:variable name="joinedString"
                    ...
                    as="xs:string"/>
      <xsl:variable name="customShortenedString"
                    select="$joinedString"
                    as="xs:string?">
         <!--<xsl:choose>
                <xsl:when test="matches($joinedString, 'zib-LaboratoryTestResult-Observation')">
                    <xsl:value-of select="replace($joinedString, 'zib-LaboratoryTestResult-Observation', 'zib-LabTestResult')"/>
                </xsl:when>
                <xsl:when test="matches($joinedString, 'zib-LaboratoryTestResult-Specimen')">
                    <xsl:value-of select="replace($joinedString, 'zib-LaboratoryTestResult-Specimen', 'zib-LabTestResult-Specimen')"/>
                </xsl:when>
                <xsl:when test="matches($joinedString, 'zib-FunctionalOrMentalStatus')">
                    <xsl:value-of select="replace($joinedString, 'zib-FunctionalOrMentalStatus', 'zib-FuncMentStat')"/>
                </xsl:when>
            </xsl:choose>-->
      </xsl:variable>
      <xsl:variable name="lengthJoinedString"
                    ...
                    as="xs:integer"/>
      <xsl:variable name="lengthCustomString"
                    select="string-length($customShortenedString)"
                    as="xs:integer"/>
      <xsl:variable name="startingLoc"
                    as="xs:integer">
         <xsl:choose>
            <xsl:when test="$lengthJoinedString gt 64 and empty($customShortenedString)">
               <xsl:variable name="start"
                             select="$lengthJoinedString - 63"/>
               <xsl:call-template name="util:logMessage">
                  <xsl:with-param name="msg">We have encountered an id (
<xsl:value-of select="$joinedString"/>) longer than 64 characters, we are truncating it to "
<xsl:value-of select="substring($joinedString, $start, $lengthJoinedString)"/>", but it should be looked at.</xsl:with-param>
                  <xsl:with-param name="level"
                                  select="$logWARN"/>
                  <xsl:with-param name="terminate"
                                  select="false()"/>
            ...
            <xsl:when test="$lengthJoinedString gt 64 and $lengthCustomString gt 64">
               <xsl:variable name="start"
                             select="$lengthJoinedString - 63"/>
               <xsl:call-template name="util:logMessage">
                  <xsl:with-param name="msg">We have encountered an id (
<xsl:value-of select="$joinedString"/>) longer than 64 characters, we are truncating it to "
<xsl:value-of select="substring($customShortenedString, $start, $lengthCustomString)"/>", but it should be looked at.</xsl:with-param>
                  <xsl:with-param name="level"
                                  select="$logWARN"/>
                  <xsl:with-param name="terminate"
                                  select="false()"/>
      ...
      <xsl:choose>
         <xsl:when test="empty($customShortenedString)">
            <xsl:value-of select="substring($joinedString, $startingLoc, $lengthJoinedString)"/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="substring($customShortenedString, $startingLoc, $lengthCustomString)"/>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:function>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:function name="nf:getGroupingKeyDefault"
      ...
      <xsl:param name="in"
                 as="element()?"/>
      <xsl:if test="$in">
         <!-- use case for adding toplevel id into the mix is geboortezorg when a twin is born in 2 uitdrijvingsfases using the exact same procedure. There would be no data to distinguish them. But by giving them individual ids you may still create a pseudo difference -->
         <!-- use case for considering all ids in the path is again geboortezorg where two Observations originating from different ada files were overwriting eachother because they are indistinguishable other than the ada id they came from. Note: if you use xsl:copy instead of xsl:sequence before this function is called, you will not get all ids from the path leading to potential mismatches! -->
         <xsl:value-of select="upper-case(string-join(($in/ancestor-or-self::*/@id, $in//@value, $in//@root, $in//@unit, $in//@code[not(../@codeSystem = $oidHL7NullFlavor)], $in//@codeSystem[not(. = $oidHL7NullFlavor)], $in//*[@codeSystem = $oidHL7NullFlavor][@code = 'OTH']/@originalText)/normalize-space(), ''))"/>
      </xsl:if>
   </xsl:function>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   ...
   <xsl:function name="nf:getGroupingKeyLaboratoryTest"
      ...
      <xsl:param name="in"
                 as="element()?"/>
      <xsl:if test="$in">
         <xsl:value-of select="upper-case(string-join((nf:getGroupingKeyDefault($in)), ''))"/>
      </xsl:if>
   </xsl:function>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   ...
   <xsl:function name="nf:ada-resolve-reference"
      ...
      <!-- BgZ: everything is in <bundle> -->
      <!-- regular: everything is in /adaxml/data/* -->
      <xsl:variable name="currentAdaTransaction"
                    select="($adaElement/ancestor::*[ancestor::data[ancestor::adaxml]], $adaElement/ancestor::bundle[last()])[1]"/>
      <!-- resolve the reference -->
      <xsl:choose>
         <xsl:when test="$adaElement[not(@datatype = 'reference')]">
            <xsl:sequence select="$adaElement"/>
         </xsl:when>
         <xsl:when test="$adaElement">
            <xsl:if test="empty($currentAdaTransaction//*[name() = name($adaElement)][@id = $adaElement/@value])">
               <xsl:call-template name="util:logMessage">
                  <xsl:with-param name="level"
                                  select="$logWARN"/>
                  <xsl:with-param name="msg">
                     <xsl:text>Could not find target for </xsl:text>
                     <xsl:value-of select="$adaElement/string-join((name(), @value), '/')"/>
                     <xsl:text> at path: /</xsl:text>
                     <xsl:value-of select="string-join(for $n in $adaElement/ancestor-or-self::* return concat(name($n), '[', count($n/preceding-sibling::*[name() = name($n)]) + 1, ']'), '/')"/>
                  </xsl:with-param>
                  <xsl:with-param name="terminate"
                                  select="false()"/>
               </xsl:call-template>
            </xsl:if>
            <xsl:sequence select="$currentAdaTransaction//*[name() = name($adaElement)][@id = $adaElement/@value]"/>
         </xsl:when>
      </xsl:choose>
   </xsl:function>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:function name="nf:get-ada-patient"
                 as="element()*">
      <xsl:param name="in"
                 as="element()?"/>
      <!-- laboratorium_test will be under parent laboratorium_uitslag with patient -->
      <xsl:variable name="thePatientReference"
                    select="($in/zibroot/onderwerp//patient[@datatype = 'reference'][@value] | $in/parent::laboratorium_uitslag/zibroot/onderwerp//patient[@datatype = 'reference'][@value])[1] | $in/hcimroot/subject//patient[@datatype = 'reference'][@value]"
                    as="element()*"/>
      <!-- BgZ MM  3.1: ancestor::bundle/patient -->
      <!-- BgZ MSZ 1.2: ancestor::bundle//onderwerp//patient[not(patient)], ancestor::bundle//subject//patient[not(patient)] -->
      <!-- BgZ MSZ 2.0: ancestor::adaxml/data/demografie_en_identificatie/onderwerp//patient[not(patient)], ancestor::bundle//subject//patient[not(patient)] -->
      <!-- Ketenzorg: ancestor::*/bundle//onderwerp//patient[not(patient)], ancestor::*/bundle//subject//patient[not(patient)] -->
      <xsl:variable name="thePatient"
                    select="$in/(ancestor::*/patient, ancestor::bundle/patient[not(patient)], ancestor::bundle//onderwerp//patient[not(patient)], ancestor::bundle//subject//patient[not(patient)], ancestor::*/bundle//onderwerp//patient[not(patient)], ancestor::*/bundle//subject//patient[not(patient)], ancestor::data[parent::adaxml]/*/demografie_en_identificatie/patient[1])[*//@value]"
                    as="element()*"/>
      <xsl:choose>
         <xsl:when test="$thePatientReference">
            <xsl:sequence select="nf:ada-resolve-reference($thePatientReference)"/>
         </xsl:when>
         <xsl:when test="$thePatient">
            <xsl:sequence select="$thePatient[1]"/>
         </xsl:when>
         <xsl:when test="$in">
            <xsl:call-template name="util:logMessage">
               <xsl:with-param name="level"
                               select="$logWARN"/>
               <xsl:with-param name="msg">
                  <xsl:text> Could not find a patient at path: /</xsl:text>
                  <xsl:value-of select="string-join(for $n in $in/ancestor-or-self::* return concat(name($n), '[', count($n/preceding-sibling::*[name() = name($n)]) + 1, ']'), '/')"/>
               </xsl:with-param>
               <xsl:with-param name="terminate"
                               select="false()"/>
            </xsl:call-template>
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            ...
            <xsl:variable name="tokenResourceId"
                          as="xs:string*">
               <xsl:choose>
                  <xsl:when test="string-length($adaEigenAchternaam) = 0">
                     <!-- empty last name, this is really not how it should be, let's report and quit here -->
                     <xsl:call-template name="util:logMessage">
                        <xsl:with-param name="level"
                                        select="$logERROR"/>
                        <xsl:with-param name="msg">
                           <xsl:text>Attempting to find a patient in QualificationTokens using an empty last name. That's asking for trouble, please investigate.</xsl:text>
                        </xsl:with-param>
                     </xsl:call-template>
                  </xsl:when>
                  <xsl:otherwise>
                     <xsl:sequence select="($patientTokensXml//*[familyName/upper-case(normalize-space(text())) = $adaEigenAchternaam]/resourceId/text())"/>
                  </xsl:otherwise>
               </xsl:choose>
            </xsl:variable>
            <xsl:choose>
               <xsl:when test="count($tokenResourceId) = 0">
                  <xsl:call-template name="util:logMessage">
                     <xsl:with-param name="level"
                                     select="$logDEBUG"/>
                     <xsl:with-param name="msg">
                        <xsl:text>Did not find a stable resource id in QualificationTokens for exact match on bsn or last name '</xsl:text>
                        <xsl:value-of select="$adaEigenAchternaam"/>
                        <xsl:text>'. We'll make one up ourselves.</xsl:text>
                     </xsl:with-param>
                  </xsl:call-template>
               </xsl:when>
               <xsl:when test="count($tokenResourceId) = 1">
               ...
               <xsl:when test="count($tokenResourceId) gt 1">
                  ...
                  <xsl:call-template name="util:logMessage">
                     <xsl:with-param name="level"
                                     select="$logERROR"/>
                     <xsl:with-param name="msg">
                        <xsl:text>Found more then one resource id in QualificationTokens for exact match on last name </xsl:text>
                        <xsl:value-of select="$adaEigenAchternaam"/>
                     ...
                     <xsl:when test="count($tokenResourceId) gt 1">
                        ...
                        <xsl:call-template name="util:logMessage">
                           <xsl:with-param name="level"
                                           select="$logDEBUG"/>
                           <xsl:with-param name="msg">
                              <xsl:text>Found more then one resource id in QualificationTokens for contains of last name </xsl:text>
                              <xsl:value-of select="$adaEigenAchternaam"/>
                              <xsl:text>. So we will not use any of those.</xsl:text>
                           </xsl:with-param>
                        </xsl:call-template>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:variable name="nf:oidPattern">^[0-2](\.(0|[1-9][0-9]*))*$</xsl:variable>
   <xsl:function name="nf:isOID"
                 as="xs:boolean">
      <xsl:param name="in"
                 as="xs:string?"/>
      <xsl:sequence select="matches($in, $nf:oidPattern)"/>
   </xsl:function>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:variable name="nf:uuidPattern">^[A-Fa-f\d]{8}-[A-Fa-f\d]{4}-[A-Fa-f\d]{4}-[A-Fa-f\d]{4}-[A-Fa-f\d]{12}$</xsl:variable>
   <xsl:function name="nf:isUUID"
                 as="xs:boolean">
      <xsl:param name="in"
                 as="xs:string?"/>
      <xsl:sequence select="matches($in, $nf:uuidPattern)"/>
   </xsl:function>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
