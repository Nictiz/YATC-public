...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <!--
    ...
    -->
   <!-- ================================================================== -->
   <xsl:import href="2_fhir_fhir_include-d660e202.xsl"/>
   ...
   <xsl:param name="serverBaseUri"
              ...
              as="xs:string"/>
   <!-- ada to resourceType, sort on ada elementname alphabetically to make the developer's live easier -->
   <xsl:variable name="ada2resourceType">
      <nm:map ada="aandoening_of_gesteldheid"
              resource="Condition"
              profile="cio-Condition"/>
      <nm:map ada="aanvraaggegevens"
              resource="ServiceRequest"
              profile="lu-OrderData"/>
      <nm:map ada="aanvraaggegevens"
              resource="DiagnosticReport"
              profile="lu-LaboratoryTestResult-DiagnosticReport"/>
      <nm:map ada="afleverlocatie"
              resource="Location"
              profile="nl-core-Dummy"/>
      <nm:map ada="afwijkende_uitslag"
...
<xsl:value-of select="$groupKey"/>' in $fhirMetadata, while no $profile was supplied.</xsl:with-param>
                  ...
                  <xsl:if test="not($fhirMetadata[@profile = $profile and nm:group-key = $groupKey])">
                     <xsl:call-template name="util:logMessage">
                        <xsl:with-param name="level"
                                        select="$logFATAL"/>
                        <xsl:with-param name="msg">makeReference: Multiple entries found for $groupKey '
<xsl:value-of select="$groupKey"/>' in $fhirMetadata, but none with profile. Is the supplied $profile ('
<xsl:value-of select="$profile"/>') valid?</xsl:with-param>
                        <xsl:with-param name="terminate"
                                        select="true()"/>
                     </xsl:call-template>
                  </xsl:if>
...
<xsl:value-of select="$groupKey"/>
      ...
      <!-- fullUrl as stored in metadata (same selection logic as insertFullUrl) -->
      ...
      <!-- If fullUrl is urn:uuid:..., extract UUID part -->
      <xsl:variable name="uuidFromFullUrl"
                    select="if (starts-with($fullUrl, 'urn:uuid:'))                     then substring-after($fullUrl, 'urn:uuid:')                 else ()"/>
      <!-- Generate FHIR id based on ADA identifier and make sure it meets the logicalid criteria -->
      <xsl:variable name="idNode"
                    select="$in/identificatie[1]"/>
      <xsl:variable name="theID"
                    select="if ($idNode[@root = $mask-ids-var] | $idNode[@nullFlavor])                     then ()                 else (string-join(($idNode/@root, $idNode/@value), '-'))"/>
      <xsl:variable name="theUUID"
                    select="if ($idNode[@root = $mask-ids-var] | $idNode[@nullFlavor])                     then ()                 else ($idNode/@value)"/>
      <xsl:variable name="cleanID"
                    select="nf:assure-logicalid-chars($theID)"/>
      <xsl:variable name="cleanUUID"
                    select="nf:assure-logicalid-chars($theUUID)"/>
      <xsl:variable name="idFromIdentifier">
         <xsl:choose>
            <!-- root+value if valid length -->
            <xsl:when test="matches($cleanID, '^[A-Za-z\d\.-]{1,64}$')                     and string-length($cleanID) le 64">
               <xsl:value-of select="$cleanID"/>
            </xsl:when>
            <!-- value if it already is a UUID -->
            <xsl:when test="matches($cleanUUID, $UUIDpattern)                     and string-length($cleanUUID) le 64">
               <xsl:value-of select="$cleanUUID"/>
            </xsl:when>
            <!-- fallback: random UUID (only used when not uuid-referencing, or no fullUrl) -->
            <xsl:otherwise>
               <xsl:value-of select="uuid:get-uuid(.)"/>
            </xsl:otherwise>
         </xsl:choose>
      ...
      <!-- FINAL DECISION:
             1) For referencingStrategy='uuid', prefer the UUID from fullUrl
             2) Otherwise, use metadata logicalId if present
             3) Otherwise, fall back to identifier-based id -->
      <xsl:choose>
         <xsl:when test="$referencingStrategy = 'uuid'                 and string-length($uuidFromFullUrl) gt 0">
            <xsl:value-of select="$uuidFromFullUrl"/>
         </xsl:when>
         <xsl:when test="string-length($logicalId) gt 0">
            <xsl:value-of select="$logicalId"/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="$idFromIdentifier"/>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:template>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
...
<xsl:value-of select="$logicalId"/>) longer than 64 characters, we are truncating it, but it should be looked at.</xsl:with-param>
   ...
   <xsl:function name="nf:getGroupingKeyDefault"
                 as="xs:string?">
      <!-- Override here for R4 to NOT use ada @id attribute as has been recently introduced in STU3. This was done for geboortezorg initially. 
                However, geboortezorg has own key generation, due to having mother and possible multiplebirth stuff. 
                STU3 has been tested thoroughly with the @id added, so no desire to abandon that at this stage.
                However, the R4-version has been tested extensively without it and deduplication does not work properly with ada @id. 
                The STU3 version does not deduplicate on actual, functional contents, because it also takes ada @id into account. 
                The original key design is needed for example in the mp-lab usecases where the same patient/healthprofessional/provider is in two different ada transactions with own unique ada/@ids -->
   ...
   <xsl:template name="_buildMedicationQuantity">
      ...
      <!-- G-Standaard (Simple)Quantity and other translations -->
      <xsl:choose>
         <!-- use adaextension if present -->
         <xsl:when test="$adaValue/adaextension/translation">
            <xsl:for-each select="$adaValue/adaextension/translation">
               <extension url="{$urlExtIso21090PQtranslation}">
                  <valueQuantity>
                     ...
                     <system value="{concat('urn:oid:', @codeSystem)}"/>
                     <code value="{@code}"/>
                  </valueQuantity>
               </extension>
            </xsl:for-each>
         ...
         <xsl:otherwise>
            <!-- original code without adaextension -->
      ...
      <!-- UCUM -->
      <!-- we use the adaextension originalUCUM if present, otherwise the original code -->
      <xsl:choose>
         <xsl:when test="$adaValue/adaextension/originalUCUM[@value | @unit]">
            <value value="{$adaValue/adaextension/originalUCUM/@value}"/>
            <xsl:if test="string-length($adaUnit/@displayName) gt 0">
               <unit value="{$adaUnit/@displayName}"/>
            </xsl:if>
            <system value="{$oidMap[@oid=$oidUCUM]/@uri}"/>
            <code value="{$adaValue/adaextension/originalUCUM/@unit}"/>
         </xsl:when>
         <xsl:otherwise>
            <!-- original code without adaextension -->
            <value value="{$adaValue/@value}"/>
