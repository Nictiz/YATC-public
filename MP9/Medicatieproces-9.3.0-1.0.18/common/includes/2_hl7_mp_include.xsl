...
<xsl:stylesheet exclude-result-prefixes="#all"
                ...
                xmlns:local="#local.2024031911041855825750100"
   ...
   <xsl:output method="xml"
               indent="yes"/>
   <!-- helper functie voor nieuwe MBHId -->
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <!-- copy the instruction text, this is a separate template to make overriding for testing purposes easier -->
   <xsl:template name="makeInstructionText">
      <xsl:param name="in"
                 as="element()*"
                 select="gebruiksinstructie/omschrijving[@value]"/>
      <xsl:for-each select="$in">
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9103_20160624141250">
      ...
      <xsl:for-each select="rol_of_functie[@code]">
         <xsl:call-template name="makeCode"/>
      </xsl:for-each>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9150_20160726150449">
      ...
      <rateQuantity>
         ...
         <xsl:choose>
            <xsl:when test="(minimum_waarde | nominale_waarde | maximum_waarde)[@value][@unit]">
               <!-- dataset from MP9 3.0.0 beta.4 -->
               <!-- ada @unit hould have proper UCUM, NICTIZ-14936 -->
               <xsl:for-each select="minimum_waarde[@value][@unit]">
                  <low value="{normalize-space(@value)}"
                       unit="{normalize-space(@unit)}"/>
               </xsl:for-each>
               <xsl:for-each select="nominale_waarde[@value][@unit]">
                  <center value="{normalize-space(@value)}"
                          unit="{normalize-space(@unit)}"/>
               </xsl:for-each>
               <xsl:for-each select="maximum_waarde[@value][@unit]">
                  <high value="{normalize-space(@value)}"
                        unit="{normalize-space(@unit)}"/>
               </xsl:for-each>
            </xsl:when>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9212_20180420174745"
      ...
      <xsl:for-each select="$in">
         <act classCode="ACT"
            ...
            <!-- Voorstelgegevens antwoord, allows for new dataset structure from MP9 3.0.0-rc.2 onwards -->
            ...
            <xsl:for-each select="relatie_verstrekkingsverzoek[identificatie[@value | @root | @nullFlavor]]">
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9213_20180420181642">
      ...
      <xsl:param name="in"
         ...
         <!-- the ada element for antwoord_verstrekkingsverzoek, defaults to context
                 MP-949: dataset structure change to antwoord_verstrekkingsverzoek/antwoord_verstrekkingsverzoek_besluit with added toelichting -->
      </xsl:param>
      <xsl:for-each select="$in">
         <observation classCode="OBS"
                      moodCode="EVN">
            ...
            <code code="17"
                  ...
                  codeSystemName="Medicatieproces observaties"
                  displayName="Antwoord voorstel verstrekkingsverzoek"/>
            <!-- allow for old and new dataset structure since MP-949 -->
            <xsl:for-each select=".[@code] | antwoord_verstrekkingsverzoek_besluit[@code]">
               <xsl:call-template name="makeCEValue"/>
            </xsl:for-each>
            <xsl:for-each select="antwoord_verstrekkingsverzoek_toelichting[@value | @nullFlavor]">
               <entryRelationship typeCode="SUBJ"
                                  inversionInd="true">
         ...
         <xsl:when test="$ada-auteur[.//(@value | @code | @nullFlavor)]">
            <xsl:for-each select="$ada-auteur">
               <author>
                  ...
                  <!-- auteur -->
                  <xsl:for-each select="auteur_is_zorgaanbieder/zorgaanbieder[*//(@value | @code)] | ancestor::data/*/bouwstenen/zorgaanbieder[@id = current()/auteur_is_zorgaanbieder/zorgaanbieder/@value]">
                     <assignedAuthor>
                        <id nullFlavor="NI"/>
                        <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.3.10.0.5_20180611000000"/>
                     </assignedAuthor>
                  </xsl:for-each>
                  <xsl:for-each select="auteur_is_zorgverlener/zorgverlener[*//(@value | @code)] | ancestor::data/*/bouwstenen/zorgverlener[@id = current()/auteur_is_zorgverlener/zorgverlener/@value]">
                     <assignedAuthor>
                        <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9113_20181205174044"/>
                     </assignedAuthor>
                  </xsl:for-each>
...
-->
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9364_20210602161935"
      ...
      <xsl:for-each select="$in">
         <supply classCode="SPLY"
            ...
            <!-- verstrekt_geneesmiddel -->
            <xsl:for-each select="(ancestor::*[bouwstenen])[1]/bouwstenen/farmaceutisch_product[@id = current()/verstrekt_geneesmiddel/farmaceutisch_product/@value]">
               <product>
            ...
            <!-- verstrekker/zorgaanbieder -->
            <xsl:for-each select="(ancestor::*[bouwstenen])[1]/bouwstenen/zorgaanbieder[@id = current()/verstrekker/zorgaanbieder/@value]">
               <performer>
                  <assignedEntity>
                     <id nullFlavor="NI"/>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9375_20210616173557"
                 match="medicatie_toediening_reden_van_afwijken | medicatietoediening_reden_van_afwijken"
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9397_20220315000000">
      ...
      <xsl:param name="in"
         ...
         <!-- the ada element for antwoord_medicatieafspraak, defaults to context
                 MP-949: dataset structure change to antwoord_medicatieafspraak/antwoord_medicatieafspraak_besluit with added toelichting -->
      </xsl:param>
      <xsl:for-each select="$in">
         <observation classCode="OBS"
                      moodCode="EVN">
            ...
            <!-- allow for old and new dataset structure since MP-949 -->
            <xsl:for-each select=".[@code] | antwoord_medicatieafspraak_besluit[@code]">
               <xsl:call-template name="makeCEValue"/>
            </xsl:for-each>
            <xsl:for-each select="antwoord_medicatieafspraak_toelichting[@value | @nullFlavor]">
               <entryRelationship typeCode="SUBJ"
                                  inversionInd="true">
                  <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.3.10.0.32_20210701000000"/>
               </entryRelationship>
