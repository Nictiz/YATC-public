...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:import href="ada2hl7_all-zibs-d660e332.xsl"/>
   ...
   <xsl:template name="makeTherapeuticAgentOf_4_template_2.16.840.1.113883.2.4.3.11.60.20.77.10.100_20130521000000">
      ...
      <xsl:variable name="cyclisch-met-start"
                    select="($herhaalperiode/@value) and count($adaBouwsteen//doseerinstructie) gt 1 and $gebruiksperiode-start/@value"/>
      <!-- bij een schema met startdatum gebruik, en doseerduur in alle doseerinstructies de juiste startdatum berekenen, 
      ...
      <xsl:variable name="gebruiksperiode-start-value"
                    as="xs:string?">
         <xsl:choose>
            <xsl:when test="($niet-cyclisch-met-start or $cyclisch-met-start) and not(exists($doseerinstructies[not(doseerduur)]))">
               <xsl:value-of select="concat(nf:calculate_Doseerinstructie_Startdate(xs:date(substring($gebruiksperiode-start/@value, 1, 10)), $dosering, $doseerinstructies), substring($gebruiksperiode-start/@value, 11))"/>
            </xsl:when>
            <xsl:otherwise>
      ...
      <therapeuticAgentOf>
         <medicationAdministrationRequest classCode="SBADM"
                                          moodCode="RQO">
            <!-- Item(s) :: omschrijving -->
            <!-- use XSL template to make override possible for test instances -->
            <xsl:call-template name="gebruiksomschrijving2text612">
               <xsl:with-param name="in"
                               select="$adaBouwsteen/gebruiksinstructie/omschrijving[@value]"/>
            </xsl:call-template>
...
<xsl:value-of select="$aantal_keer"/>". Dit kan niet gestructureerd worden meegegeven.</xsl:comment>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <!-- separate template to make overriding possible for testing purposes -->
   <xsl:template name="gebruiksomschrijving2text612">
      <!-- the ada element which had the omschrijving in @value, normally max 1 omschrijving, but we'll output any number of input elements -->
      <xsl:param name="in"
                 select="."
                 as="element()*"/>
...
<xsl:value-of select="$herhaalperiode/@value"/>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template name="makeMBHwithTamveSeparatedByProduct">
      <!--
            Normalises MBH-Id:
            1. Old MBH-ID structure: TA/MVE below medicamenteuze_behandeling.
            2. New MBH-ID structure: TA/MVE as top-level bouwstenen with medicamenteuze_behandeling_id below the bouwsteen.           
        -->
      <xsl:param name="mbh"
                 as="element()*"/>
      <xsl:variable name="transaction"
                    as="element()?"
                    select="($mbh[self::beschikbaarstellen_medicatiegegevens], $mbh/ancestor-or-self::beschikbaarstellen_medicatiegegevens, $mbh/..[self::beschikbaarstellen_medicatiegegevens])[1]"/>
      <adaxml xmlns="">
         <data>
            <beschikbaarstellen_medicatiegegevens>
               <xsl:choose>
                  <!-- New MP9 layout: MBH id is a child of the individual building block. -->
                  <xsl:when test="$transaction/(toedieningsafspraak | medicatieverstrekking)[medicamenteuze_behandeling_id]">
                     <xsl:for-each-group select="$transaction/(toedieningsafspraak | medicatieverstrekking)[medicamenteuze_behandeling_id]"
                                         group-by="concat(medicamenteuze_behandeling_id/@root, '|', medicamenteuze_behandeling_id/@value, '|', ((.//farmaceutisch_product[@value])[1]/@value, '')[1])">
                        <xsl:variable name="currentGroup"
                                      select="current-group()"
                                      as="element()*"/>
                        <xsl:variable name="ta"
                                      select="$currentGroup[self::toedieningsafspraak]"
                                      as="element(toedieningsafspraak)*"/>
                        <xsl:variable name="mve"
                                      select="$currentGroup[self::medicatieverstrekking]"
                                      as="element(medicatieverstrekking)*"/>
                        <xsl:choose>
                           <xsl:when test="count($mve) gt 0">
                              <xsl:for-each select="$mve">
                                 <medicamenteuze_behandeling>
                                    <xsl:copy-of select="medicamenteuze_behandeling_id"/>
                                    <xsl:copy-of select="$ta[1]"/>
                                    <xsl:copy-of select="."/>
                                 </medicamenteuze_behandeling>
                              </xsl:for-each>
                           </xsl:when>
                           <xsl:otherwise>
                              <medicamenteuze_behandeling>
                                 <xsl:copy-of select="$currentGroup[1]/medicamenteuze_behandeling_id"/>
                                 <xsl:copy-of select="$ta[1]"/>
                              </medicamenteuze_behandeling>
                           </xsl:otherwise>
                        </xsl:choose>
                  ...
                  <!-- Old MP9 MHB-id structure: MBH is the wrapper around TA/MVE. -->
                  <xsl:otherwise>
                     <xsl:for-each select="$mbh[self::medicamenteuze_behandeling] | $transaction/medicamenteuze_behandeling">
                        <xsl:variable name="currentMbh"
                                      select="."
                                      as="element(medicamenteuze_behandeling)"/>
                        <xsl:for-each-group select="$currentMbh/(toedieningsafspraak | medicatieverstrekking)"
                                            group-by="((.//farmaceutisch_product[@value])[1]/@value, '')[1]">
                           <xsl:variable name="currentGroup"
                                         select="current-group()"
                                         as="element()*"/>
                           <xsl:variable name="ta"
                           ...
                           <xsl:choose>
                              <xsl:when test="count($mve) gt 0">
                                 <xsl:for-each select="$mve">
                                    <medicamenteuze_behandeling>
                                       <xsl:copy-of select="$currentMbh/(identificatie | medicamenteuze_behandeling_id)"/>
                                       <xsl:copy-of select="$ta[1]"/>
                                       <xsl:copy-of select="."/>
                                    </medicamenteuze_behandeling>
                                 </xsl:for-each>
               ...
               <xsl:copy-of select="$transaction/bouwstenen | $mbh/../bouwstenen"/>
            </beschikbaarstellen_medicatiegegevens>
         </data>
      </adaxml>
   </xsl:template>
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9026_20150318000000">
      ...
      <MedicationDispenseList>
         ...
         <!-- separate ta and mve bouwstenen by mbh product -->
         <xsl:variable name="tamveSeparatedByProduct"
                       as="element()*">
            <xsl:call-template name="makeMBHwithTamveSeparatedByProduct">
               <xsl:with-param name="mbh"
                               select="$mbh"/>
            </xsl:call-template>
         </xsl:variable>
         <!-- if needed we may need to separate based on identifier, really only possible when the TA/MVE were converted in the first place (thus when roundtripping with 6.12 -> ada -> 6.12) -->
         <xsl:variable name="tamveSeparatedByConvertedId"
                       as="element()*">
            <xsl:choose>
               <xsl:when test="$tamveSeparatedByProduct/data/beschikbaarstellen_medicatiegegevens/medicamenteuze_behandeling[count(toedieningsafspraak) gt 1 or count(medicatieverstrekking) gt 1]">
                  <!-- let's see if the ids are helpful -->
                  <xsl:for-each select="$tamveSeparatedByProduct/data/beschikbaarstellen_medicatiegegevens/medicamenteuze_behandeling">
                     <xsl:variable name="currentMbh"
                                   select="."/>
                     <!-- separate ta and mve bouwstenen by id -->
                     <xsl:call-template name="separateMbhOn612Id">
                        <xsl:with-param name="ta"
                                        select="toedieningsafspraak[identificatie/starts-with(@root, $concatOidTA)]"/>
                     </xsl:call-template>
                  </xsl:for-each>
               </xsl:when>
               <xsl:otherwise>
                  <xsl:sequence select="$tamveSeparatedByProduct"/>
               </xsl:otherwise>
            </xsl:choose>
            <!-- now let's add the stuff without farmaceutisch_product, should not happen, but garbage in garbage out -->
            <xsl:if test="$mbh/toedieningsafspraak[not(.//farmaceutisch_product[@value])][identificatie/starts-with(@root, $concatOidTA)]">
               <!-- separate ta and mve bouwstenen by id -->
               <xsl:call-template name="separateMbhOn612Id">
                  <xsl:with-param name="ta"
                                  select="$mbh/toedieningsafspraak[not(.//farmaceutisch_product[@value])][identificatie/starts-with(@root, $concatOidTA)]"/>
               </xsl:call-template>
            </xsl:if>
         </xsl:variable>
         <!-- medicamenteuze_behandeling is also added in this ada variable for the new MP-2104 dataset structure -->
         <xsl:for-each select="$tamveSeparatedByConvertedId/data/beschikbaarstellen_medicatiegegevens/medicamenteuze_behandeling[count(toedieningsafspraak) != 1 or count(medicatieverstrekking) != 1]">
            <xsl:variable name="message"
                          select="concat('Converting ada to 6.12 verstrekking. Encountered an MBH which does not have 1 TA and 1 MVE for the same product, we will skip those (found ', count(toedieningsafspraak), ' TA(''s) and ', count(medicatieverstrekking), ' MVE(''s)). This is not supported. MBH with id @extension: ', (identificatie | medicamenteuze_behandeling_id)/@value, ' (oid: ', (identificatie | medicamenteuze_behandeling_id)/@root, '). Product/@id in ada = ', farmaceutisch_product/@id, '.')"/>
            <xsl:call-template name="util:logMessage">
               <xsl:with-param name="level"
                               select="$logERROR"/>
            ...
            <xsl:if test="$logLevel = $logDEBUG">
               <!-- put a comment in the resulting message, to help when comparing instances when roundtripping -->
               <xsl:comment select="$message"/>
            </xsl:if>
         </xsl:for-each>
         <xsl:for-each select="$tamveSeparatedByConvertedId/data/beschikbaarstellen_medicatiegegevens/medicamenteuze_behandeling[count(toedieningsafspraak) = 1][count(medicatieverstrekking) = 1]">
            <component>
   ...
   <xsl:template name="separateMbhOn612Id">
      <xsl:param name="ta"
                 as="element(toedieningsafspraak)*">
         <!-- the ada toedieningsafspra(a)k(en) to be handled -->
      </xsl:param>
      <xsl:variable name="patternConcatOid"
                    select="replace($concatOidTA, '\.', '\\.')"/>
      <xsl:if test="count($ta) gt 0">
         <adaxml xmlns="">
            <data>
               <beschikbaarstellen_medicatiegegevens>
                  <xsl:for-each select="$ta[identificatie/starts-with(@root, $concatOidTA)]">
                     <!-- start with mbh -->
                     <medicamenteuze_behandeling>
                        <xsl:copy-of select="../identificatie | medicamenteuze_behandeling_id"/>
                        <!-- find the corresponding medicatieverstrekking -->
                        <xsl:variable name="mveRoot"
                                      select="replace(identificatie/@root, $patternConcatOid, '')"/>
                        <xsl:variable name="correspondingMVE">
                           <xsl:sequence select="../medicatieverstrekking[identificatie[@root = $mveRoot][@value = current()/identificatie/@value]]"/>
                        </xsl:variable>
                        <xsl:copy-of select=". | $correspondingMVE"/>
                     </medicamenteuze_behandeling>
                  </xsl:for-each>
                  <!-- add the bouwstenen -->
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.100_20130521000000">
      ...
      <xsl:for-each select="$adaBouwsteen/gebruiksinstructie/doseerinstructie/dosering">
         <xsl:sort select="xs:int(../volgnummer/@value)"
                   data-type="number"/>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.110_20130521000000">
      <xsl:param name="mbhCombi4mve"
                 select="."
                 as="element()*">
         <!-- mbh which should contain at least one of ta/mve but at the same time max 1 ta and max 1 mve. This way we make a 6.12 medicationDispenseEvent -->
      </xsl:param>
      <xsl:for-each select="$mbhCombi4mve">
         <!-- Template  Medication Dispense Event -->
         <medicationDispenseEvent>
            <xsl:for-each select="(toedieningsafspraak, medicatieverstrekking)[1]">
               <xsl:for-each select="identificatie">
                  <xsl:call-template name="makeIIValue">
                     <xsl:with-param name="elemName">id</xsl:with-param>
                     <xsl:with-param name="root"
               ...
               <xsl:for-each select="toedieningsafspraak_datum_tijd | registratie_datum_tijd | medicatieverstrekkings_datum_tijd">
                  <xsl:call-template name="makeTSValue">
            ...
            <xsl:choose>
               <xsl:when test="count(medicatieverstrekking) = 1 and medicatieverstrekking/verstrekte_hoeveelheid/aantal[@value]">
                  <quantity>
                     <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9164_20170118000000_2">
                        <xsl:with-param name="Gstd_value"
                                        select="medicatieverstrekking/verstrekte_hoeveelheid/aantal/@value"/>
                        <xsl:with-param name="Gstd_unit"
                                        select="medicatieverstrekking/verstrekte_hoeveelheid/eenheid"/>
                     </xsl:call-template>
                  </quantity>
               </xsl:when>
               <xsl:otherwise>
            ...
            <!-- verbruiksduur -->
            <xsl:for-each select="medicatieverstrekking/verbruiksduur[.//(@value | @unit)]">
               <expectedUseTime>
                  <width value="{@value}"
                         unit="{nf:convertTime_ADA_unit2UCUM(@unit)}"/>
               </expectedUseTime>
            </xsl:for-each>
            <performer>
               <assignedPerson>
            ...
            <product>
               <dispensedMedication>
                  <xsl:for-each select="ancestor::*/bouwstenen/farmaceutisch_product[@id = current()/(toedieningsafspraak, medicatieverstrekking)[1]/*/farmaceutisch_product/@value]">
                     <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.106_20130521000000"/>
                  </xsl:for-each>
                  <xsl:for-each select="toedieningsafspraak/relatie_medicatieafspraak">
            ...
            <xsl:variable name="theAdaPerformer"
               ...
               <xsl:choose>
                  <xsl:when test="(toedieningsafspraak, medicatieverstrekking)[1]/verstrekker//zorgaanbieder[not(zorgaanbieder)][@value][not(*)]">
                     <xsl:sequence select="(ancestor::*/bouwstenen/zorgaanbieder[@id = current()/(toedieningsafspraak, medicatieverstrekking)[1]/verstrekker//zorgaanbieder[not(zorgaanbieder)]/@value])[1]"/>
                  </xsl:when>
                  <xsl:otherwise>
                     <xsl:sequence select="(current()/(toedieningsafspraak, medicatieverstrekking)[1]/verstrekker//zorgaanbieder[not(zorgaanbieder)])[1]"/>
                  </xsl:otherwise>
               </xsl:choose>
            </xsl:variable>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9019_20130521000000">
      ...
      <!-- Item(s) :: begindatum -->
      <xsl:for-each select="$begindatum">
         <xsl:call-template name="makeTSValue">
            <xsl:with-param name="inputValue"
                            select="."/>
            <xsl:with-param name="inputNullFlavor"/>
      ...
      <xsl:choose>
         <xsl:when test="count($doseerinstructies) gt 1">
            <xsl:variable name="current-volgnummer"
                          select="xs:int($theDosering/../volgnummer/@value)"
                          as="xs:int"/>
            <xsl:variable name="offset-in-days">
              ...
              uur moet daarom uitgesloten zijn in de aanroepende template(s)! -->
               <xsl:variable name="weeks2days"
                             select="sum($doseerinstructies[xs:int(volgnummer/@value) lt $current-volgnummer]/doseerduur[@unit = $ada-unit-week]/@value) * 7"/>
               <xsl:variable name="years2days"
                             select="sum($doseerinstructies[xs:int(volgnummer/@value) lt $current-volgnummer]/doseerduur[@unit = $ada-unit-year]/@value) * 365"/>
               <xsl:value-of select="sum($doseerinstructies[xs:int(volgnummer/@value) lt $current-volgnummer]/doseerduur[@unit = $ada-unit-day]/@value) + $weeks2days + $years2days"/>
            </xsl:variable>
