      ...
      <xsl:for-each select="$in[*[not(self::f:boundsDuration | self::f:duration | self::f:durationUnit | self::modifierExtension[@url = $urlExtRepeatPeriodCyclical])]]">
         ...
         <toedieningsschema>
            ...
            <adaextension>
               ...
               <!-- MP-2299: HL7v3 instances also have the repeat period cyclical schedule (modifierExtension) in FHIR Timing, but FHIR does not. 
                        We don't want to copy that into adaextension, because it is already in an ada element. We only want repeat -->
               <effectiveTime xsi:type="Timing"
                              xmlns="http://hl7.org/fhir">
                  <xsl:sequence select="../f:repeat"/>
               </effectiveTime>
            </adaextension>
         </toedieningsschema>
      </xsl:for-each>
