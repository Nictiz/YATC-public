...
<!-- == Provenance: YATC-internal/fhir-2-ada-r4/env/mp/9.3.0/payload/2.0.0-rc.4/mp-InstructionsForUse.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <!--
    ...
    -->
   <!-- ================================================================== -->
   <xsl:import href="../../../../../common/includes/mp-fhir2ada.xsl"/>
   ...
   <!-- MP-2045 additional as needed criteria in extension -->
   <xsl:variable name="extAsneeded"
                 select="'http://nictiz.nl/fhir/StructureDefinition/ext-Dosage-AsNeededFor'"/>
                  ...
                  <xsl:element name="toedieningsweg">
                     <xsl:call-template name="Coding-to-code">
                        <xsl:with-param name="in"
                                        as="element()">
                           <f:coding>
                              <f:system value="{$oidMap[@oid = $oidHL7NullFlavor]/@uri}"/>
                              <f:code value="NI"/>
      ...
      <xsl:if test="not(preceding-sibling::*[self::f:dosageInstruction] or preceding-sibling::*[self::f:dosage])">
         <gebruiksinstructie>
            ...
            <xsl:apply-templates select="parent::f:*/f:modifierExtension[@url = $ext-InstructionsForUse-RepeatPeriodCyclicalSchedule]"
                                 mode="#current"/>
            <!-- MP-1024, group the dosage(Instruction)s with same sequence number and boundsduration -->
            <xsl:for-each-group select="(. | following-sibling::*[self::f:dosageInstruction or self::f:dosage])"
                                group-by="concat(f:sequence/@value, f:timing/f:repeat/f:boundsDuration/concat(f:value/@value, f:code/@value, f:system/@value))">
               <xsl:if test="f:sequence | f:asNeededCodeableConcept | f:extension[@url = $extAsneeded] | f:doseAndRate | f:timing | f:asNeededCodeableConcept | f:maxDosePerPeriod">
                  <!-- doseerinstructie -->
                  <doseerinstructie>
                     <!-- doseerduur -->
                     <xsl:apply-templates select="current-group()[1]/f:timing/f:repeat/f:boundsDuration"
                                          mode="#current"/>
                     <!-- volgnummer -->
                     <xsl:apply-templates select="current-group()[1]/f:sequence"
                                          mode="#current"/>
                     <!-- dosering -->
                     <xsl:for-each select="current-group()">
                        <dosering>
                           ...
                           <xsl:apply-templates select="f:timing"
                                                mode="#current"/>
                           <!-- zo_nodig -->
                           <xsl:if test="f:extension[@url = $extAsneeded] | f:asNeededCodeableConcept | f:maxDosePerPeriod">
                              <zo_nodig>
                                 <!-- criterium -->
                                 <xsl:apply-templates select="f:asNeededCodeableConcept"
                                                      mode="#current"/>
                                 <!-- MP-2045 additional as needed criteria in extension -->
                                 <xsl:apply-templates select="f:extension[@url = $extAsneeded]/f:valueCodeableConcept"
                                                      mode="#current"/>
                                 <!-- maximale_dosering -->
   ...
   <xsl:template match="f:repeat"
      ...
      <!-- this is a shared template with HL7v3 so hl7_2_ada -->
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="f:asNeededCodeableConcept | f:extension[@url = $extAsneeded]/f:valueCodeableConcept"
                 mode="mp-InstructionsForUse">
      <!-- Template to convert f:asNeededCodeableConcept or the equivalent extension to criterium with code -->
      <xsl:call-template name="CodeableConcept-to-code">
   ...
   <xsl:template match="f:maxDosePerPeriod"
      ...
      <maximale_dosering>
         ...
         <xsl:for-each select="f:numerator/f:extension[@url = $ext-iso21090-PQ-translation]/f:valueQuantity[contains(f:system/@value, $oidGStandaardBST902THES2)]">
            <eenheid code="{f:code/@value}"
                     displayName="{f:unit/@value}"
                     codeSystem="{$oidGStandaardBST902THES2}"
                     codeSystemName="{$oidMap[@oid=$oidGStandaardBST902THES2]/@displayName}"/>
   ...
   <xsl:template match="f:rateRange"
      ...
      <!-- MP-1367 / ZIB-815 tijdseenheid no longer a part of toedieningssnelheid from MP9.3 beta.4 and eenheid is NOT in Gstd anymore but UCUM -->
