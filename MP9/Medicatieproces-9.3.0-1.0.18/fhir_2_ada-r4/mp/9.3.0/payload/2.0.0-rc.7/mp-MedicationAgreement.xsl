...
<!-- == Provenance: YATC-internal/fhir-2-ada-r4/env/mp/9.3.0/payload/2.0.0-rc.7/mp-MedicationAgreement.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:template match="f:MedicationRequest"
      ...
      <medicatieafspraak>
         <!-- MP-2104 .id toevoegen aan medicatieafspraak wanneer het om voorstelgegevens gaat -->
         <xsl:if test=".[f:intent/@value = 'plan']">
         ...
         <xsl:apply-templates select=".[f:intent/@value = 'order']/f:identifier"
                              mode="#current"/>
         <!-- medicamenteuze_behandeling_id MP-2104-->
         <xsl:apply-templates select="f:extension[@url = $urlExtPharmaceuticalTreatmentIdentifier]"
                              mode="urlExtPharmaceuticalTreatmentIdentifier"/>
         ...
         <xsl:if test="f:extension[@url = ($urlExtTimeInterval-Period, $urlExtTimeIntervalPeriod)] or f:extension[@url = ($urlExtTimeInterval-Duration, $urlExtTimeIntervalDuration)]">
            <gebruiksperiode>
               ...
               <xsl:apply-templates select="f:extension[@url = ($urlExtTimeInterval-Period, $urlExtTimeIntervalPeriod)]/f:valuePeriod/f:extension[@url = ($urlExtTimeInterval-Duration, $urlExtTimeIntervalDuration)]"
                                    mode="urlExtTimeInterval-Duration"/>
               <!-- criterium -->
               <!-- MP-1668 extension ext-MedicationAgreementPeriodOfUseCondition has been replaced by a more generic extension extPeriodOfUseCondition from MP9.3 beta.5 
                          to ensure backwards compatibility the former extension is kept in the stylesheet -->
               <!-- up to MP9.3 beta.5 -->
               ...
               <!-- from MP9.3 beta.5 onwards -->
               <xsl:apply-templates select="f:extension[@url = ($urlExtTimeInterval-Period, $urlExtTimeIntervalPeriod)]/f:valuePeriod/f:extension[@url = $urlExtPeriodOfUseCondition]"
                                    mode="urlExtPeriodOfUseCondition"/>
            </gebruiksperiode>
         </xsl:if>
         <!-- relatie_contact -->
   ...
   <xsl:template match="f:extension[@url = $urlExtMedicationAgreementNextPractitioner]"
      ...
      <!-- MP-1935 add support for zorgaanbieder in volgende_behandelaar -->
      <xsl:variable name="nextPractitionerResource"
                    select="nf:resolveRefInBundle(f:valueReference)"
                    as="element(*)"/>
      <xsl:variable name="adaElementName">
         <xsl:choose>
            <xsl:when test="$nextPractitionerResource[f:Organization or f:Location]">
               <xsl:value-of select="'zorgaanbieder'"/>
            </xsl:when>
            <xsl:otherwise>
               <xsl:value-of select="'zorgverlener'"/>
            </xsl:otherwise>
         </xsl:choose>
      </xsl:variable>
      <volgende_behandelaar>
         <xsl:element name="{$adaElementName}">
            <xsl:attribute name="value"
                           select="nf:process-reference-2NCName(f:valueReference/f:reference/@value, ancestor::f:entry/f:fullUrl/@value)"/>
   ...
   <xsl:template match="f:extension[@url = $urlExtMedicationAgreementRelationMedicationUse]"
                 mode="mp-MedicationAgreement">
      <!-- Template to convert f:extension/relatedMedicationUse to relatie_medicatiegebruik element. -->
   ...
   <xsl:template match="f:modifierExtension[@url = $urlExtStoptype]"
                 mode="mp-MedicationAgreement">
      <!-- Template to resolve this f:modifierExtension to medicatieafspraak_stop_type -->
