...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:template match="/">
      ...
      <xsl:variable name="verstrekkingslijst-612"
                    select="//hl7:MedicationDispenseList"/>
      <xsl:choose>
         ...
         <xsl:otherwise>
            <beschikbaarstellen_medicatiegegevens app="mp-{$mpVersion}"
...
<xsl:value-of select="concat('root: ', /hl7:QURX_IN990113NL/hl7:id/@root, ' and extension: ', /hl7:QURX_IN990113NL/hl7:id/@extension)"/>.</xsl:comment>
            ...
            <data>
               <xsl:for-each select="$dispense-lists">
                  ...
                  <beschikbaarstellen_medicatiegegevens app="mp-mp93"
                     ...
                     <xsl:if test="$outputMP9Bouwstenen = 'TA'">
                        <xsl:for-each select="$dispense-events">
                           <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.110_20130521000000_2_MP93">
                              ...
                              <xsl:with-param name="bouwsteen4output"
                                              select="'TA'"/>
                           </xsl:call-template>
                        </xsl:for-each>
                     </xsl:if>
                     <xsl:if test="$outputMP9Bouwstenen = 'MVE'">
                        <xsl:for-each select="$dispense-events">
                           <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.110_20130521000000_2_MP93">
                              <xsl:with-param name="current-dispense-event"
                                              select="."/>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.110_20130521000000_2_MP93">
      ...
      <xsl:param name="bouwsteen4output"
                 as="xs:string*"
                 select="$outputMP9Bouwstenen">
         <!-- param to influence which bouwsteen to output, override of global param because we need to influence the order to obey the dataset structure -->
      </xsl:param>
      <xsl:variable name="mbhId"
                    as="element(medicamenteuze_behandeling_id)">
      ...
      <xsl:variable name="HPK"
                    select="(hl7:product/hl7:dispensedMedication/hl7:MedicationKind/(hl7:code | hl7:code/hl7:translation)[@codeSystem = $oidGStandaardHPK][@code])[1]"/>
      <xsl:if test="$bouwsteen4output = 'TA'">
      ...
      <xsl:if test="$bouwsteen4output = 'MVE'">
         <xsl:call-template name="mp9-medicatieverstrekking-from-mp612-MP93">
            <xsl:with-param name="current-dispense-event"
                            select="$current-dispense-event"/>
            <xsl:with-param name="transaction"
                            select="$transaction"/>
   ...
   <xsl:template name="mp9-toedieningsafspraak-from-mp612-MP93"
      ...
      <xsl:variable name="effectiveTimes-eenmalig"
                    select="$mar-sorted/hl7:effectiveTime[not(@xsi:type) or replace(xs:string(@xsi:type), '(.*:)?(.+)', '$2') = 'TS']"/>
      <!-- variable that contains all IVL_TS in all of the medicationAdministrationRequest's -->
      <xsl:variable name="IVL_TS"
                    as="element()*"
                    select="$mar-sorted//(hl7:effectiveTime | hl7:comp)[replace(xs:string(@xsi:type), '(.*:)?(.+)', '$2') = 'IVL_TS']"/>
      <!-- toedieningsafspraak -->
         ...
         <toedieningsafspraak>
            ...
            <xsl:if test="$transaction = 'beschikbaarstellen_medicatiegegevens'">
               ...
               <xsl:for-each select="hl7:id[@extension]">
                  <identificatie root="{concat($concatOidTA, @root)}"
                                 value="{@extension}"/>
               </xsl:for-each>
            </xsl:if>
            <!-- MP-2104, MBH id now in all the bouwstenen -->
            ...
            <!-- afspraakdatum toedieningsafspraak_datum_tijd -->
            ...
            <!-- gebruiksperiode -->
            <xsl:variable name="inhoudGebruiksperiode"
                          as="element(gebruiksperiode)">
               <gebruiksperiode>
                  ...
                  <xsl:if test="count($mar-sorted) gt 0 and not($mar-sorted[not((.//hl7:effectiveTime | .//hl7:comp)[replace(xs:string(@xsi:type), '(.*:)?(.+)', '$2') = 'IVL_TS']/hl7:low/@value)])">
                     ...
                     <xsl:call-template name="handleTS">
                        <xsl:with-param name="in"
                                        select="$IVL_TS[1]/hl7:low"/>
                        <xsl:with-param name="elemName"
                                        select="'start_datum_tijd'"/>
                        <xsl:with-param name="datatype"
                                        select="'datetime'"/>
                     </xsl:call-template>
                  </xsl:if>
...
<xsl:value-of select="$current-dispense-event/hl7:id/@extension"/>. Not supported to convert this into structured information for gebruiksperiode-start</xsl:comment>
                  ...
                  <xsl:variable name="AllIVL_TSHaveWidth"
                                as="xs:boolean"
                                select="count($IVL_TS) gt 0 and (count($IVL_TS) = count($IVL_TS[hl7:width]))"/>
                  <xsl:variable name="comparedWidth"
                                as="element()*">
                     <xsl:for-each-group select="$IVL_TS"
                                         group-by="concat(hl7:width/@value, hl7:width/@unit)">
                        <xsl:sequence select="hl7:width"/>
                     </xsl:for-each-group>
                  </xsl:variable>
                  <xsl:variable name="AllorNoneIVL_TSHaveStartDate"
                                as="xs:boolean"
                                select="count($IVL_TS/hl7:low) = 0 or count($IVL_TS) = count($IVL_TS[hl7:low])"/>
                  <xsl:variable name="comparedStartDate"
                                as="element()*">
                     <xsl:for-each-group select="$IVL_TS"
                                         group-by="hl7:low/@value">
                  ...
                  <xsl:choose>
                     ...
                     <xsl:when test="count($IVL_TS[hl7:width]) gt 0">
                        ...
                        <xsl:if test="$AllIVL_TSHaveWidth and $AllorNoneIVL_TSHaveStartDate and count($comparedWidth) = 1 and count($comparedStartDate) lt 2">
                           <tijds_duur value="{$comparedWidth/@value}"
                                       unit="{nf:convertTime_UCUM2ADA_unit($comparedWidth/@unit)}"/>
                        </xsl:if>
                     </xsl:when>
                  </xsl:choose>
                  ...
                  <xsl:choose>
                     ...
                     <xsl:when test="count($mar-sorted) gt 0 and not($mar-sorted[not((.//hl7:effectiveTime | .//hl7:comp)[replace(xs:string(@xsi:type), '(.*:)?(.+)', '$2') = 'IVL_TS']/hl7:high/@value)])">
                        ...
                        <xsl:call-template name="handleTS">
                           <xsl:with-param name="in"
                                           select="$eind-datums[last()]"/>
                           <xsl:with-param name="elemName"
                                           select="'eind_datum_tijd'"/>
                           <xsl:with-param name="datatype"
                                           select="'datetime'"/>
                        </xsl:call-template>
                     </xsl:when>
                     ...
                     <xsl:when test="$current-dispense-event[count(.//hl7:medicationAdministrationRequest) gt 1] and not($mar-sorted[not((.//hl7:effectiveTime | .//hl7:comp)[replace(xs:string(@xsi:type), '(.*:)?(.+)', '$2') = 'IVL_TS']/(hl7:low/@value and hl7:width[@unit = 'd']/@value))])">
                        ...
                        <xsl:variable name="lastHl7Einddatum"
                                      select="nf:format2HL7Date(xs:string($einddatums-sorted[last()]), 'seconds')"/>
                        <gebruiksperiode_eind datatype="datetime"
                                              value="{nf:formatHL72VagueAdaDate($lastHl7Einddatum, nf:determine_date_precision($lastHl7Einddatum))}"/>
                     </xsl:when>
                  </xsl:choose>
               </gebruiksperiode>
            </xsl:variable>
            ...
            <!-- verstrekker -->
            <xsl:variable name="hl7Verstrekker"
                          as="element()*">
               <xsl:choose>
            ...
            <xsl:for-each select="$hl7Verstrekker[.//@*[normalize-space()] or .//text()[normalize-space()]]">
               <verstrekker>
                  <xsl:call-template name="mp92-zorgaanbieder">
                     <xsl:with-param name="hl7CurrentOrganization"
                                     select="."/>
                  </xsl:call-template>
               </verstrekker>
            </xsl:for-each>
            ...
            <xsl:apply-templates select="$current-dispense-event/hl7:product"
                                 mode="MP92">
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template name="mp9-medicatieverstrekking-from-mp612-MP93"
                 match="hl7:medicationDispenseEvent"
                 mode="MP93">
      <!-- Handle an hl7 dispenseEvent and create an ada toedieningsafspraak element. -->
      <xsl:param name="current-dispense-event"
      ...
      <xsl:for-each select="$current-dispense-event">
         <medicatieverstrekking>
            ...
            <xsl:if test="$transaction = 'beschikbaarstellen_medicatiegegevens'">
               <!-- identificatie -->
               <xsl:if test="$logLevel = $logDEBUG">
                  <xsl:comment>The medicatieverstrekking/id is the medicationDispenseEvent/id. root and extension copied.</xsl:comment>
               </xsl:if>
               <xsl:for-each select="hl7:id[@extension]">
            ...
            <!-- verbruiksduur -->
            <xsl:for-each select="hl7:expectedUseTime/hl7:width[@value | @unit]">
               <xsl:variable name="calcDuurInDagen"
                             select="nf:calculate_Duur_In_Dagen(@value, @unit)"/>
               <verbruiksduur value="{nf:calculate_Duur_In_Dagen(@value, @unit)}"
                              unit="{nf:convertTime_UCUM2ADA_unit('d')}">
                  <xsl:choose>
                     <xsl:when test="xs:decimal($calcDuurInDagen)">
                        <xsl:attribute name="value"
                                       select="$calcDuurInDagen"/>
                        <xsl:attribute name="unit"
                                       select="$ada-unit-day[1]"/>
                     </xsl:when>
                     <xsl:otherwise>
                        <!-- let's output whatever we received -->
                        <xsl:attribute name="value"
                                       select="@value"/>
                        <xsl:attribute name="unit"
                                       select="@unit"/>
            ...
            <!-- afleverlocatie, distributievorm, aanvullende informatie, toelichting 
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template name="mp9-verstrekteHoeveelheid">
      <!-- Creates an ada verstrekte_hoeveelheid element based on a HL7 input. -->
      <xsl:param name="hl7-quantity">
         <!-- the HL7 quantity element -->
      </xsl:param>
      <xsl:if test="$logLevel = $logDEBUG">
         <xsl:comment>mp9-verstrekte-hoeveelheid</xsl:comment>
      </xsl:if>
