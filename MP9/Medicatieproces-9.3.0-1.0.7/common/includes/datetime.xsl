...
<xsl:value-of select="$timePart"/>"</xsl:with-param>
   ...
   <xsl:function name="nf:determine_date_precision_from_ada_datetime"
      ...
      <xsl:for-each select="string-length($in) gt 0">
         <xsl:variable name="regExXmlDateTime"
                       select="'(.+?)(Z|(\+|-)((0[0-9]|1[0-3]):[0-5][0-9]|14:00))$'"/>
         <xsl:variable name="inputWithoutTimeZone"
                       select="replace($in, $regExXmlDateTime, '$1')"
                       as="xs:string?"/>
         <xsl:choose>
            ...
            <xsl:when test="string-length($inputWithoutTimeZone) = 4">YEAR</xsl:when>
            <xsl:when test="string-length($inputWithoutTimeZone) = 7">MONTH</xsl:when>
            <xsl:when test="string-length($inputWithoutTimeZone) = 10">DAY</xsl:when>
            <xsl:when test="string-length($inputWithoutTimeZone) = 13">HOUR</xsl:when>
            <xsl:when test="string-length($inputWithoutTimeZone) = 16">MINUTE</xsl:when>
            <xsl:when test="string-length($inputWithoutTimeZone) = 19">SECOND</xsl:when>
            <xsl:when test="string-length($inputWithoutTimeZone) gt 19">MILLISECOND</xsl:when>
         </xsl:choose>
      </xsl:for-each>
   </xsl:function>
</xsl:stylesheet>