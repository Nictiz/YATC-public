...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <!--
    ...
    -->
   <!-- ================================================================== -->
   <xsl:import href="fhir_2_ada_fhir_include-d620e247.xsl"/>
   ...
   <xsl:template name="Reference-to-identificatie">
      ...
      <xsl:param name="resourceList"
                 as="xs:string*">
         <!-- Xpath sequence of the resource names as strings that are allowed when resolving the reference (e.g. "('MedicationUse','MedicationStatement'). When not given, no filtering on resourcename is applied. -->
      </xsl:param>
      <xsl:choose>
         <xsl:when test="f:reference">
            <xsl:variable name="nonFilteredResource"
                          select="ancestor::f:Bundle/f:entry[f:fullUrl/@value = current()/f:reference/@value]/f:resource"/>
               ...
               <xsl:when test="$resource/f:*/f:identifier">
                  <xsl:call-template name="Identifier-to-identificatie">
                     <xsl:with-param name="in"
                                     select="$resource/f:*/f:identifier"/>
                  </xsl:call-template>
               </xsl:when>
               <xsl:otherwise>
   ...
   <xsl:template name="GstdQuantity2ada">
      <!-- Template to convert FHIR Gstandaard (Simple)Quantity to ada waarde and eenheid elements (no range supported) -->
      <xsl:param name="in"
                 as="element()?"
                 select=".">
         <!-- FHIR element of type Gstandaard (Simple)Quantity -->
      </xsl:param>
      <xsl:param name="adaElementNameWaarde"
                 as="xs:string"
                 select="'aantal'">
         <!-- The ada element name for value. Defaults to aantal. -->
      </xsl:param>
      <xsl:param name="adaElementNameEenheid"
                 as="xs:string"
                 select="'eenheid'">
         <!-- The ada element name for unit. Defaults to eenheid. -->
      </xsl:param>
         ...
         <xsl:for-each select="f:extension[@url = $urlExtIso21090PQtranslation]/f:valueQuantity[contains(f:system/@value, $oidGStandaardBST902THES2)]">
            ...
            <xsl:element name="{$adaElementNameEenheid}">
               ...
               <xsl:attribute name="codeSystemName"
                              select="$oidMap[@oid = $oidGStandaardBST902THES2]/@displayName"/>
            </xsl:element>
         </xsl:for-each>
      </xsl:for-each>
   </xsl:template>
   ...
   <xsl:function name="nf:replaceChar">
      ...
      <xsl:variable name="pelString"
                    select="subsequence($charStringInAndtestChar, 1, 1)"/>
      <xsl:variable name="testChar"
                    select="substring($pelString, 1, 1)"/>
      <xsl:variable name="finalString"
                    select="subsequence($charStringInAndtestChar, 2, 1)"/>
      ...
      <xsl:choose>
         ...
         <xsl:when test="not(matches($testChar, $NCNameChars)) or ($testChar eq ':')">
            ...
            <xsl:variable name="testCharEscaped"
                          as="xs:string">
               <xsl:choose>
   ...
   <xsl:function name="nf:convert2NCName">
      ...
      <xsl:variable name="stringOut"
                    select="replace($stringIn, '^([\d_])(.*)', '_$2')"/>
      <xsl:variable name="parm"
                    select="($stringOut, $stringOut)"/>
      <xsl:value-of select="nf:replaceChar($parm)"/>
   </xsl:function>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:function name="nf:resolveRefInBundle"
                 as="element(f:resource)*">
      <!-- Find the resource for a given Reference in the same Bundle this Reference is in. Tries to find it with reference, when not succesful with identifier. -->
      <xsl:param name="in"
                 as="element()?">
         <!-- The FHIR reference to resolve in it's own Bundle. Optional, no output if no input. -->
      </xsl:param>
      <xsl:for-each select="$in">
         <!-- find based on reference -->
         <xsl:variable name="referenceValue"
                       select="nf:process-reference(current()/f:reference/@value, ancestor::f:entry/f:fullUrl/@value)"/>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:function name="nf:process-reference-2NCName"
                 as="xs:string">
      <!--  
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:function name="nf:process-reference"
                 as="xs:string">
      <!--  
            A function which processes the reference:
             - it checks whether the referenceUrl is a relative url and converts it to an absolute url according to the R4 FHIR Documentation
            ...
            <xsl:choose>
               ...
               <xsl:when test="matches($fullUrl, $restRegexR4)">
                  <xsl:variable name="baseUrl"
                                select="replace($fullUrl, $restRegexR4, '$1')"/>
