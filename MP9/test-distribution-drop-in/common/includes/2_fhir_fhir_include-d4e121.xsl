...
<?yatc-distribution-provenance href="C:/Data/Erik/work/Nictiz/new/HL7-mappings/ada_2_fhir-r4/fhir/2_fhir_fhir_include.xsl"?>
<?yatc-distribution-info name="VZVZ-test-distribution-drop-in" timestamp="2024-01-25T12:13:11.57+01:00" version="0.2"?>
<!-- == Provenance: C:/Data/Erik/work/Nictiz/new/HL7-mappings/ada_2_fhir-r4/fhir/2_fhir_fhir_include.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:import href="2_fhir_fhir_include-d4e64.xsl"/>
   <xd:doc>
      <xd:desc>
   ...
   <xsl:param name="populateId"
              ...
              as="xs:boolean"/>
   <xd:doc>
      <xd:desc>$referencingStrategy will be filled with one of the following values: 
<xd:ul>
   ...
   <xsl:variable name="ada2resourceType">
      ...
      <nm:map ada="overgevoeligheid"
              resource="AllergyIntolerance"
              profile="cio-HypersensitivityDispositionReaction"/>
      <nm:map ada="overgevoeligheid"
              resource="Condition"
              profile="cio-HypersensitivityDisposition"/>
...
<xd:ul>
            ...
            <xd:li>nm:logical-id: the logical id for the FHIR instance, if one is needed (see the 
<xd:ref name="populateId"
                       type="parameter"/>populateId and 
<xd:ref name="referencingStrategy"
                       type="parameter"/> parameters). If a logical id is not given by ADA, it will be generated using the best matching template with the _generateId mode.</xd:li>
            <xd:li>nm:full-url: the fullUrl for the resource, based on what is needed for the current referencing strategy (see the 
<xd:ref name="referencingStrategy"
                       type="parameter"/> parameter).</xd:li>
            <xd:li>nm:ref-url: the url to reference the resource, based on what is needed for the current referencing strategy (see the 
<xd:ref name="referencingStrategy"
   ...
   <xsl:template name="buildFhirMetadata">
      ...
      <xsl:for-each-group select="$in[self::zorgverlener[.//(@value | @code | @nullFlavor)]]"
                          group-by="                 concat(nf:ada-healthprofessional-id(zorgverlener_identificatienummer)/@root,                 nf:ada-healthprofessional-id(zorgverlener_identificatienummer)/normalize-space(@value))">
      ...
      <xsl:for-each-group select="$in[self::zorgaanbieder[.//(@value | @code | @nullFlavor)]]"
                          group-by="                 concat(nf:ada-healthprovider-id(zorgaanbieder_identificatienummer)/@root,                 nf:ada-healthprovider-id(zorgaanbieder_identificatienummer)/normalize-space(@value))">
         <xsl:for-each-group select="current-group()"
                             group-by="nf:getGroupingKeyDefault(.)">
            <xsl:call-template name="_buildFhirMetadataForAdaEntry">
               <xsl:with-param name="partNumber"
...
<xsl:value-of select="local-name()"/>
   ...
   <xd:doc>
      <xd:desc>Helper template to build the FHIR metadata for a single ADA instance. See the documentation on 
<xd:ref name="buildFhirMetadata"
                 type="template"/> for more information.</xd:desc>
...
<xsl:value-of select="$groupKey"/>
   ...
   <xd:doc>
      <xd:desc>Insert the FHIR resource id element for the specified ADA instance, if an id is required (see the 
<xd:ref name="populateId"
                 type="parameter"/> and 
<xd:ref name="referencingStrategy"
   ...
   <xd:doc>
      <xd:desc>Get the FHIR resource id element for the specified ADA instance, if an id is available (see the 
<xd:ref name="populateId"
                 type="parameter"/> and 
<xd:ref name="referencingStrategy"
                 type="parameter"/> parameters).
...
<xsl:value-of select="$groupKey"/>
   ...
   <xd:doc>
      <xd:desc>Insert the FHIR fullUrl element for the specified instance, if one is available (see the 
<xd:ref name="referencingStrategy"
                 type="parameter"/> parameter).</xd:desc>
      <xd:param name="in">The ADA instance that the FHIR resource is generated for.</xd:param>
      <xd:param name="profile">The id of the profile that is being generated from the specified ADA instance. This is needed to specify which profile is targeted when a single ADA instance results is mapped onto multiple FHIR profiles. It may be omitted otherwise.</xd:param>
   </xd:doc>
