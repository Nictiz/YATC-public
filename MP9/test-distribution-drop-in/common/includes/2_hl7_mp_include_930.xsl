...
<?yatc-distribution-provenance href="C:/Data/Erik/work/Nictiz/new/HL7-mappings/ada_2_hl7/mp/9.3.0/2_hl7_mp_include_930.xsl"?>
<?yatc-distribution-info name="VZVZ-test-distribution-drop-in" timestamp="2024-01-25T12:13:11.57+01:00" version="0.2"?>
<!-- == Provenance: C:/Data/Erik/work/Nictiz/new/HL7-mappings/ada_2_hl7/mp/9.3.0/2_hl7_mp_include_930.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:import href="ada2hl7_all-zibs-d4e68.xsl"/>
      ...
      <xsl:for-each select="$in">
         <substanceAdministration classCode="SBADM"
            ...
            <xsl:variable name="wdsDatumTijd"
                          select="wisselend_doseerschema_datum_tijd"/>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9415_20221121070356"
      ...
      <xsl:for-each select="$in">
         ...
         <!-- Verstrekker -->
         <xsl:variable name="theAfspraakdatum"
                       select="(afspraak_datum_tijd | afspraakdatum | toedieningsafspraak_datum_tijd)[@value | @nullFlavor]"/>
            ...
            <author>
               <xsl:call-template name="makeTSValue">
                  ...
                  <xsl:with-param name="inputValue"
                                  select="$theAfspraakdatum/@value"/>
                  <xsl:with-param name="inputNullFlavor"
                                  select="$theAfspraakdatum/@nullFlavor"/>
               </xsl:call-template>
               <assignedAuthor>
                  <id nullFlavor="NI"/>
                  <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.121.10.33_20210701000000"/>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9429_20221122130054">
      ...
      <xsl:for-each select="$in">
         ...
         <xsl:variable name="maAfspraakDatumTijd"
                       select="afspraak_datum_tijd | afspraakdatum | medicatieafspraak_datum_tijd"
                       as="element()*"/>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9442_20221124134142"
      ...
      <xsl:for-each select="$in">
         ...
         <consumable>
            <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9363_20210602155855">
               <xsl:with-param name="product"
                               select="../../bouwstenen/farmaceutisch_product[@id = current()/gebruiksproduct/farmaceutisch_product/@value]"/>
            </xsl:call-template>
         </consumable>
         <!-- author and time -->
         <xsl:call-template name="template_2.16.840.1.113883.2.4.3.11.60.20.77.10.9387_20210701000000">
