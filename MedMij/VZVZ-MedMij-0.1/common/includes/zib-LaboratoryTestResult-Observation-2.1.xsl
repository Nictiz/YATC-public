...
<?yatc-distribution-provenance href="C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/zibs2017/payload/zib-LaboratoryTestResult-Observation-2.1.xsl"?>
<?yatc-distribution-info name="VZVZ-MedMij" timestamp="2024-01-29T11:45:25.52+01:00" version="0.1"?>
<!-- == Provenance: C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/zibs2017/payload/zib-LaboratoryTestResult-Observation-2.1.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <!-- ============================================================================================== -->
   <xsl:variable name="labObservations"
                 as="element()*">
      <xsl:for-each-group select="//(laboratorium_test[not(laboratorium_test)] | laboratory_test[not(laboratory_test)])[not(@datatype = 'reference')][.//(@value | @code | @nullFlavor)]"
   ...
   <xsl:template name="laboratoryResultObservationReference"
      ...
      <xsl:variable name="theGroupElement"
                    select="$labObservations[group-key = $theGroupKey]"
                    as="element()?"/>
      <xsl:choose>
   ...
   <xsl:template name="laboratoryResultObservationEntry"
      ...
      <xsl:param name="adaPatient"
                 select="(ancestor::*//patient[not(patient)][*//@value] | ancestor::*/bundle/subject/patient[*//@value])[1]"
                 as="element()">
            ...
            <Observation>
                  ...
                  <xsl:choose>
                     <xsl:when test="(resultaat_status | result_status)[@code]">
                        <xsl:call-template name="code-to-code">
                           <xsl:with-param name="in"
                                           select="resultaat_status | result_status"/>
               ...
               <code>
                  <xsl:choose>
                     <xsl:when test="(onderzoek | panel_or_battery)[@code]">
                        <xsl:call-template name="code-to-CodeableConcept">
                           <xsl:with-param name="in"
                                           select="onderzoek | panel_or_battery"/>
                        </xsl:call-template>
                     </xsl:when>
                     <xsl:otherwise>
                        <extension url="{$urlExtHL7DataAbsentReason}">
               ...
               <xsl:if test="laboratory_test/test_date_time[@value]">
                  <xsl:variable name="maxValue"
                                select="max(laboratory_test/test_date_time/@value/number(replace(., '\D', '')))"/>
                  <effectiveDateTime>
                     <xsl:attribute name="value">
                        <xsl:call-template name="format2FHIRDate">
                           <xsl:with-param name="dateTime"
                                           select="xs:string((laboratory_test/test_date_time[number(replace(@value, '\D', '')) = $maxValue])[1]/@value)"/>
                           <xsl:with-param name="dateT"
                                           select="$dateT"/>
                        </xsl:call-template>
                     </xsl:attribute>
               ...
               <status>
                  <xsl:choose>
                     <xsl:when test="(test_uitslag_status | test_result_status)[@code]">
                        <xsl:call-template name="code-to-code">
                           <xsl:with-param name="in"
                                           select="test_uitslag_status | test_result_status"/>
                           <xsl:with-param name="codeMap"
                                           as="element()*">
                              <map inCode="pending"
                                   inCodeSystem="2.16.840.1.113883.2.4.3.11.60.40.4.16.1"
