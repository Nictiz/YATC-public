...
<?yatc-distribution-provenance href="C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/zibs2017/payload/zib-LaboratoryTestResult-Specimen-2.1.xsl"?>
<?yatc-distribution-info name="VZVZ-MedMij" timestamp="2024-01-29T11:45:25.52+01:00" version="0.1"?>
<!-- == Provenance: C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/zibs2017/payload/zib-LaboratoryTestResult-Specimen-2.1.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <!-- ============================================================================================== -->
   <xsl:variable name="labSpecimens"
                 as="element()*">
      <xsl:for-each-group select="//(laboratory_test_result/specimen | //laboratorium_uitslag/monster)[.//(@value | @code | @nullFlavor)]"
   ...
   <xsl:template name="laboratoryResultSpecimenReference"
      ...
      <xsl:variable name="theGroupElement"
                    select="$labSpecimens[group-key = $theGroupKey]"
                    as="element()?"/>
      <xsl:choose>
   ...
   <xsl:template name="laboratoryResultSpecimenEntry"
      ...
      <xsl:param name="adaPatient"
                 select="(ancestor::*/patient[*//@value], ancestor::*/bundle/subject/patient/patient[*//@value], ancestor::*/hcimroot/subject/patient/patient[*//@value], ancestor::*/bundle/subject/patient[*//@value], ancestor::*/hcimroot/subject/patient[*//@value])[1]"
                 as="element()">
   ...
   <xsl:template name="zib-LaboratoryTestResult-Specimen-2.1"
      ...
      <xsl:param name="adaPatient"
                 select="(ancestor::*/patient[*//@value], ancestor::*/bundle/subject/patient/patient[*//@value], ancestor::*/bundle/subject/patient[*//@value])[1]"
                 as="element()">
      ...
      <xsl:for-each select="$in">
         ...
         <xsl:variable name="containedSpecimenId"
                       select="generate-id(microorganism | microorganisme)"/>
         <xsl:variable name="containedDeviceId"
                       select="generate-id(specimen_source | bron_monster)"/>
         ...
         <xsl:variable name="receivedDateTime"
                       as="element()?">
                  ...
                  <xsl:attribute name="value">
                     <xsl:call-template name="format2FHIRDate">
                        <xsl:with-param name="dateTime"
                                        select="xs:string((received_date_time | aanname_datum_tijd)/@value)"/>
                        <xsl:with-param name="dateT"
                                        select="$dateT"/>
                     </xsl:call-template>
                  </xsl:attribute>
         ...
         <xsl:variable name="collectionDetails"
                       as="element()?">
               ...
               <collection>
                        ...
                        <collectedPeriod>
                           <xsl:call-template name="startend-to-Period">
                              ...
                              <xsl:with-param name="start"
                                              select="collection_period/start_date_time | verzamelperiode/start_datum_tijd"/>
                              <!-- EndDateTime -->
                              <xsl:with-param name="end"
                                              select="collection_period/end_date_time | verzamelperiode/eind_datum_tijd"/>
                     ...
                     <xsl:when test="collection_period[@nullFlavor | @value] | verzamelperiode[@nullFlavor | @value] | collection_date_time | afname_datum_tijd">
                        <collectedDateTime>
                           <xsl:attribute name="value">
                              <xsl:call-template name="format2FHIRDate">
                                 <xsl:with-param name="dateTime"
                                                 select="xs:string((collection_date_time | afname_datum_tijd)/@value)"/>
                                 <xsl:with-param name="dateT"
                                                 select="$dateT"/>
                              </xsl:call-template>
                           </xsl:attribute>
                  ...
                  <xsl:if test="collected_volume | verzamelvolume">
                     <quantity>
                        <xsl:call-template name="hoeveelheid-to-Quantity">
                           <xsl:with-param name="in"
                                           select="collected_volume | verzamelvolume"/>
                        </xsl:call-template>
                     </quantity>
                  </xsl:if>
                  ...
                  <xsl:if test="collection_method | afname_procedure">
                     <method>
                        <xsl:call-template name="code-to-CodeableConcept">
                           <xsl:with-param name="in"
                                           select="collection_method | afname_procedure"/>
                        </xsl:call-template>
                     </method>
                  </xsl:if>
                  ...
                  <xsl:if test="anatomical_location | anatomische_locatie">
                     <bodySite>
                        ...
                        <xsl:call-template name="code-to-CodeableConcept">
                           <xsl:with-param name="in"
                                           select="anatomical_location | anatomische_locatie"/>
                        </xsl:call-template>
                     </bodySite>
                  </xsl:if>
               </collection>
            ...
            <Specimen>
                     ...
                     <Specimen>
                        ...
                        <xsl:if test="specimen_material | monstermateriaal">
                           <type>
                              <xsl:call-template name="code-to-CodeableConcept">
                                 <xsl:with-param name="in"
                                                 select="specimen_material | monstermateriaal"/>
                              </xsl:call-template>
                           </type>
                        </xsl:if>
               ...
               <xsl:choose>
                  <xsl:when test="microorganism | microorganisme">
                     <type>
                        <xsl:call-template name="code-to-CodeableConcept">
                           <xsl:with-param name="in"
                                           select="microorganism | microorganisme"/>
                        </xsl:call-template>
                     </type>
                  </xsl:when>
                  <xsl:otherwise>
               ...
               <xsl:if test="specimen_number_extension | monstervolgnummer | container_type | containertype">
                  <container>
                     ...
                     <xsl:variable name="specimenNumber"
                                   select="specimen_id | monsternummer"
                                   as="element()?"/>
                     <xsl:for-each select="specimen_number_extension | monstervolgnummer">
                        <xsl:variable name="specimenNumberExtension"
                                      select="@value"/>
                        <identifier>
                           <xsl:call-template name="id-to-Identifier">
                              <xsl:with-param name="in"
                                 ...
                                 <xsl:copy copy-namespaces="no">
                                    ...
                                    <xsl:attribute name="value"
                                                   select="string-join(($specimenNumber/@value, $specimenNumberExtension), '-')"/>
                                 </xsl:copy>
                              </xsl:with-param>
                           </xsl:call-template>
                        </identifier>
                     ...
                     <xsl:if test="container_type | containertype">
                        <type>
                           <xsl:call-template name="code-to-CodeableConcept">
                              <xsl:with-param name="in"
                                              select="container_type | containertype"/>
                           </xsl:call-template>
                        </type>
                     </xsl:if>
                  </container>
