...
<?yatc-distribution-provenance href="C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/ketenzorg/2_fhir_ketenzorg_include.xsl"?>
<?yatc-distribution-info name="VZVZ-MedMij" timestamp="2024-02-06T09:13:30.86+01:00" version="0.2"?>
<!-- == Provenance: C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/ketenzorg/2_fhir_ketenzorg_include.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:param name="dojournalentries"
              as="xs:string"
              select="'SOEP'"/>
   <xsl:variable name="doJournalEntries"
                 as="xs:string*"
                 select="for $s in string-to-codepoints(upper-case($dojournalentries)) return codepoints-to-string($s)"/>
   <xsl:variable name="patient-ada"
                 select="/adaxml/data/*/bundle/subject/patient"/>
   ...
   <xsl:variable name="body-observations"
                 as="element()*">
      <!-- journaalRegels -->
      <xsl:for-each-group select="//journaalregel | //journal_entry"
         ...
         <unieke-observatie xmlns="">
            ...
            <xsl:apply-templates select="current-group()[1]"
                                 mode="doObservationEntry"/>
         </unieke-observatie>
      </xsl:for-each-group>
   </xsl:variable>
   <xsl:variable name="prescribe-reasons"
                 as="element()*"/>
   <xsl:variable name="bouwstenen"
            ...
            <resource>
               <xsl:call-template name="zib-AllergyIntolerance-2.1">
                  ...
                  <xsl:with-param name="adaPatient"
                                  select="(ancestor-or-self::*//subject//patient[patient_identification_number])[1]"
                                  as="element()"/>
               </xsl:call-template>
            </resource>
            <search>
      ...
      <xsl:for-each select="//*[bundle]/encounter_note">
         <xsl:variable name="journalEntries"
                       as="element()*">
            <xsl:for-each select="journal_entry[type/@code = $doJournalEntries]">
               <xsl:variable name="bodyGroupingKey"
                             select="nf:getGroupingKeyDefault(.)"/>
               <xsl:variable name="bodyGroupJournalEntry"
                             select="$body-observations[group-key = $bodyGroupingKey]"/>
         ...
         <entry>
            ...
            <resource>
               <xsl:call-template name="gp-EncounterReport">
                  ...
                  <xsl:with-param name="gp-JournalEntries"
   ...
   <xsl:variable name="theEncounters"
                 as="element(f:entry)*">
      <!-- Contactmomenten -->
      <xsl:choose>
         <xsl:when test="$matchResource = 'Composition'">
            <xsl:variable name="referencedEncounterIds"
                          as="xs:string*">
               <xsl:for-each select="//encounter_notes_response/encounter_note/encounter">
                  <xsl:value-of select="concat(@value, '-', @root)"/>
               </xsl:for-each>
            </xsl:variable>
            <xsl:for-each-group select="//*[bundle]/encounter"
                                group-by="((hcimroot/identification_number | identifier)/concat(@value, '-', @root))[1]">
               <xsl:variable name="encounterGroup"
                             select="current-group()"
                             as="element()+"/>
               <xsl:for-each select="current-group()[1]">
                  <xsl:variable name="encounterIds"
                                select="(hcimroot/identification_number | identifier)/concat(@value, '-', @root)"
                                as="xs:string*"/>
                     ...
                     <entry>
                        ...
                        <resource>
                           <xsl:call-template name="gp-Encounter">
                              <xsl:with-param name="encounterGroup"
                                              select="$encounterGroup"/>
                           </xsl:call-template>
                        </resource>
                        <search>
                           <mode value="include"/>
         ...
         <entry>
            ...
            <resource>
               <xsl:call-template name="nl-core-episodeofcare-2.0">
                  <xsl:with-param name="episodeofcare"
                                  select="."/>
                  <xsl:with-param name="custodian"
                                  select="../bundle/custodian"/>
                  <xsl:with-param name="author"
                                  select="../bundle/author"/>
               </xsl:call-template>
            </resource>
            <search>
                           ...
                           <xsl:otherwise>
                              <xsl:call-template name="util:logMessage">
                                 <xsl:with-param name="level"
                                                 select="$logWARN"/>
                                 <xsl:with-param name="msg">Unsupported EpisodeOfCare status code "
<xsl:value-of select="@code"/>"</xsl:with-param>
                              </xsl:call-template>
                           </xsl:otherwise>
                        </xsl:choose>
                     </xsl:attribute>
         ...
         <Flag>
            <xsl:choose>
               ...
               <xsl:otherwise>
                  <xsl:call-template name="id-to-id">
                     <xsl:with-param name="in"
                                     select="hcimroot/identification_number"/>
                  </xsl:call-template>
               </xsl:otherwise>
            </xsl:choose>
            <meta>
            ...
            <xsl:for-each select="episode">
               <xsl:variable name="theReference"
                             select="nf:getUriFromAdaId(., 'EpisodeOfCare', true())"/>
               <xsl:variable name="theEpisode"
                             select="$theEpisodes[f:fullUrl[ends-with(@value, $theReference)]]/f:resource/f:*"
                             as="element()*"/>
            ...
            <xsl:variable name="is_completed"
                          as="xs:boolean">
            ...
            <subject>
               <xsl:apply-templates select="../bundle/subject/patient"
                                    mode="doPatientReference-2.1"/>
            </subject>
            <xsl:if test="start_date_time | end_date_time[$is_completed]">
               <period>
                  <xsl:call-template name="startend-to-Period">
                     ...
                     <xsl:with-param name="end"
                                     select="end_date_time[$is_completed]"/>
                  </xsl:call-template>
               </period>
            </xsl:if>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template name="gp-Encounter"
                 as="element()">
         ...
         <Encounter>
               ...
               <xsl:choose>
                  ...
                  <xsl:when test="contact_type[@code = '07'][@codeSystem = $oidNHGTabel14Contactwijze]">
                     ...
                     <xsl:call-template name="code-to-Coding">
                        <xsl:with-param name="in"
                                        as="element()">
                           <contact_type code="OTH"
                                         codeSystem="{$oidHL7NullFlavor}"
                                         displayName="Other"
                                         originalText="{(@displayName, @code)[1]}"/>
                        </xsl:with-param>
                     </xsl:call-template>
                  </xsl:when>
                     ...
                     <xsl:call-template name="code-to-Coding">
                        <xsl:with-param name="in"
                           ...
                           <contact_type code="OTH"
                                         codeSystem="{$oidHL7NullFlavor}"
                                         displayName="Other"
                                         originalText="{(@displayName, 'herhaalrecept')[1]}"/>
                        </xsl:with-param>
                     </xsl:call-template>
                  </xsl:when>
            ...
            <xsl:for-each select="$encounterGroup/contact_reason/episode">
               <xsl:variable name="theValue"
                             select="@value"/>
               <xsl:variable name="theRoot"
                             select="local:getUri(@root)"/>
               <xsl:variable name="theReference"
                             select="$theEpisodes[f:resource/f:EpisodeOfCare/f:identifier[f:system/@value = $theRoot][f:value/@value = $theValue]]"
                             as="element(f:entry)*"/>
               <episodeOfCare>
                  <xsl:choose>
               ...
               <participant>
                  <xsl:if test="health_professional_role">
                     <type>
                        <xsl:call-template name="code-to-CodeableConcept">
                           <xsl:with-param name="in"
                                           select="health_professional_role"/>
                        </xsl:call-template>
                     </type>
                  </xsl:if>
                  <individual>
            ...
            <xsl:if test="contact_with/health_professional/healthcare_provider">
               <serviceProvider>
                  <xsl:apply-templates select="contact_with/health_professional/healthcare_provider"
                                       mode="doOrganizationReference-2.0"/>
               </serviceProvider>
            </xsl:if>
         </Encounter>
         ...
         <xsl:variable name="resource">
            <Observation>
               ...
               <xsl:for-each select="../encounter">
                  <xsl:variable name="theReference"
                                select="nf:getUriFromAdaId(., 'Encounter', true())"/>
                  <context>
                     <xsl:choose>
                  ...
                  <interpretation>
                     ...
                     <xsl:call-template name="code-to-CodeableConcept">
                        <xsl:with-param name="in"
                                        select="."/>
                        <xsl:with-param name="codeMap"
                                        select="$zibInterpretatieVlaggen_to_hl7v2Table0078AbnormalFlags/*"
                                        as="element()+"/>
                     </xsl:call-template>
                  </interpretation>
               </xsl:for-each>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template name="gp-JournalEntry"
                 match="journal_entry"
                 mode="gp-JournalEntry">
   ...
   <xsl:template name="gp-EncounterReport">
      ...
      <xsl:param name="subject"
                 select="../bundle/subject/patient"
                 as="element()*">
         <!-- Patient entry from the bundle -->
      </xsl:param>
      <xsl:param name="encounterreport-id"
                 as="xs:string?"/>
      <xsl:param name="gp-JournalEntries"
                 as="element()*">
         <!-- The journal entries to include as gp-JournalEntry instances -->
      </xsl:param>
      <xsl:variable name="episodeofcare"
                    select="episode"
                    as="element()*"/>
      <xsl:variable name="resource">
         <Composition>
            ...
            <date>
               <xsl:attribute name="value">
                  <xsl:call-template name="format2FHIRDate">
                     <xsl:with-param name="dateTime"
                                     select="hcimroot/date_time/@value"/>
                  </xsl:call-template>
               </xsl:attribute>
            </date>
            <xsl:for-each select="$author/health_professional">
            ...
            <xsl:for-each select="$gp-JournalEntries[descendant-or-self::f:Observation[1]]">
               <xsl:variable name="journalEntryObservation"
                             select="descendant-or-self::f:Observation[1]"/>
               <section>
                        ...
                        <xsl:choose>
                           ...
                           <xsl:when test="$journalEntryObservation/f:component/f:valueCodeableConcept">
                              <xsl:call-template name="doDT_CodeableConcept">
                                 <xsl:with-param name="in"
                                                 select="$journalEntryObservation/f:component/f:valueCodeableConcept"/>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:function name="nf:get-encounter-display"
                 as="xs:string?">
      ...
      <xsl:for-each select="$encounter">
         <xsl:variable name="theType"
                       select="f:type/f:coding/f:display/@value"/>
         <xsl:variable name="personName"
                       select="f:participant[f:type/f:coding/f:code/@value = 'PRF']/f:individual/f:display/@value"/>
         <xsl:variable name="theDateString"
                       select="substring(f:period/f:start/@value, 1, 10)"/>
         <xsl:variable name="theDate"
                       select="if ($theDateString castable as xs:date) then format-date(xs:date($theDateString), '[D01]-[M01]-[Y0001]') else $theDateString"/>
         <xsl:value-of select="normalize-space(string-join((string-join(($theType, string-join($personName, ' en ')), ' met '), $theDate), ' op '))"/>
      </xsl:for-each>
   </xsl:function>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template name="observationEntry"
                 match="journaalregel | journal_entry"
                 mode="doObservationEntry">
         ...
         <resource>
            <xsl:call-template name="gp-JournalEntry">
               ...
               <xsl:with-param name="subject"
                               select="ancestor::*[bundle]/bundle/subject"/>
            </xsl:call-template>
         </resource>
         <search>
            <mode value="include"/>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template name="observationReference"
                 match="unieke-observatie | f:entry | journal_entry"
                 mode="doObservationReference">
      <xsl:variable name="theObject"
                    select="."/>
      <xsl:variable name="theIdentifier"
                    select="()"/>
      <xsl:variable name="theGroupKey"
                    select="(group-key, ../group-key, nf:getGroupingKeyDefault(.))[1]"/>
      <xsl:variable name="theGroupElement"
                    select="$body-observations[group-key = $theGroupKey]"
                    as="element()?"/>
      <xsl:choose>
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template match="f:entry/f:resource"
                 mode="updateSearchMode">
      <xsl:copy-of select="."/>
   </xsl:template>
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
