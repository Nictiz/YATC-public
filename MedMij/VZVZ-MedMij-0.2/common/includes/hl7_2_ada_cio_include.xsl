...
<?yatc-distribution-provenance href="C:/Data/Erik/work/Nictiz/new/HL7-mappings/hl7_2_ada/cio/hl7_2_ada_cio_include.xsl"?>
<?yatc-distribution-info name="VZVZ-MedMij" timestamp="2024-02-06T09:13:30.86+01:00" version="0.2"?>
<!-- == Provenance: C:/Data/Erik/work/Nictiz/new/HL7-mappings/hl7_2_ada/cio/hl7_2_ada_cio_include.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:variable name="patients"
      ...
      <xsl:variable name="patient"
                    select="(//hl7:REPC_IN000024NL/hl7:ControlActProcess/hl7:subject/hl7:Condition/hl7:subject/hl7:patient)[1]"/>
      <patients>
...
-->
   ...
   <xsl:template name="HandleProblem"
                 match="hl7:Condition"
                 mode="HandleProblem">
      <xsl:param name="adaId"
                 as="xs:string?"
      ...
      <xsl:element name="{$elmProblem}">
         ...
         <xsl:element name="{$elmProblemType}">
            ...
            <xsl:attribute name="codeSystemName"
                           select="$oidMap[@oid = $oidSNOMEDCT]/@displayName"/>
         </xsl:element>
         <!-- problem name -->
         <xsl:call-template name="handleCV">
            <xsl:with-param name="in"
   ...
   <xsl:template name="HandleConditionComment"
                 match="hl7:annotation"
                 mode="HandleConditionComment">
      ...
      <xsl:variable name="comment"
         ...
         <xsl:for-each select="hl7:author/hl7:time[@value]">
            <xsl:variable name="xmlDate"
                          select="nf:formatHL72XMLDate(./@value)"/>
            <xsl:variable name="strXmlDate"
                          select="ncf:formatDate($xmlDate)"/>
            <xsl:variable name="strXmlTime"
                          select="ncf:datetime-2-timestring($xmlDate)"/>
         ...
         <xsl:for-each select="hl7:author/hl7:assignedPerson/hl7:id[@root]">
            <xsl:variable name="thisRoot"
                          select="@root"/>
            <xsl:variable name="rootDisplay"
                          select="$oidMap[@oid = $thisRoot]/@displayName"/>
      ...
      <xsl:element name="{$elmComment}">
         <xsl:attribute name="value"
                        select="normalize-space(string-join($comment, ' '))"/>
      </xsl:element>
   </xsl:template>
   <xd:doc>
   ...
   <xsl:template name="tmp-2.16.840.1.113883.2.4.3.11.60.20.77.10.111_20130525000000_2_allergy"
                 match="hl7:Condition"
                 mode="Condition2AllergyIntolerance">
      ...
      <xsl:element name="{$elmAllergyIntolerance}">
         ...
         <xsl:for-each select="hl7:causativeAgent/hl7:administerableMaterial/(hl7:administerableMedicationKind | hl7:administerableMaterialKind)">
            <xsl:call-template name="handleCV">
               <xsl:with-param name="in"
                               select="hl7:code"/>
               <xsl:with-param name="elemName"
                               select="$elmAllergyIntoleranceCausativeAgent"/>
            </xsl:call-template>
         </xsl:for-each>
         <!-- allergy category -->
         ...
         <xsl:for-each select="hl7:statusCode[@code]">
            <xsl:element name="{$elmAllergyIntoleranceAllergyStatus}">
               ...
               <xsl:attribute name="codeSystemName"
                              select="$oidMap[@oid = $oidHL7ActStatus]/@displayName"/>
            </xsl:element>
         </xsl:for-each>
         <!-- start date time -->
         <xsl:for-each select="hl7:effectiveTime/hl7:low">
            <xsl:call-template name="handleTS">
               <xsl:with-param name="in"
                               select="."/>
               <xsl:with-param name="elemName"
                               select="$elmAllergyIntoleranceStartDateTime"/>
            </xsl:call-template>
         </xsl:for-each>
   ...
   <xsl:template name="tmp-2.16.840.1.113883.2.4.3.11.60.20.77.10.810_20130521000000"
         ...
         <xsl:for-each select="hl7:subject">
            <xsl:element name="{$elmZibrootSubject}">
               <xsl:element name="{$elmPatient}">
                  ...
                  <xsl:element name="{$elmPatient}">
                     <xsl:attribute name="value"
                                    select="$patients/patient_information/patient/@id"/>
                     <xsl:attribute name="datatype">reference</xsl:attribute>
                  </xsl:element>
               </xsl:element>
            </xsl:element>
   ...
   <xsl:template name="tmp-2.16.840.1.113883.2.4.3.11.60.20.77.10.111_20130525000000_2_alert"
                 match="hl7:Condition"
                 mode="Condition2Alert">
   ...
   <xsl:template name="tmp-2.16.840.1.113883.2.4.3.11.60.20.77.10.813_20130525000000"
   ...
   <xsl:template name="tmp-2.16.840.1.113883.2.4.3.11.60.20.77.10.800_20130521000000"
                 match="hl7:patient">
      ...
      <xsl:element name="{$elmPatient}">
         ...
         <xsl:call-template name="handleTS">
            <xsl:with-param name="in"
                            select="$current-patient/hl7:Person/hl7:birthTime"/>
            <xsl:with-param name="elemName"
                            select="$elmBirthdat"/>
         </xsl:call-template>
         <!-- geslacht -->
         <xsl:call-template name="handleCV">
            <xsl:with-param name="in"
                            select="$current-patient/hl7:Person/hl7:administrativeGenderCode"/>
            <xsl:with-param name="elemName"
                            select="$elmGender"/>
         ...
         <xsl:call-template name="handleBL">
            <xsl:with-param name="in"
                            select="$current-patient/hl7:Person/hl7:multipleBirthInd"/>
            <xsl:with-param name="elemName"
   ...
   <xsl:function name="ncf:datetime-2-timestring"
                 as="xs:string?">
      <xsl:param name="in"
                 as="xs:string?"/>
      <xsl:choose>
   ...
   <xsl:function name="ncf:getDutchMonthName"
                 as="xs:string?">
   ...
   <xsl:function name="ncf:formatDate"
                 as="xs:string?">
