...
<?yatc-distribution-provenance href="C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/zibs2017/payload/zib-medicationuse-3.0.xsl"?>
<?yatc-distribution-info name="VZVZ-MedMij" timestamp="2024-01-29T11:45:25.52+01:00" version="0.1"?>
<!-- == Provenance: C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/zibs2017/payload/zib-medicationuse-3.0.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:param name="referById"
              ...
              select="false()"/>
   <!-- ================================================================== -->
   <xsl:template name="MedicationUseEntry-3.0"
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template name="zib-MedicationUse-3.0"
                 match="medicatiegebruik[not(@datatype = 'reference')][.//(@value | @code | @nullFlavor)] | medication_use[not(@datatype = 'reference')][.//(@value | @code | @nullFlavor)]"
                 mode="doZibMedicationUse-3.0"
                 as="element()">
                  ...
                  <xsl:when test="stoptype/@code or stoptype[@codeSystem = $oidHL7NullFlavor]">
                     <xsl:call-template name="util:logMessage">
                        <xsl:with-param name="level"
                                        select="$logFATAL"/>
                        <xsl:with-param name="msg">An unknown stoptype has been encountered, @code: 
<xsl:value-of select="stoptype/@code"/>, @codeSystem: 
<xsl:value-of select="stoptype/@codeSystem"/>
                        </xsl:with-param>
