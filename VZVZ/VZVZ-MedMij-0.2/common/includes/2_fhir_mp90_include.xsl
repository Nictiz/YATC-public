...
<?yatc-distribution-provenance href="C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/mp/2_fhir_mp90_include.xsl"?>
<?yatc-distribution-info name="VZVZ-MedMij" timestamp="2024-02-06T09:13:30.86+01:00" version="0.2"?>
<!-- == Provenance: C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/mp/2_fhir_mp90_include.xsl == -->
...
<xsl:stylesheet version="2.0"
   ...
   <xsl:param name="outputKopieBouwstenen"
              as="xs:boolean?"
              select="true()"/>
   <xsl:variable name="bouwstenen-907"
                 as="element(f:entry)*">
         ...
         <entry xmlns="http://hl7.org/fhir">
            ...
            <resource>
               <xsl:call-template name="zib-MedicationUse-2.0">
                  <xsl:with-param name="medicatiegebruik"
                                  select="."/>
                  <xsl:with-param name="medicationstatement-id"
                                  select="                                 if ($referById) then                                     nf:removeSpecialCharacters(./identificatie/@value)                                 else                                     ()"/>
               </xsl:call-template>
            </resource>
            <xsl:if test="string-length($searchMode) gt 0">
            ...
            <MedicationRequest xsl:exclude-result-prefixes="#all"
               ...
               <xsl:for-each select="./lichaamslengte[.//@value]">
                  <supportingInformation>
                     ...
                     <xsl:variable name="datum-string"
                                   select="                                 if (./lengte_datum_tijd/@value) then                                 format-dateTime(./lengte_datum_tijd/@value, '[D01] [MN,*-3], [Y0001] [H01]:[m01]')                                 else                                 'onbekend'"/>
               ...
               <xsl:for-each select="./lichaamsgewicht[.//@value]">
                  <supportingInformation>
                     ...
                     <xsl:variable name="datum-string"
                                   select="                                 if (./gewicht_datum_tijd/@value) then                                 format-dateTime(./gewicht_datum_tijd/@value, '[D01] [MN,*-3], [Y0001] [H01]:[m01]')                                 else                                 'onbekend'"/>
               ...
               <!-- voorschrijver -->
               <xsl:apply-templates select="./voorschrijver[.//(@value | @code)]"
   ...
   <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
   <xsl:template name="patient-entry-612"
                 match="patient"
                 mode="doPatientEntry612">
      <!--  -->
      <xsl:param name="uuid"
                 as="xs:boolean">
         <!--  -->
      ...
      <xsl:variable name="patient-fullUrl"
            ...
            <xsl:choose>
               <xsl:when test="$referById">
                  ...
                  <xsl:call-template name="patient-612-1.0">
                     <xsl:with-param name="ada-patient"
                                     select="."/>
...
<xsl:value-of select="./string-join((@value, @root), ' || ')"/>
               ...
               <!-- auteur -->
               <xsl:apply-templates select="./auteur[.//(@value | @code)]"
                                    mode="doRequesterExtension"/>
               <!-- toelichting -->
               <xsl:for-each select="./toelichting[@value]">
                  <note>
...
<xsl:value-of select="./string-join((@value, @root), ' || ')"/>
   ...
   <xsl:template name="zib-MedicationUse-2.0">
      ...
      <xsl:param name="medicationstatement-id"
                 as="xs:string?">
         <!--  -->
      </xsl:param>
      <xsl:param name="searchMode"
                  ...
                  <informationSource>
                     <xsl:choose>
                        ...
                        <xsl:when test="informant_is_zorgverlener[.//@value]">
                           <xsl:apply-templates select="./informant_is_zorgverlener/zorgverlener"
                                                mode="doPractitionerRoleReference-2.0"/>
                        </xsl:when>
                     </xsl:choose>
                  </informationSource>
