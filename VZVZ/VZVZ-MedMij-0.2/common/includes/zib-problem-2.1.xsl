...
<?yatc-distribution-provenance href="C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/zibs2017/payload/zib-problem-2.1.xsl"?>
<?yatc-distribution-info name="VZVZ-MedMij" timestamp="2024-02-06T09:13:30.86+01:00" version="0.2"?>
<!-- == Provenance: C:/Data/Erik/work/Nictiz/new/YATC-internal/ada-2-fhir/env/zibs2017/payload/zib-problem-2.1.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:template name="zib-Problem-2.1"
      ...
      <xsl:param name="in"
                 select="."
                 as="element()?">
         <!-- Node to consider in the creation of a Condition resource -->
      </xsl:param>
            ...
            <Condition>
               ...
               <clinicalStatus>
                  ...
                  <xsl:call-template name="ext-code-specification-1.0">
                     <xsl:with-param name="in"
                                     select="problem_status | probleem_status"/>
                  </xsl:call-template>
               </clinicalStatus>
               <!-- Verification Status-->
               ...
               <code>
                  <xsl:call-template name="code-to-CodeableConcept">
                     <xsl:with-param name="in"
                                     select="problem_name | probleem_naam"/>
                  </xsl:call-template>
               </code>
               ...
               <xsl:variable name="problemAnatomic"
                             select="(problem_anatomical_location | probleem_anatomische_locatie)[@value | @code]"/>
               <xsl:variable name="problemLateral"
                             select="(problem_laterality | probleem_lateraliteit)[@value | @code]"/>
                           ...
                           <xsl:attribute name="value">
                              <xsl:call-template name="format2FHIRDate">
                                 <xsl:with-param name="dateTime"
                                                 select="xs:string((problem_start_date | probleem_begin_datum)/@value)"/>
                              </xsl:call-template>
                           </xsl:attribute>
                        </start>
                     </xsl:if>
