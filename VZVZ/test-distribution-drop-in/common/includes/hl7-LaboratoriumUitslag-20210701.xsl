...
<?yatc-distribution-provenance href="C:/Data/Erik/work/Nictiz/new/HL7-mappings/ada_2_hl7/zib2020bbr/payload/hl7-LaboratoriumUitslag-20210701.xsl"?>
<?yatc-distribution-info name="VZVZ-test-distribution-drop-in" timestamp="2024-01-25T12:13:11.57+01:00" version="0.2"?>
<!-- == Provenance: C:/Data/Erik/work/Nictiz/new/HL7-mappings/ada_2_hl7/zib2020bbr/payload/hl7-LaboratoriumUitslag-20210701.xsl == -->
...
<xsl:stylesheet exclude-result-prefixes="#all"
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.66.10.77_20220330000000"
                 match="laboratorium_uitslag | laboratory_result"
                 mode="HandleLaboratoryResult">
      <xsl:param name="in"
                 as="element()?"
                 select="."/>
      <xsl:variable name="doToplevelCopyIndicator"
                    select="count($in) = count($in/kopie_indicator) and count(distinct-values($in/kopie_indicator/@value)) = 1"/>
      <xsl:variable name="doToplevelEdifactReference"
                    select="count($in) = count($in/edifact_referentienummer) and count(distinct-values($in/edifact_referentienummer/@value)) = 1"/>
                        ...
                        <organizer classCode="CLUSTER"
                           ...
                           <specimen typeCode="SPC">
                              <specimenRole classCode="ISLT">
                                 <id extension="55584739"
                                     root="1.3.6.1.4.1.19376.1.3.4"/>
               ...
               <xsl:if test="$doToplevelCopyIndicator">
                  <entryRelationship typeCode="COMP">
                     <xsl:call-template name="doCopyIndicator">
                        <xsl:with-param name="copyIndicator"
                                        select="($in/kopie_indicator)[1]"/>
                     </xsl:call-template>
                  </entryRelationship>
               </xsl:if>
               <xsl:if test="$doToplevelEdifactReference">
                  <entryRelationship typeCode="COMP">
                     <xsl:call-template name="doEdifactReference">
                        <xsl:with-param name="edifactReference"
                                        select="($in/edifact_referentienummer)[1]"/>
                     </xsl:call-template>
                  </entryRelationship>
               </xsl:if>
            </act>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.7.10.31_20220330000000"
                 match="laboratorium_test"
                 mode="HandleLaboratoryResultTest">
      <xsl:param name="in"
                 as="element()?"
                 select="."/>
      <xsl:param name="copyIndicator"
                 as="element()*"/>
      <xsl:param name="edifactReference"
                 as="element()*"/>
      <!-- Not supported: -->
      ...
      <xsl:for-each select="$in">
         ...
         <observation classCode="OBS"
               ...
               <xsl:choose>
                  ...
                  <xsl:when test=".[@code = '131196009'][@codeSystem = '2.16.840.1.113883.6.96']">
                     <interpretationCode code="S"
                                         codeSystem="2.16.840.1.113883.5.83"
                                         displayName="Ontvankelijk">
                        <translation code="131196009"
                                     codeSystem="2.16.840.1.113883.6.96"
                                     displayName="Ontvankelijk"/>
                     </interpretationCode>
                  </xsl:when>
            ...
            <xsl:analyze-string select="test_uitslag/normalize-space(@value)"
                                regex="^(&lt;|&gt;=?)">
               <xsl:matching-substring>
                  <interpretationCode code="{.}"
                                      codeSystem="2.16.840.1.113883.4.642.3.59"
                                      displayName="{.}"/>
               </xsl:matching-substring>
            </xsl:analyze-string>
   ...
   <xsl:template name="template_2.16.840.1.113883.2.4.3.11.60.7.10.3_20171016000000"
                 match="monster"
                 mode="HandleLaboratoryResultSpecimen">
      <xsl:param name="in"
                 as="element()?"
                 select="."/>
      <xsl:for-each select="$in">
   ...
   <xsl:template name="doEdifactReference">
      <xsl:param name="edifactReference"
                 as="element(edifact_referentienummer)?"/>
