...
<?yatc-distribution-provenance href="C:/Data/Erik/work/Nictiz/new/HL7-mappings/hl7_2_ada/zibs2020/payload/uni-Gebruiksinstructie.xsl"?>
<?yatc-distribution-info name="VZVZ-test-distribution-drop-in" timestamp="2024-01-25T12:13:11.57+01:00" version="0.2"?>
<!-- == Provenance: C:/Data/Erik/work/Nictiz/new/HL7-mappings/hl7_2_ada/zibs2020/payload/uni-Gebruiksinstructie.xsl == -->
                           ...
                           <toedieningsschema>
                              ...
                              <xsl:if test="(f:frequency[@value ne '1'] or f:frequencyMax[@value]) and f:extension[@url = $urlTimingExact]/f:valueBoolean/@value = 'true'">
                                 <xsl:call-template name="util:logMessage">
                                    <xsl:with-param name="level"
                                                    select="$logERROR"/>
                                    <xsl:with-param name="msg">Encountered a frequency value other than 1 (found: 
<xsl:value-of select="f:frequency/@value"/>) or a frequencyMax/@value (
<xsl:value-of select="f:frequencyMax/@value"/>) with timing-exact. This is unexpected. Not outputting interval to ada, but outputting frequentie instead</xsl:with-param>
                                 </xsl:call-template>
                                 <frequentie>
                                    <aantal>
                                       <xsl:if test="f:frequencyMax[@value] and f:frequency[@value]">
