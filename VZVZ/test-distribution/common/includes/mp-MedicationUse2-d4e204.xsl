...
<!-- == Flattened from: C:/Data/Erik/work/Nictiz/new/HL7-mappings/ada_2_fhir-r4/mp/9.3.0/payload/2.0.0-beta.2/mp-MedicationUse2.xsl == -->
<!--
Copyright © Nictiz
      ...
      <xsl:for-each select="$in">
         <MedicationStatement>
            ...
            <xsl:if test="relatie_zorgepisode/(identificatie | identificatienummer)[@value] and not(relatie_contact/(identificatie | identificatienummer)[@value])">
               <context>
                  <xsl:apply-templates select="relatie_zorgepisode/identificatienummer[@value]"
               ...
               <xsl:choose>
                  <xsl:when test="self::persoon/contactpersoon[@value]">
                     <informationSource>
                        <xsl:call-template name="makeReference">
                           <xsl:with-param name="in"
                                           select="ancestor::data/*//contactpersoon[@id = current()/contactpersoon/@value]"/>
                        </xsl:call-template>
                     </informationSource>
                  </xsl:when>
                  ...
                  <xsl:when test="self::informant_is_zorgverlener/zorgverlener[@value]">
                     <informationSource>
                        <xsl:call-template name="makeReference">
                           <xsl:with-param name="in"
                                           select="ancestor::data/*//zorgverlener[@id = current()/zorgverlener/@value]"/>
                           <xsl:with-param name="profile"
                                           select="$profileNameHealthProfessionalPractitionerRole"/>
                        </xsl:call-template>
                     </informationSource>
