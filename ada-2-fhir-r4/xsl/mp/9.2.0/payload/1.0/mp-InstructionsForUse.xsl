...
<xsl:stylesheet version="2.0" exclude-result-prefixes="#all" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:util="urn:hl7:utilities" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:uuid="http://www.uuid.org" xmlns:local="#local.2023041908255615675470200" xmlns:nm="http://www.nictiz.nl/mappings">
    <!-- ================================================================== -->
    <!--
    ...
    <xsl:template name="ext-InstructionsForUse.RepeatPeriodCyclicalSchedule" mode="ext-InstructionsForUse.RepeatPeriodCyclicalSchedule" match="gebruiks_instructie" as="element(f:modifierExtension)?">
        ...
        <xsl:for-each select="$in">
            <xsl:for-each select="herhaalperiode_cyclisch_schema[@value != '']">
                <modifierExtension url="http://nictiz.nl/fhir/StructureDefinition/ext-InstructionsForUse.RepeatPeriodCyclicalSchedule" xmlns="http://hl7.org/fhir">
                    <valueDuration>
                            ...
                            <xsl:variable name="sequence" as="element(f:sequence)*">
                                <xsl:choose>
                                    <xsl:when test="volgnummer[@value]">
                                        <sequence value="{volgnummer/@value}" xmlns="http://hl7.org/fhir"/>
                                    </xsl:when>
                                    <xsl:when test="count(dosering) gt 1">
                                        ...
                                        <sequence value="1" xmlns="http://hl7.org/fhir"/>
                                    </xsl:when>
                                </xsl:choose>
                            </xsl:variable>
                            ...
                            <xsl:choose>
                                <xsl:when test="dosering">
                                    <xsl:for-each select="dosering">
                                        ...
                                        <content xmlns="http://hl7.org/fhir">
    ...
    <xsl:template name="_buildTimingRepeat" as="element()*">
        ...
        <xsl:for-each select="toedieningsschema">
            <xsl:if test="interval[@value] and not(is_flexibel[@value])">
                <extension url="http://hl7.org/fhir/StructureDefinition/timing-exact" xmlns="http://hl7.org/fhir">
            ...
            <xsl:if test="not(frequentie//*[@unit != ''])">
                <xsl:if test="frequentie/(nominale_waarde | */nominale_waarde)[@value]">
                    <count value="{frequentie/(nominale_waarde | */nominale_waarde)/@value}" xmlns="http://hl7.org/fhir"/>
                </xsl:if>
                <xsl:if test="frequentie/(minimum_waarde | */minimum_waarde)[@value]">
                    <count value="{frequentie/(minimum_waarde | */minimum_waarde)/@value}" xmlns="http://hl7.org/fhir"/>
                </xsl:if>
                <xsl:if test="frequentie/(maximum_waarde | */maximum_waarde)[@value]">
                    <countMax value="{frequentie/(maximum_waarde | */maximum_waarde)/@value}" xmlns="http://hl7.org/fhir"/>
                </xsl:if>
            </xsl:if>
            <xsl:if test="../toedieningsduur/tijds_duur[@value]">
                <duration value="{../toedieningsduur/tijds_duur/@value}" xmlns="http://hl7.org/fhir"/>
                <xsl:if test="../toedieningsduur/tijds_duur[@unit]">
                    <durationUnit value="{nf:convertTime_ADA_unit2UCUM_FHIR(../toedieningsduur/tijds_duur/@unit)}" xmlns="http://hl7.org/fhir"/>
            ...
            <xsl:if test="frequentie//*[@unit != '']">
                ...
                <xsl:if test="$nominaleWaarde[@value]">
                    <frequency value="{$nominaleWaarde/@value}" xmlns="http://hl7.org/fhir"/>
                </xsl:if>
                <xsl:if test="$minWaarde[@value]">
                    <frequency value="{$minWaarde/@value}" xmlns="http://hl7.org/fhir"/>
                </xsl:if>
                <xsl:if test="$maxWaarde[@value]">
                    <frequencyMax value="{$maxWaarde/@value}" xmlns="http://hl7.org/fhir"/>
                </xsl:if>
                <period value="{(frequentie/*[@unit])[1]/@value}" xmlns="http://hl7.org/fhir"/>
                <periodUnit value="{nf:convertTime_ADA_unit2UCUM_FHIR((frequentie/*[@unit]/@unit)[1])}" xmlns="http://hl7.org/fhir"/>
            </xsl:if>
            <xsl:if test="interval[@value and @unit]">
                <frequency value="{((frequentie/*[@unit])[1]/@value, '1')[1]}" xmlns="http://hl7.org/fhir"/>
                <period value="{interval/@value}" xmlns="http://hl7.org/fhir"/>
                <periodUnit value="{nf:convertTime_ADA_unit2UCUM_FHIR(interval/@unit)}" xmlns="http://hl7.org/fhir"/>
            </xsl:if>
            <xsl:for-each select="weekdag">
                <dayOfWeek value="{$weekdayMap[@code = current()/@code][@codeSystem=current()/@codeSystem]/@fhirDayOfWeek}" xmlns="http://hl7.org/fhir"/>
            </xsl:for-each>
                     ...
                     So we just let the user create a dateTime and discard the date part. -->
                        <xsl:analyze-string select="@value" regex="[0-9\-]*T([0-9:Z]+)">
                            <xsl:matching-substring>
                                <timeOfDay value="{regex-group(1)}" xmlns="http://hl7.org/fhir"/>
                            </xsl:matching-substring>
                        </xsl:analyze-string>
                    </xsl:when>
                    <xsl:when test="matches(@value, '[0-9:Z]+')">
                        <!-- mp9 ada has datatype time -->
                        <timeOfDay value="{@value}" xmlns="http://hl7.org/fhir"/>
                    </xsl:when>
                    <xsl:otherwise>
            ...
            <xsl:for-each select="dagdeel">
                <when value="{$daypartMap[@code = current()/@code][@codeSystem=current()/@codeSystem]/@fhirWhen}" xmlns="http://hl7.org/fhir"/>
