...
<xsl:stylesheet version="2.0" exclude-result-prefixes="#all" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:util="urn:hl7:utilities" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:uuid="http://www.uuid.org" xmlns:local="#local.2023041908255671598790200">
    <!-- ================================================================== -->
    <!--
    ...
    <xsl:template name="nl-core-LaboratoryTestResult-singular" match="laboratorium_uitslag[count(laboratorium_test) = 1]" mode="nl-core-LaboratoryTestResult" as="element(f:Observation)?">
        <!-- Create a single nl-core-LaboratoryTestResult instance as an Observation FHIR instance from ADA laboratorium_uitslag for a singlular test, that is, when it contains a single laboratorium_test. -->
    ...
    <xsl:template name="_nl-core-LaboratoryTestResult-panel" match="laboratorium_uitslag[count(laboratorium_test) gt 1]" mode="nl-core-LaboratoryTestResult" as="element(f:Observation)">
        <!-- Create a hierarchy of nl-core-LaboratoryTestResult instances as Observation FHIR instances from ADA laboratorium_uitslag for a panel test, that is, when a single laboratoriunm_uitslag contains multiple laboratorium_test's. -->
        <xsl:param name="in" as="element()?" select=".">
            <!-- ADA element as input. Defaults to self. -->
        </xsl:param>
    ...
    <xsl:template name="_nl-core-LaboratoryTestResult-individualTest" mode="_nl-core-LaboratoryTestResult-individualTest" match="laboratorium_test" as="element(f:Observation)?">
        <!-- 
            Helper template to create a nl-core-LaboratoryTestResult instance representing a single LaboratoryTest (depending on the situation, this might be one of the tests within a panel or the entire zib).
    ...
    <xsl:template name="nl-core-LaboratoryTestResult.Specimen" mode="nl-core-LaboratoryTestResult.Specimen" match="monster" as="element(f:Specimen)?">
        <!-- Helper template to create a nl-core-LaboratoryTestResult.Specimen instance as Specimen FHIR instance from ADA laboratorium_uitslag/monster. This template can result in two slightly different outputs based on the type parameter:
    ...
    <xsl:template name="nl-core-LaboratoryTestResult.Specimen.Source" match="bron_monster" as="element(f:Device)?">
        <!-- Helper template to create a nl-core-LaboratoryTestResult.Specimen.Source instance as Device FHIR instance from ADA laboratorium_uitslag/monster/bron_monster. -->
        <xsl:param name="in" as="element()?" select=".">
            <!-- ADA element as input. Defaults to self. -->
        </xsl:param>
        <xsl:param name="subject" select="$in/../../../../patientgegevens/patient" as="element()?">
    ...
    <xsl:template match="laboratorium_uitslag" mode="_generateDisplay"><!-- Template to generate a display that can be shown when referencing this instance. -->
    </xsl:template>
</xsl:stylesheet>
