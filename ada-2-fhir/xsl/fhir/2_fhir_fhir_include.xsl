...
<xsl:stylesheet version="2.0" exclude-result-prefixes="#all" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:util="urn:hl7:utilities" xmlns:f="http://hl7.org/fhir" xmlns:nf="http://www.nictiz.nl/functions" xmlns:yatcs="https://nictiz.nl/ns/YATC-shared" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:uuid="http://www.uuid.org" xmlns:local="urn:fhir:stu3:functions">
    <!-- ================================================================== -->
    <!--
        TBD
    -->
    ...
    <xsl:template name="boolean-to-boolean" as="item()?">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$in/@nullFlavor">
                <extension url="{$urlExtHL7NullFlavor}" xmlns="http://hl7.org/fhir">
    ...
    <xsl:template name="date-to-datetime" as="item()?">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$in/@nullFlavor">
                <extension url="{$oidHL7NullFlavor}" xmlns="http://hl7.org/fhir">
                    <valueCode value="{$in/@nullFlavor}"/>
                </extension>
            </xsl:when>
        </xsl:choose>
    ...
    <xsl:template name="blob-to-attachment" as="item()?">
        ...
        <xsl:choose>
            <xsl:when test="$in/@value">
                <data value="{$in/@value}" xmlns="http://hl7.org/fhir"/>
            </xsl:when>
            <xsl:when test="$in/@nullFlavor">
                <extension url="{$urlExtHL7NullFlavor}" xmlns="http://hl7.org/fhir">
                    <valueCode value="{$in/@nullFlavor}"/>
    ...
    <xsl:template name="code-to-CodeableConcept" as="element()*">
        ...
        <xsl:param name="elementName" as="xs:string?">
            <!-- Optionally provide the element name, default = coding. In extensions it is valueCoding. --> coding</xsl:param>
        ...
        <xsl:if test="$in[@originalText]">
            <text value="{replace($in/@originalText, '(^\s+)|(\s+$)', '')}" xmlns="http://hl7.org/fhir"/>
        </xsl:if>
    </xsl:template>
        ...
        <xsl:choose>
            ...
            <xsl:when test="$in[not(@codeSystem = $oidHL7NullFlavor) or $treatNullFlavorAsCoding]">
                ...
                <xsl:for-each select="$in/@codeSystem">
                    <system value="{local:getUri(.)}" xmlns="http://hl7.org/fhir"/>
                </xsl:for-each>
                ...
                <code value="{normalize-space($in/@code)}" xmlns="http://hl7.org/fhir"/>
                ...
                <xsl:if test="exists($userSelected)">
                    <userSelected value="{$userSelected}" xmlns="http://hl7.org/fhir"/>
                </xsl:if>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="hoeveelheid-to-Duration" as="element()*">
        ...
        <xsl:choose>
            <xsl:when test="$in[@value]">
                <value value="{replace($in/@value, '(^\s+)|(\s+$)', '')}" xmlns="http://hl7.org/fhir"/>
                <xsl:if test="$unit-UCUM">
                    <unit value="{replace($in/@unit, '(^\s+)|(\s+$)', '')}" xmlns="http://hl7.org/fhir"/>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:for-each select="$in[@unit]">
                    <!-- UCUM -->
                    <unit value="{./@unit}" xmlns="http://hl7.org/fhir"/>
                    <xsl:choose>
                        <xsl:when test="$in[@datatype = 'currency']">
                            <system value="urn:iso:std:iso:4217" xmlns="http://hl7.org/fhir"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <system value="{local:getUri($oidUCUM)}" xmlns="http://hl7.org/fhir"/>
                        </xsl:otherwise>
                    </xsl:choose>
                    <code value="{nf:convert_ADA_unit2UCUM_FHIR(./@unit)}" xmlns="http://hl7.org/fhir"/>
                </xsl:for-each>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="hoeveelheid-complex-to-Quantity" as="element()*">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <value value="{$waarde/@value}" xmlns="http://hl7.org/fhir"/>
                <xsl:for-each select="$eenheid[@code]">
                    <xsl:for-each select="./@displayName">
                        <unit value="{replace(., '(^\s+)|(\s+$)', '')}" xmlns="http://hl7.org/fhir"/>
                    </xsl:for-each>
                    <xsl:for-each select="./@codeSystem">
                        <system value="{local:getUri(.)}" xmlns="http://hl7.org/fhir"/>
                    </xsl:for-each>
                    <code value="{if (@codeSystem = $oidUCUM) then nf:convert_ADA_unit2UCUM_FHIR($eenheid/@code) else $eenheid/@code}" xmlns="http://hl7.org/fhir"/>
                </xsl:for-each>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="id-to-id" as="element()?">
        ...
        <xsl:if test="matches($theID, '^[A-Za-z\d\.-]{1,64}$')">
            <id value="{$theID}" xmlns="http://hl7.org/fhir"/>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="id-to-Identifier" as="element()*">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$in[string-length(@root) gt 0][@root = $mask-ids-var] or $in[@nullFlavor = 'MSK' and string-length(@root) gt 0]">
                <system value="{local:getUri($in/@root)}" xmlns="http://hl7.org/fhir"/>
                <value xmlns="http://hl7.org/fhir">
            ...
            <xsl:when test="$in[@value | @root]">
                ...
                <xsl:for-each select="$in/@value">
                    <value value="{replace(., '(^\s+)|(\s+$)', '')}" xmlns="http://hl7.org/fhir"/>
                </xsl:for-each>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:function name="nf:getUriFromAdaId" as="xs:string">
        <!-- Requires param adaIdentification or resourceId. If possible generates a uri based on oid or uuid from input. If not possible generates an uri based on gerenated uuid making use of input element -->
        <xsl:param name="adaIdentification" as="element()?">
            <!-- input element for which uri is needed -->
        </xsl:param>
    ...
    <xsl:function name="nf:try-complex-as-Quantity" as="element()*">
        ...
        <xsl:analyze-string select="normalize-space($value_string)" regex="^([\d\.,]+)\s*(.*)$">
            <xsl:matching-substring>
                <value value="{replace(regex-group(1), ',', '.')}" xmlns="http://hl7.org/fhir"/>
                <xsl:if test="regex-group(2)">
                    <unit value="{regex-group(2)}" xmlns="http://hl7.org/fhir"/>
                </xsl:if>
            </xsl:matching-substring>
        </xsl:analyze-string>
    </xsl:function>
    ...
    <xsl:template name="format2FHIRDate">
        ...
        <xsl:param name="precision">
            <!-- Determines the precision of the output. Precision of minutes outputs seconds as '00' --> second</xsl:param>
    ...
    <xsl:template name="NullFlavor-to-DataAbsentReason" as="element()?">
        ...
        <xsl:for-each select="$in[@codeSystem = $oidHL7NullFlavor]">
            <extension url="{$urlExtHL7DataAbsentReason}" xmlns="http://hl7.org/fhir">
                <valueCode>
                    <xsl:attribute name="value">
                        <xsl:choose>
