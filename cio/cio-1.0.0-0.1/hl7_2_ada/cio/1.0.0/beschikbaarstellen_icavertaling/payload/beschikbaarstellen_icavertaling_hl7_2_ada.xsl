...
<!-- == Provenance: HL7-mappings/hl7_2_ada/cio/1.0.0/beschikbaarstellen_icavertaling/payload/beschikbaarstellen_icavertaling_hl7_2_ada.xsl == -->
...
<xsl:value-of select="concat('root: ', ./hl7:id/@root, ' and extension: ', ./hl7:id/@extension)"/>.</xsl:comment>
         ...
         <beschikbaarstellen_icavertaling app="cio"
   ...
   <xsl:template match="/">
      <xsl:if test="$ica612Root">
         <adaxml xsi:noNamespaceSchemaLocation="../ada_schemas/ada_beschikbaarstellen_icavertaling.xsd">
            <meta status="new"
               ...
               <xsl:for-each select="$transactionResult/beschikbaarstellen_icavertaling">
                  <xsl:copy>
                     <xsl:apply-templates select="@* | node()"
                                          mode="adaOutput"/>
                     <!-- output problems -->
                     <xsl:apply-templates select="alert/conditie/probleem[*]"
                                          mode="adaOutputHcim"/>
   ...
   <xsl:template match="*[not(local-name() = $elmPatient)][@id]"
                 mode="adaOutput">
